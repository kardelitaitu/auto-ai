    ...
    const portChecks = await Promise.allSettled(
      bravePorts.map(async (bravePort) => {
        try {
          ...
          logger.debug(`Found ${browserTabs.length} browser tabs from Brave on port ${bravePort}`);
          
          let browserWsUrl = null;
          try {
            const versionResponse = await fetch(`http://127.0.0.1:${bravePort}/json/version`, { 
