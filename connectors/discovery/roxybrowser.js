...
class RoxybrowserDiscover extends BaseDiscover {
  ...
  async discover() {
    ...
    try {
      ...
      logger.info(`Received profile info with code: ${profileInfo.code}, message: ${profileInfo.msg || 'none'}`);

      if (profileInfo.code !== 0 || !profileInfo.data || !Array.isArray(profileInfo.data) || profileInfo.data.length === 0) {
      ...
      const discoveredProfiles = profileInfo.data
        .filter(profile => {
          if (!profile.ws && !profile.http) {
            logger.warn(`Profile ${profile.sortNum || 'unknown'} missing ws and http fields, skipping`);
            return false;
          }
          return true;
        })
        .map((profile, index) => ({
          ...
          name: profile.name || profile.windowName || `Roxybrowser Profile ${profile.sortNum || index}`,
