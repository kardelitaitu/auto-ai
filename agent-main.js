...
function parseArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === 'play') {
            result.mode = result.loops !== GAME_CONFIG.maxLoops ? 'play' : 'playInfinite';
            continue;
        }
...
(async () => {
    ...
    logger.info(`Mode: ${args.mode}, Strategy: ${args.strategy}, Model: ${args.model}`);
    ...
    try {
        ...
        switch (args.mode) {
            ...
            case 'state':
                for (let i = 0; i < args.repeat; i++) {
                    logger.info(`Running state ${args.target} (repeat ${i + 1}/${args.repeat})`);
                    result = await owbAgents.executeState({ key: args.target, name: args.target });
                }
                break;
            case 'build':
        ...
        if (args.exitOnComplete) {
            ...
            logger.info('Cycle complete. Keeping browser open. Press Ctrl+C to exit.');
            await new Promise(() => {});
        }
