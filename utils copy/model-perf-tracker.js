...
export class ModelPerfTracker {
  ...
  trackSuccess(model, duration, apiKey = null) {
    ...
    logger.debug(`[ModelPerfTracker] ${key} success (${stat.successes}/${stat.total}, ${stat.recentAvgDuration.toFixed(0)}ms avg)`);
  }
  ...
  trackFailure(model, error, apiKey = null) {
    ...
    logger.debug(`[ModelPerfTracker] ${key} failure (${stat.failures}/${stat.total}, error: ${error.substring(0, 50)}...)`);
  }
    ...
    this.stats.forEach((stat, key) => {
      result[key] = {
        ...
        successRate: stat.total > 0
          ? ((stat.successes / stat.total) * 100).toFixed(1) + '%'
          : 'N/A',
        avgDuration: stat.avgDuration.toFixed(0) + 'ms',
        recentSuccessRate: stat.recentTotal > 0
  ...
  getStats() {
    ...
    return {
      ...
      totalSuccesses,
      totalFailures,
      overallSuccessRate: total > 0
        ? ((totalSuccesses / total) * 100).toFixed(1) + '%'
        : 'N/A',
      avgDuration: total > 0
        ? (totalDuration / total).toFixed(0) + 'ms'
        : 'N/A',
