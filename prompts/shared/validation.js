...
export function validateStateDetection(response) {
    const errors = [];
    const validStates = ['A', 'B', 'C', 'D', 'E', 'FREE_TERRITORY', 'ENEMY_TERRITORY', 'OWN_TERRITORY', 'BUILDING_MENU', 'BUILD_OPTIONS'];

    if (!response) {
        return { valid: false, errors: ['Response is null or undefined'], parsed: null };
    }
    ...
    if (!response.state) {
        ...
        errors.push(`Invalid state: ${response.state}. Must be one of: ${validStates.join(', ')}`);
    }
 ...
 * @returns {object} Validation result {valid, errors, parsed}
 */
export function validateCoordinateResponse(response, { width, height, margin = 10 }) {
    const errors = [];
...
export function validateActionResponse(response) {
    const errors = [];
    const validActions = ['clickAt', 'click', 'drag', 'type', 'wait', 'done', 'none'];
    ...
    if (response.action === 'done' || response.action === 'none') {
        return {
            valid: true,
            errors: [],
            parsed: { action: response.action, rationale: response.rationale || 'No action needed' }
        };
    }
    ...
    if (!response.action) {
        ...
        errors.push(`Invalid action: ${response.action}. Must be one of: ${validActions.join(', ')}`);
    }
