...
import { STRATEGIES as _STRATEGIES, GAME_CONFIG, GAME_MECHANICS, BUILDING_COSTS, LAND_COSTS as _LAND_COSTS, LLM_CONFIG } from './owb-config.js';
...
async function getOwbLlmClient() {
    ...
    logger.info(`[OWB] Initialized OWB-specific LLM client: model=${owbLlmClient.config.model}, maxTokens=${owbLlmClient.config.maxTokens}, timeout=${owbLlmClient.config.timeoutMs}ms`);
    return owbLlmClient;
}
...
async function getViewportDimensions() {
    ...
    if (viewport) {
        ...
        if (mismatchX > 50 || mismatchY > 50) {
            logger.warn(`[Viewport] MISMATCH detected! viewportSize(): ${viewport.width}x${viewport.height}, window.inner: ${jsSize.width}x${jsSize.height}. Using window.inner values.`);
...
async function dragToCenterAndClick(targetX, targetY, duration = 700) {
    ...
    logger.info(`[dragToCenter] Click hold at (${targetX}, ${targetY}), drag to center (${endX}, ${endY}), duration ${duration}ms`);
...
function getNextState() {
    ...
    if (requestCount % 5 === 0) {
        ...
        logger.info(`[StateMachine] Randomized state order: ${stateOrder.join(' → ')}`);
    }
...
export async function detectState() {
    logger.info('[State Detection] Analyzing game state with modular prompt system...');
    ...
    const state = await api.agent.captureState({
        ...
        axTree: false,
        vprep: true,
        vprepConfig: api.vprep?.presets?.OWB_GAME || { targetWidth: 640, contrast: 1.25, quality: 78 }
    });

    const screenshotSize = state.screenshot ? state.screenshot.length : 0;
    const base64SizeKB = Math.round(screenshotSize / 1024);
    logger.info(`[LLM Image] Base64 length: ${screenshotSize} chars (~${base64SizeKB} KB)`);
    ...
    logger.info(`[State Detection] Viewport: ${viewport.width}x${viewport.height}, V-PREP: ${vprepWidth}x${vprepHeight}`);
    ...
    try {
        ...
        for (const stateKey of stateKeys) {
            try {
                ...
                const messages = [
                    ...promptPackage.messages,
                    {
                        ...
                        content: [
                            ...
                            { type: 'image_url', image_url: { url: `data:image/jpeg;base64,${state.screenshot}` } }
                        ]
                    }
                ];

                // Replace system message with proper content
                messages[0] = { role: 'system', content: promptPackage.messages[0].content };
                ...
                logger.warn(`[State Detection] State ${stateKey} detection error: ${stateErr.message}`);
            }
        }

        // Fallback: If no state detected with high confidence, use the original single-prompt approach
        if (bestMatch.confidence < 0.5) {
            logger.info('[State Detection] Low confidence, using fallback detection...');
...
{"state": "A|B|C|D|E", "confidence": 0.0-1.0, "reason": "explanation"}
            ...
            const fallbackMessages = [
                { role: 'system', content: 'You are a game state detector for a hexagonal territory game.' },
                {
                    role: 'user',
                    content: [
            ...
            const fallbackResult = await owbClient.generateCompletion(fallbackMessages);
        ...
        logger.info(`[State Detection] Detected State: ${detected} (${STATES[detected]?.desc}) - Confidence: ${(bestMatch.confidence * 100).toFixed(1)}%`);
        ...
        return { key: 'A', ...STATES.A, analysis: { state: 'A', confidence: 0, reason: 'Error fallback' } };
    }
}
...
async function _runStateAction(prompt, stateName) {
    ...
    const state = await api.agent.captureState({
        screenshot: true,
        vprep: true,
        vprepConfig: api.vprep?.presets?.OWB_GAME || { targetWidth: 1024, contrast: 1.25, quality: 78 }
    });
    ...
    try {
        ...
        if (llmResult?.x !== undefined && llmResult?.y !== undefined) {
            ...
            x = llmResult[0].x;
            y = llmResult[0].y;
        } else if (llmResult?.coordinates && Array.isArray(llmResult.coordinates) && llmResult.coordinates.length > 0) {
        ...
        if (x !== undefined && y !== undefined &&
            ...
            x >= 10 && x <= viewport.width - 10 && y >= 10 && y <= viewport.height - 10) {
            result.x = x;
            result.y = y;
            result.success = true;
            ...
            await debugScreenshot(`after-${stateName.toLowerCase().replace(' ', '')}`);
        } else {
            logger.warn(`[${stateName}] Invalid coordinates: (${x}, ${y}) - must be within viewport ${viewport.width}x${viewport.height}`);
        }
    } catch (e) {
...
export async function debugScreenshot(label, base64 = null) {
    ...
    try {
        if (base64) {
            ...
            logger.info(`[DEBUG] V-PREP processed image saved: ${filename} (${Math.round(buffer.length / 1024)} KB)`);
        } else {
...
async function executeStateA() {
    ...
    try {
        ...
        if (page) {
            ...
            if (canvasInfo) {
                ...
                logger.info(`[State A] Canvas detected at (${canvasInfo.x}, ${canvasInfo.y}), size: ${canvasInfo.width}x${canvasInfo.height}`);
            }
        }
    } catch (e) {
    ...
    const MAX_SCALE = 2.0;
    const vprepTargetWidth = Math.min(1280, Math.ceil(viewport.width / MAX_SCALE));
    const vprepTargetHeight = Math.min(720, Math.ceil(viewport.height / MAX_SCALE));
    ...
    logger.info(`[State A] V-PREP config: ${vprepTargetWidth}x${vprepTargetHeight} (max scale: ${MAX_SCALE}x)`);
    ...
    const vprepOutputWidth = state.vprepStats?.outputDimensions?.width || vprepTargetWidth;
    const vprepOutputHeight = state.vprepStats?.outputDimensions?.height || vprepTargetHeight;
    const vprepScaleFactorX = viewport.width / vprepOutputWidth;
    const vprepScaleFactorY = viewport.height / vprepOutputHeight;
    logger.info(`[V-PREP] Output: ${vprepOutputWidth}x${vprepOutputHeight}, Viewport: ${viewport.width}x${viewport.height}, Scale factors: X=${vprepScaleFactorX.toFixed(3)}, Y=${vprepScaleFactorY.toFixed(3)}`);
...
`.trim();
    ...
    const owbClient = await getOwbLlmClient();
    console.log(`[AGENT_STATE] Dispatching prompt to ${owbClient.config.model} vision model...`);
    ...
    try {
        ...
        async function validateNotForbidden(x, y) {
            try {
                ...
                if (isBlack) {
                    logger.warn(`[State A] ❌ BLACK (masked blue) at (${x}, ${y}): RGB(${r},${g},${b}) - FORBIDDEN`);
                    return false;
                }

                if (isBlue) {
                    logger.warn(`[State A] ❌ BLUE at (${x}, ${y}): RGB(${r},${g},${b}) - FORBIDDEN`);
                    return false;
                }
        ...
        for (const target of targets) {
            ...
            if (x !== undefined && y !== undefined) {
                ...
                logger.info(`[State A] Coords: (${origX}, ${origY}) -> scaled (${x - canvasOffset.x}, ${y - canvasOffset.y}) -> final (${x}, ${y})`);
        ...
        if (validTargets.length > 0) {
            ...
            for (let i = 0; i < validTargets.length; i++) {
                ...
                logger.info(`[State A] Target ${i + 1}/${validTargets.length}: (${x}, ${y}) price=${price || '?'}`);
            ...
            if (DEBUG) await debugScreenshot(`state-a-verified-${Date.now()}`, screenshotBase64);
            ...
            logger.info(`[State A] Action completed successfully - clicked ${validTargets.length} target(s)`);
        } else {
...
export async function executeState(stateInfo = null) {
    ...
    if (process.env.DEBUG === 'true' && api.visualDebug && typeof api.visualDebug.enable === 'function') {
    ...
    switch (state.key) {
        ...
        default:
            logger.warn(`[executeState] Unknown state key: ${state.key}, defaulting to State A`);
            return await executeStateA();
    }
}
    ...
    try {
        ...
        if (parsed) {
            ...
            if (x >= 10 && x <= viewport.width - 10 && y >= 10 && y <= viewport.height - 10) {
                ...
                await dragToCenterAndClick(x, y);
                await api.wait(1000);
                await debugScreenshot(`state-b-verified-${Date.now()}`, state.screenshot);
    ...
    try {
        ...
        if (parsed) {
            ...
            if (x >= 10 && x <= viewport.width - 10 && y >= 10 && y <= viewport.height - 10) {
                ...
                logger.info(`[State C] Valid coords: (${x}, ${y}), clicking to open build menu...`);
                await api.clickAt(x, y, { speed: 'fast' });
                await api.wait(1000);
                await debugScreenshot(`state-c-verified-${Date.now()}`, state.screenshot);
    ...
    try {
        ...
        if (parsed && parsed.found !== false) {
            ...
            if (x >= 10 && x <= viewport.width - 10 && y >= 10 && y <= viewport.height - 10) {
                ...
                logger.info(`[State D] Valid coords: (${x}, ${y}), action: ${parsed.action}...`);
                await api.clickAt(x, y, { speed: 'fast' });
                await api.wait(1000);
                await debugScreenshot(`state-d-verified-${Date.now()}`, state.screenshot);
    ...
    try {
        ...
        if (parsed && parsed.found !== false) {
            ...
            if (x >= 10 && x <= viewport.width - 10 && y >= 10 && y <= viewport.height - 10) {
                ...
                logger.info(`[State E] Valid coords: (${x}, ${y}), selecting ${parsed.building}...`);
                await api.clickAt(x, y, { speed: 'fast' });
                await api.wait(1000);
                await debugScreenshot(`state-e-verified-${Date.now()}`, state.screenshot);
...
export async function buildBuilding(buildingType, count = 1) {
    ...
    logger.info(`State 2: Build/Upgrade Menu - Building ${count} ${building.name}(s) - cost: ${cost} gold`);
...
export async function autoPlay(_strategyName = null, options = {}) {
    ...
    logger.info(`States: A=GreyFree, B=EnemyNoBuilding, C=BlueNoBuilding, D=OwnBlue, E=MenuBuildUpgrade`);
    ...
    return { success: successCount > 0, loops: config.maxLoops, successCount, failCount };
}
...
export async function buildStructure(name) { return await buildBuilding(name, 1); }
export async function trainUnits(name, count) { return await buildBuilding(name, count); }
export async function attack() { return await runGoal('Attack enemy buildings'); }
export async function gatherResources() { return await gatherGold(); }
export async function buildEconomy(count) { return await buyLand(count); }
export async function trainArmy() { return await buildBuilding('defensive', 5); }
export async function explore() { return await runGoal('Explore the map'); }
export async function rushStrategy() { return await autoPlay('rush', { maxLoops: 5 }); }
export async function turtleStrategy() { return await autoPlay('turtle', { maxLoops: 5 }); }
