    ...
    if (tasksToRun.length === 0) {
      ...
      for (let i = 0; i < taskGroups.length; i++) {
        if (MAX_PROCESS_TIMEOUT && (Date.now() - overallStartTime) >= MAX_PROCESS_TIMEOUT) {
          logger.error(`[V2] Overall process timeout (${MAX_PROCESS_TIMEOUT}ms) exceeded`);
          break;
        }
        ...
        logger.info(`[V2] Group ${i + 1} tasks queued. Waiting (timeout: ${groupTimeoutMs}ms)...`);
        
        const result = await orchestrator.waitForTasksToComplete({ timeoutMs: groupTimeoutMs });
        ...
        if (result.timedOut) {
          ...
          if (activeTasks.length > 0) {
            ...
            activeTasks.forEach(t => logger.warn(`  - ${t.taskName} (elapsed: ${t.elapsed}ms)`));
          }
        } else {
