...
/**
 ...
 class CloudClient {
     ...
     async _loadConfig() {
         try {
             ...
             if (providers.length > 1) {
                 ...
                 logger.info(`CloudClient initialized with ${providers.length} API keys (fallback enabled)`);
     ...
     async sendRequest(request) {
        ...
        try {
            ...
            const messages = [
                {
                    role: 'system',
                    content: 'You are an intelligent agent executing browser automation tasks. Respond with clear, actionable JSON when requested.'
                },
                {
                    role: 'user',
    ...
    async _sendWithMultiClient(request, startTime, maxTokens, temperature) {
        ...
        const systemContent = request.payload?.systemPrompt ||
        ...
        try {
            ...
            if (response.success) {
                ...
                if (response.keyUsed !== undefined && response.keyUsed > 0) {
                    ...
                    logger.info(`[Cloud] Used key ${response.keyUsed + 1} after ${response.keyUsed} fallback(s)`);
                }
      ...
      async _sendWithFreeRouter(request, startTime) {
          ...
          if (modelsInfo.testedWorking.length > 0 && modelsInfo.testedWorking.length < modelsInfo.allConfigured.length) {
              logger.info(`[Cloud] Using ${modelsInfo.testedWorking.length}/${modelsInfo.allConfigured.length} tested working models`);
          }
