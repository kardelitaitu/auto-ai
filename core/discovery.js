...
class Discovery {
  constructor() {
    ...
    this.connectors = [];
    this.discoveryDir = path.join(path.dirname(fileURLToPath(import.meta.url)), '../connectors/discovery');
  }
  ...
  async loadConnectors(allowedConnectors = []) {
    try {
      let files = fs.readdirSync(this.discoveryDir)
        ...
        try {
          ...
          if (stat.isFile()) {
            ...
            if (ConnectorClass && typeof ConnectorClass === 'function') {
              ...
              logger.warn(`Skipping ${connectorName}: invalid connector class export. Export was: ${typeof ConnectorClass}`);
            }
          }
        } catch (error) {
  ...
  async discoverBrowsers() {
    ...
    logger.info(`Starting browser discovery with ${this.connectors.length} connectors in parallel...`);
    const allEndpoints = [];
