...
async function analyzeLogTimeRange(logPath, startTime, endTime) {
    ...
    return {
        ...
        errorRate:
            inRange.length > 0 ? ((errors.length / inRange.length) * 100).toFixed(2) + '%' : '0%',
        samples: {
            first: inRange[0],
...
async function categorizeErrors(logPath) {
    ...
    const sorted = Object.entries(categorized)
        ...
        .map(([category, errors]) => ({
            ...
            count: errors.length,
            percentage:
                ((errors.length / lines.filter((l) => l.includes('[ERROR]')).length) * 100).toFixed(
                    1
                ) + '%',
...
console.log(
    `Root cause: ${parsed.rootCause.function} at ${parsed.rootCause.file}:${parsed.rootCause.line}`
);
```
...
async function profileTaskExecution(taskName, taskFn) {
    ...
    profile.analysis = {
        ...
        memory: {
            ...
            growth:
                profile.memorySnapshots.length > 1
...
async function retryWithDebug(fn, options = {}) {
    const { maxAttempts = 3, delay = 1000, backoff = 'exponential', onRetry = null } = options;
