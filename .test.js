        ...
        try {
            ...
            if (res.ok) {
                ...
                if (!models.includes(TARGET_MODEL)) {
                    console.warn(`[DISCOVERY] ${TARGET_MODEL} not found. Defaulting to ${models[0]}`);
...
async function orchestrateBenchmark() {
    ...
    if (!discovered) {
        console.error("[FATAL] No valid vLLM endpoints discovered. Ensure Model Runner is active.");
        return;
    }

    console.log(`[INIT] Dispatching ${CONCURRENT_AGENTS} parallel sequences to ${TARGET_MODEL}...`);
    
    // Standardized prompt to test Radix/Paged cache hit rate
    const systemPrompt = "Explain the architectural advantages of modular system design in 3 paragraphs.";
    ...
    console.log(`DPS = Damage (${avgDamage.toFixed(2)} t/s) × CPS (${cps}) = Result (${finalDps} Total Tokens/sec)`);
}
