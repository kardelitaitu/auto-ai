    ...
    await Promise.race([
      api.withPage(page, async () => {
        ...
        await page.route('**/*', (route) => {
          ...
          if (type === 'media' ||
            /video|googlevideo|youtube|vimeo|twitch|tiktok|dailymotion|stream/i.test(url)) {
        ...
        for (let i = 0; i < loopCount; i++) {
          ...
          try {
            ...
            const scrollDuration = ((Date.now() - scrollStart) / 1000).toFixed(1);
            logger.info(`${domain} scrolled for ${scrollDuration}s`);

            await api.wait(CONFIG.postScrollDelay);
            const readTime = (api.randomInRange(CONFIG.minReadSecond * 10, CONFIG.maxReadSecond * 10) / 10).toFixed(1);
