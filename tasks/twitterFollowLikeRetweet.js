...
export default async function twitterFollowLikeRetweetTask(page, payload) {
    ...
    try {
        ...
        await Promise.race([
            (async () => {
                ...
                if (typeof MANUAL_REFERRER === 'string' && MANUAL_REFERRER !== '' && Math.random() < 0.20) {
                    ...
                    logger.info(`[followLikeRetweet][Anti-Sybil] Referrer: ${ctx.referrer || '(Direct Traffic - No Referrer)'}`);
                }
                ...
                logger.info(`[followLikeRetweet] Reading Tweet for ${(tweetReadTime / 1000).toFixed(1)}s...`);
                ...
                if (page.url().includes('/status/')) {
                    logger.warn(`[followLikeRetweet] ⚠️ Navigation check: Still on status page. Retrying with Avatar...`);
                ...
                if (!page.url().includes('/status/')) {
                    ...
                    logger.warn(`[followLikeRetweet] 🛑 Failed to navigate to profile. Still on status page: ${page.url()}`);
                }
                ...
                const followResult = await agent.robustFollow('[followLikeRetweet]', profileUrl);

                if (followResult.success && followResult.attempts > 0) {
                    const username = extractUsername(targetUrl);
                    logger.info(`[followLikeRetweet] ✅✅ Followed '\x1b[94m${username}\x1b[0m' Successfully ✅✅✅`);
                ...
                try {
                    ...
                    await client.send('Runtime.evaluate', { expression: 'window.history.back()' });
                    ...
                    logger.warn(`[followLikeRetweet] CDP Back failed: ${e.message}. Fallback to page.goBack().`);
                    try {
                        await page.goBack();
                        await page.waitForTimeout(mathUtils.randomInRange(2000, 4000));
                } catch (e2) {
                ...
                const goldenZone = async (locator) => {
                    try {
                        ...
                        logger.warn(`[followLikeRetweet][goldenZone] Scroll failed: ${e.message}`);
                    }
                };
                ...
                for (let attempt = 1; attempt <= 3; attempt++) {
                    ...
                    if (await retweetBtn.count() > 0 && await retweetBtn.isVisible()) {
                        if (attempt === 1) await goldenZone(retweetBtn);

                        logger.info(`[followLikeRetweet] Clicking Retweet button (Attempt ${attempt})...`);
                        try {
                            ...
                            if (await confirmBtn.isVisible()) {
                                ...
                                logger.warn(`[followLikeRetweet] Retweet confirm menu not found.`);
                                // If menu didn't appear, likely click failed. Retry loop handles this.
                                continue;
                            }

                            // Verify success
                            if (await unretweetBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
                            ...
                            logger.warn(`[followLikeRetweet] Retweet attempt ${attempt} failed: ${e.message}`);
                        }
                    } else {
                ...
                const likeButton = page.locator('button[data-testid="like"][role="button"]').first();
                const unlikeButton = page.locator('button[data-testid="unlike"][role="button"]').first();
                ...
                for (let attempt = 1; attempt <= 3; attempt++) {
                    ...
                    if (await likeButton.count() > 0 && await likeButton.isVisible()) {
                        ...
                        try {
                            logger.info(`[followLikeRetweet] Clicking Like button (Attempt ${attempt})...`);
                            ...
                            logger.warn(`[followLikeRetweet] Like attempt ${attempt} failed: ${e.message}`);
                        }
                    } else {
                ...
                if (!await agent.checkLoginState()) {
                    logger.warn('[followLikeRetweet] ⚠️ Potential logout detected after task completion.');
                }
        ...
        if (error.message.includes('Target page, context or browser has been closed')) {
            logger.warn(`[followLikeRetweet] Task interrupted: Browser/Page closed (likely Ctrl+C).`);
        } else {
