...
try {
  ...
  if (urls.length === 0) {
    console.error('[exampleTask.js] Warning: sites file was read, but it is empty or contains no valid URLs.');
  }
} catch (error) {
  console.error(`[exampleTask.js] CRITICAL: Failed to read site list from ${SITES_FILE}. The task will not be able to run. Error: ${error.message}`);
}
...
export default async function exampleTask(page, payload) {
  ...
  try {
    ...
    await Promise.race([
        ...
        for (let i = 0; i < loopCount; i++) {
          ...
          try {
            ...
            if (navError.message.includes('interrupted by another navigation') ||
              ...
              logger.warn(`Navigation to ${randomUrl} was interrupted (or page closed).`);
              break;
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error(`exampleTask exceeded ${CONFIG.taskTimeoutMs}ms limit`)), CONFIG.taskTimeoutMs))
    ]);
  } catch (error) {
