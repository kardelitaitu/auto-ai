  ...
  try {
    ...
    await Promise.race([
      (async () => {
        ...
        if (
          (await api.visible(handleSelector)) ||
          (await api.visible(fallbackSelector))
        ) {
    ...
    if (
      ...
      logger.warn(
        `[twitterFollow] Task interrupted: Browser/Page closed (likely Ctrl+C).`,
      );
    } else {
    ...
    try {
      ...
      logger.warn(
        `[twitterFollow] Failed to close page: ${closeError.message}`,
      );
    }
  }
}
...
async function _applyHumanizationPatch(page, logger) {
  const { applyHumanizationPatch: sharedPatch } =
    await import("../utils/browserPatch.js");
