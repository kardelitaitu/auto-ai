...
function selectRetweetBranch(payload, logger) {
    ...
    const forcedBranch = Object.entries(DEBUG_FORCE_BRANCH).find(([, forced]) => forced);
    if (forcedBranch) {
    ...
    if (payload?.debugBranch) {
        ...
        logger.warn(`[DEBUG] Invalid debugBranch: ${branchName}, using random selection`);
    }
...
async function retweetBranch1_directRetweet(page, logger) {
    ...
    await tweetElement.waitFor({ state: 'visible', timeout: TWEET_VISIBLE_TIMEOUT_MS });
    ...
    while (Date.now() - browseStart < browseTimeMs) {
        ...
        const scrollDistance = mathUtils.randomInRange(BROWSE_SCROLL_MIN_PX, BROWSE_SCROLL_MAX_PX);
        await api.scroll(scrollDistance);

        // Random pause between scrolls (simulates reading/scanning)
        const pauseTime = mathUtils.randomInRange(BROWSE_PAUSE_MIN_MS, BROWSE_PAUSE_MAX_MS);
        await api.wait(pauseTime);

        // Occasionally scroll back up (configurable chance)
        if (Math.random() < BROWSE_BACK_CHANCE) {
            const backDistance = mathUtils.randomInRange(BROWSE_BACK_SCROLL_MIN_PX, BROWSE_BACK_SCROLL_MAX_PX);
            await api.scroll.back(backDistance);

            const backPause = mathUtils.randomInRange(BROWSE_BACK_PAUSE_MIN_MS, BROWSE_BACK_PAUSE_MAX_MS);
...
async function retweetBranch2_profileVisitRetweet(page, logger, targetUrl) {
    ...
    if (!profileMatch) {
        logger.warn('[Branch2] Invalid tweet URL format, falling back to direct retweet');
    ...
    if (await api.visible(profileLink).catch(() => false)) {
        ...
        await api.goto(`https://x.com/${profileHandle}`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    }
    ...
    logger.info(`[Branch2] Current URL: ${profileUrl} (profile match: ${onProfile})`);

    if (!onProfile) {
        logger.warn('[Branch2] Profile page may not have loaded correctly, continuing anyway...');
    }

    // Read profile (20-90 seconds)
    const profileReadTime = mathUtils.randomInRange(20000, 90000);
    logger.info(`[Branch2] Reading profile for ${(profileReadTime / 1000).toFixed(1)}s...`);
    ...
    logger.info(`[Branch2] Reading tweet page for ${(tweetReadTime / 1000).toFixed(1)}s...`);
    ...
    if (result.success) {
        ...
        const homeReadMs = mathUtils.randomInRange(120000, 300000); // 2-5 minutes
        logger.info(`[Branch2] Reading home feed for ${(homeReadMs / 60000).toFixed(1)} minutes...`);
        await api.twitter.home({ readDurationMs: homeReadMs });
    }
    return result;
}
...
async function retweetBranch3_homeReadRetweet(page, logger, targetUrl) {
    ...
    logger.info(`[Phase1] Browsing home feed for ${(homeReadMs / 60000).toFixed(1)} minutes...`);
    await api.twitter.home({ readDurationMs: homeReadMs });
...
async function retweetBranch4_homeProfileRetweet(page, logger, targetUrl) {
    ...
    const profileMatch = targetUrl.match(/x\.com\/([^/]+)\/status\/\d+/);
    if (!profileMatch) {
        logger.warn('[Phase3] Invalid tweet URL format, falling back to direct retweet');
        const retweetBtn = tweetElement.locator('[data-testid="retweet"]');
        await retweetBtn.waitFor({ state: 'visible', timeout: TWEET_VISIBLE_TIMEOUT_MS });
        await api.scroll.focus2(retweetBtn);
        await api.wait(mathUtils.randomInRange(500, 1000));
    ...
    logger.info(`[Phase4] Reading profile for ${(profileReadTime / 1000).toFixed(1)}s...`);
    ...
    logger.info(`[Phase6] Reading tweet page for ${(tweetReadTime / 1000).toFixed(1)}s...`);
...
async function retweetBranch5_threadReader(page, logger, targetUrl) {
    ...
    logger.info(`[Phase3] Reading tweet page for ${(tweetReadTime / 1000).toFixed(0)}s...`);
    ...
    const showThreadBtn = tweetElement.locator('[data-testid="tweet-text-show-show-thread"]').first();
    ...
    if (clickedThread) {
        ...
        logger.info(`[Phase4] Reading thread for ${(threadReadTime / 1000).toFixed(0)}s...`);
...
async function retweetBranch7_notificationCheck(page, logger, targetUrl) {
    ...
    await api.goto('https://x.com/notifications', { waitUntil: 'domcontentloaded', timeout: 60000 });
    await api.wait(HYDRATION_DELAY_MS);

    const notifReadTime = mathUtils.randomInRange(60000, 120000);
    logger.info(`[Phase2] Browsing notifications for ${(notifReadTime / 1000).toFixed(0)}s...`);
    ...
    const tweetReadTime = mathUtils.randomInRange(60000, 120000);
    logger.info(`[Phase4] Reading tweet for ${(tweetReadTime / 1000).toFixed(0)}s...`);
...
async function retweetBranch9_searchToRetweet(page, logger, targetUrl) {
    ...
    await api.goto('https://x.com/search', { waitUntil: 'domcontentloaded', timeout: 60000 });
    await api.wait(HYDRATION_DELAY_MS);
    ...
    const searchBox = page.locator('[data-testid="SearchBox_Search_Input"]').first();
    if (await api.visible(searchBox).catch(() => false)) {
        ...
        logger.info(`[Phase2] Browsing search results for ${(searchReadTime / 1000).toFixed(0)}s...`);
        const scrollStart = Date.now();
...
async function retweetBranch11_multiPhaseEngagement(page, logger, targetUrl) {
    ...
    const homeReadMs = mathUtils.randomInRange(60000, 120000);
    logger.info(`[Phase1] Browsing home feed for ${(homeReadMs / 1000).toFixed(0)}s...`);
    await api.twitter.home({ readDurationMs: homeReadMs });
    ...
    const tweetReadTime = mathUtils.randomInRange(60000, 120000);
    logger.info(`[Phase3] Reading tweet for ${(tweetReadTime / 1000).toFixed(0)}s...`);
    const tweetReadStart = Date.now();
    while (Date.now() - tweetReadStart < tweetReadTime) {
        await api.scroll(mathUtils.randomInRange(100, 300));
        await api.wait(mathUtils.randomInRange(1000, 3000));
    ...
    if (result.success) {
        ...
        if (profileMatch) {
            ...
            await api.goto(profileUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
            await api.wait(HYDRATION_DELAY_MS);

            // Browse profile (1-2 minutes)
            const profileReadTime = mathUtils.randomInRange(60000, 120000);
            logger.info(`[Phase5] Browsing profile for ${(profileReadTime / 1000).toFixed(0)}s...`);
                        ...
                        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
                            try {
                                if (attempt > 0) {
                                    ...
                                    logger.info(`Retry ${attempt}/${MAX_RETRIES} in ${delay}ms...`);
                                ...
                                logger.info(`[Referrer] Strategy: ${refCtx.strategy}, Referrer: ${refCtx.referrer || '(direct)'}`);
                                ...
                                switch (selectedBranch) {
                                    case 'retweetBranch1_directRetweet':
                                        result = await retweetBranch1_directRetweet(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch2_profileVisitRetweet':
                                        result = await retweetBranch2_profileVisitRetweet(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch3_homeReadRetweet':
                                        result = await retweetBranch3_homeReadRetweet(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch4_homeProfileRetweet':
                                        result = await retweetBranch4_homeProfileRetweet(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch5_threadReader':
                                        result = await retweetBranch5_threadReader(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch6_likeRetweet':
                                        result = await retweetBranch6_likeRetweet(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch7_notificationCheck':
                                        result = await retweetBranch7_notificationCheck(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch8_overscrollReturn':
                                        result = await retweetBranch8_overscrollReturn(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch9_searchToRetweet':
                                        result = await retweetBranch9_searchToRetweet(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch10_impulseRetweet':
                                        result = await retweetBranch10_impulseRetweet(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch11_multiPhaseEngagement':
                                        result = await retweetBranch11_multiPhaseEngagement(page, logger, targetUrl);
                                        break;
                                    case 'retweetBranch12_misclickRecovery':
                                        result = await retweetBranch12_misclickRecovery(page, logger, targetUrl);
                                        break;
                                    default:
                                ...
                                if (result.success) {
                                    logger.info(`✅ Retweet Test PASSED. Branch: ${selectedBranch}, Reason: ${result.reason}`);
                                } else {
                                    logger.error(`❌ Retweet Test FAILED. Branch: ${selectedBranch}, Reason: ${result.reason}`);
                                }

                                // Observation delay
                                logger.info(`Waiting ${OBSERVATION_DELAY_MS}ms before finishing...`);
                                await api.wait(OBSERVATION_DELAY_MS);
                                return;

                            } catch (innerError) {
                                logger.warn(`Attempt ${attempt + 1} failed: ${innerError.message}`);
