            ...
            await Promise.race([
                (async () => {
                    ...
                    if (!profileClicked) {
                        ...
                        const profileHref = await page.locator(profileLinkSelector).getAttribute('href').catch(() => null);
                        if (profileHref) {
                            logger.warn(`Profile link click failed, navigating to ${profileHref}`);
                            await api.goto(`https://x.com${profileHref}`, { waitUntil: 'domcontentloaded' });
                        } else {
                            throw new Error('Could not find profile link — cannot proceed to followers');
                        }
                    }
                    ...
                    if (!followersClicked) {
                        ...
                        if (usernameMatch) {
                            await api.goto(`https://x.com/${usernameMatch[1]}/followers`, { waitUntil: 'domcontentloaded' });
                        }
                    }
                    ...
                    const followOnlySelector = '[data-testid$="-follow"]:not([data-testid$="-unfollow"])';
                    const allButtons = page.locator(followOnlySelector);
                    const totalButtons = await allButtons.count();
                    let followBackBtns = 0, genericFollowBtns = 0, alreadyFollowing = 0;
                    ...
                    for (let j = 0; j < totalButtons; j++) {
                        ...
                        if (btnText.includes('following') || btnText.includes('pending') || btnAria.includes('following') || btnAria.includes('unfollow')) {
                            alreadyFollowing++;
                        } else if (btnText.trim() === 'follow back' || btnAria.includes('follow back')) {
                            followBackBtns++;
                        } else if (btnText.trim() === 'follow' || btnAria.includes('follow @')) {
                            genericFollowBtns++;
                        }
                    }
                    logger.info(`Button scan: ${followBackBtns} Follow back, ${genericFollowBtns} Follow, ${alreadyFollowing} Already following (${totalButtons} total)`);
                    ...
                    const requestedFollows = parseInt(payload?.maxFollows || payload?.value || payload?.url);
                    const maxFollows = (!isNaN(requestedFollows) && requestedFollows > 0) ? requestedFollows : 1;
                    ...
                    while (followBackCount < maxFollows && emptyScrollRounds < maxEmptyScrolls && Date.now() < clickLoopDeadline) {
                        ...
                        for (let i = 0; i < count; i++) {
                            ...
                            if (await api.visible(btn)) {
                                ...
                                if (text.includes('following') || text.includes('pending') || ariaLabel.includes('following') || ariaLabel.includes('unfollow')) {
                                    logger.info(`Skipping button ${i}: already following (text="${text.trim()}", aria="${ariaLabel.trim()}")`);
                                    continue;
                                }

                                // ONLY target "Follow back" buttons — skip generic "Follow" (sidebar recommendations)
                                const trimmedText = text.trim();
                                const isFollowBack = trimmedText === 'follow back' || ariaLabel.includes('follow back');

                                if (!isFollowBack) continue;

                                const usernameMatch = ariaLabel.match(/@(\w+)/);
                                const targetUser = usernameMatch ? usernameMatch[0] : 'unknown user';

                                logger.info(`(${followBackCount + 1}/${maxFollows}) Clicking Follow back for ${targetUser}`);

                                // Human-like scroll to button (Golden View focus)
                                try {
                                    await api.scroll.focus(btn);
                                } catch (scrollErr) {
                                    logger.warn(`scroll.focus failed for ${targetUser}, using fallback`);
                                    await btn.scrollIntoViewIfNeeded();
                                }
                                await api.wait(api.randomInRange(300, 600));
                                await api.click(btn, { precision: 'safe', hoverBeforeClick: true });
                                ...
                                if (testId) {
                                    ...
                                    const unfollowTestId = testId.replace(/-follow$/, '-unfollow');
                                    const unfollowBtn = page.locator(`[data-testid="${unfollowTestId}"]`);
                                ...
                                if (verified) {
                                    ...
                                    logger.warn(`🔴 Could not verify follow for ${targetUser} — button did not change to "Following"`);
                                }
                        ...
                        if (!clickedThisRound) {
                            ...
                            if (await lastBtn.count() > 0) {
                                logger.info(`Scrolling to last button to load more... (attempt ${emptyScrollRounds}/${maxEmptyScrolls})`);
                                await api.scroll(api.randomInRange(300, 500));
                            } else {
                                logger.info(`No buttons found, scrolling down... (attempt ${emptyScrollRounds}/${maxEmptyScrolls})`);
                                await api.scroll(api.randomInRange(300, 500));
                            }
                            await api.wait(api.randomInRange(2000, 3000));
                        } else {
                    ...
                    if (Date.now() >= clickLoopDeadline && followBackCount < maxFollows) {
                        logger.warn(`⏱️ Click loop 2-minute budget expired (followed ${followBackCount}/${maxFollows})`);
                    }

                    if (followBackCount > 0) {
                        logger.info(`✅ Successfully followed ${followBackCount}/${maxFollows} users`);
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Task timeout (${taskTimeoutMs}ms)`)), taskTimeoutMs))
            ]);
