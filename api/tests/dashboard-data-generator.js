...
function generateSessions() {
    ...
    for (let i = 0; i < numSessions; i++) {
        ...
        sessions.push({
            ...
            taskName: status === 'online' ? tasks[randomInt(0, tasks.length - 1)] : null,
...
function generateTwitterMetrics(previous) {
    const actions = previous?.twitter?.actions || { likes: 0, retweets: 0, replies: 0, quotes: 0, follows: 0, bookmarks: 0 };
    
    const shouldAdd = Math.random() > 0.3;
    if (shouldAdd) {
        const actionType = randomInt(0, 5);
        switch (actionType) {
            case 0: actions.likes += randomInt(1, 3); break;
            case 1: actions.retweets += randomInt(1, 2); break;
            case 2: actions.replies += randomInt(1, 2); break;
            case 3: actions.quotes += randomInt(0, 1); break;
            case 4: actions.follows += randomInt(1, 2); break;
            case 5: actions.bookmarks += randomInt(0, 1); break;
        }
    }
    
    actions.total = actions.likes + actions.retweets + actions.replies + actions.quotes + actions.follows + actions.bookmarks;
...
function generateApiMetrics(previous) {
    ...
    const failures = (previous?.api?.failures || 0) + (Math.random() > 0.8 ? 1 : 0);
...
async function startDataGenerator() {
    ...
    console.log('[DataGenerator] This will generate realistic data for dashboard panels');
    ...
    console.log(`[DataGenerator] Dashboard server running on port ${dashboard.port}`);
    ...
    const interval = setInterval(() => {
        ...
        if (tickCount % 5 === 0) {
            console.log(`[Tick ${tickCount}] Sessions: ${sessions.filter(s => s.status === 'online').length}/${sessions.length} | ` +
