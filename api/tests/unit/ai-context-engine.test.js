...
describe('ai-context-engine', () => {
    ...
    it('parses numbers and engagement levels', () => {
        ...
        expect(
            engine.calculateEngagementLevel({ likes: 0, retweets: 0, replies: 0, views: 200000 })
        ).toBe('viral');
        expect(
            engine.calculateEngagementLevel({ likes: 800, retweets: 200, replies: 5, views: 5000 })
        ).toBe('high');
        expect(
            engine.calculateEngagementLevel({ likes: 50, retweets: 60, replies: 5, views: 500 })
        ).toBe('medium');
        expect(
            engine.calculateEngagementLevel({ likes: 7, retweets: 2, replies: 3, views: 200 })
        ).toBe('low');
        expect(
            engine.calculateEngagementLevel({ likes: 1, retweets: 1, replies: 1, views: 50 })
        ).toBe('minimal');
    });
    ...
    it('extracts replies and authors from multiple strategies', async () => {
        ...
        expect(replies.some((r) => r.text.includes('Reply') || r.text.includes('Deep reply'))).toBe(
            true
        );
    });
