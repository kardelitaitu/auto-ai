...
vi.mock('@api/index.js', () => {
    ...
    Object.assign(scrollMock, { focus: vi.fn() });
    return {
        api: {
            setPage: vi.fn(),
            withPage: vi.fn(),
...
vi.mock('@api/behaviors/micro-interactions.js', () => ({
    microInteractions: {
        createMicroInteractionHandler: vi.fn().mockReturnValue({
            ...
            randomRightClick: vi.fn().mockResolvedValue({ success: true }),
            logoClick: vi.fn().mockResolvedValue({ success: true }),
            whitespaceClick: vi.fn().mockResolvedValue({ success: true }),
            fidget: vi.fn().mockResolvedValue({ success: true, actions: [] }),
            shouldFidget: vi.fn().mockReturnValue(true),
            getFidgetInterval: vi.fn().mockReturnValue(15000),
            startFidgetLoop: vi.fn().mockReturnValue({ id: 123 }),
            stopFidgetLoop: vi.fn().mockReturnValue(true),
            executeMicroInteraction: vi.fn().mockResolvedValue({ success: true }),
        }),
    },
}));
...
describe('AITwitterAgent Coverage Tests', () => {
    ...
    beforeEach(async () => {
        mockPage = {
            ...
            evaluate: vi
                .fn()
                .mockResolvedValue({ readyState: 'complete', title: 'Home', hasBody: true }),
            on: vi.fn(),
            off: vi.fn(),
    ...
    describe('Page State Methods', () => {
        ...
        it('isOnTweetPage should return true when URL contains /status/', async () => {
            api.getCurrentUrl.mockResolvedValue('https://x.com/user/status/123456');

            expect(await agent.isOnTweetPage()).toBe(true);
        });

        it('isOnTweetPage should return true when pageState is TWEET_PAGE', async () => {
            api.getCurrentUrl.mockResolvedValue('https://x.com/home');
        ...
        it('getPageState should return current state object', async () => {
            api.getCurrentUrl.mockResolvedValue('https://x.com/home');
    ...
    describe('Micro Interaction Methods', () => {
        ...
        it('simulateFidget should handle errors', async () => {
            agent.microHandler.executeMicroInteraction = vi
                .fn()
                .mockRejectedValue(new Error('fidget error'));
    ...
    describe.skip('handleAIReply', () => {
        it('should handle handleAIReply with pre-validated action', async () => {
            ...
            await agent.handleAIReply('test content', 'user123', {
    ...
    describe.skip('handleAIQuote', () => {
        it('should handle handleAIQuote with pre-validated action', async () => {
            ...
            await agent.handleAIQuote('test content', 'user123', {
                url: 'https://x.com/test',
                action: 'quote',
            });
        });
    ...
    describe.skip('executeAIReply', () => {
        ...
        it('should handle reply execution failure', async () => {
            agent.replyEngine.executeReply = vi
                .fn()
                .mockResolvedValue({ success: false, reason: 'failed', method: 'native' });
    ...
    describe.skip('handleLike', () => {
        ...
        it('should handle like when element not found', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(
                expect.stringContaining('Could not find like button')
            );
        });
    ...
    describe.skip('handleBookmark', () => {
        ...
        it('should handle bookmark when element not found', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(
                expect.stringContaining('Could not find bookmark')
            );
        });
        ...
        it('should skip when already bookmarked', async () => {
            ...
            agent.humanInteraction.findWithFallback
                ...
                .mockResolvedValueOnce({
                    element: {},
                    selector: 'button[data-testid="removeBookmark"]',
                });

            await agent.handleBookmark();

            expect(mockLogger.info).toHaveBeenCalledWith(
                expect.stringContaining('ALREADY bookmarked')
            );
        });
    });
    ...
    describe.skip('handleFallback', () => {
        ...
        it('should handle none fallback', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(
                expect.stringContaining('No action taken')
            );
        });
    });
    ...
    describe.skip('runSession', () => {
        ...
        it('should handle refresh feed branch', async () => {
            ...
            agent.twitterConfig = {
                probabilities: { refresh: 0.1, profileDive: 0.1, tweetDive: 0.1 },
            };
        ...
        it('should handle diveProfile branch', async () => {
            ...
            agent.twitterConfig = {
                probabilities: { refresh: 0.1, profileDive: 0.2, tweetDive: 0.1 },
            };
        ...
        it('should handle diveTweet branch', async () => {
            ...
            agent.twitterConfig = {
                probabilities: { refresh: 0.1, profileDive: 0.2, tweetDive: 0.4 },
            };
    ...
    describe.skip('_diveTweetWithAI full flow', () => {
        it('should handle no target tweet found', async () => {
            ...
            mockPage.evaluate.mockResolvedValueOnce(0).mockResolvedValueOnce(0);
        ...
        it('should use queue wrapper for AI execution', async () => {
            ...
            const entropy = await import('@api/utils/entropyController.js');
            entropy.entropy.scrollSettleTime.mockReturnValue(100);

            agent.actionRunner.selectAction = vi.fn().mockReturnValue('like');
            agent.actionRunner.executeAction = vi
                .fn()
