...
describe('Error Handling Improvements', () => {
    describe('Session Success/Failure Tracking', () => {
        ...
        it('BUG: current handler logs "Session completed" even on failure', async () => {
            ...
            expect(logger.info).not.toHaveBeenCalledWith('Session completed successfully');
        ...
        it('FIXED: handler should log success on successful session', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith('Session completed successfully');
    ...
    describe('Error Propagation in makeApiExecutor', () => {
        ...
        it('FIXED: executor should handle custom error types', async () => {
            ...
            const apiFn = vi.fn().mockRejectedValue(new CustomError('Custom failed', 'CUSTOM_CODE'));
    ...
    describe('Configuration Validation', () => {
        ...
        const validateConfig = (config) => {
            ...
            if (!config.engagement?.probabilities) {
                ...
                if (typeof probs.reply !== 'number') errors.push('Invalid reply probability');
                if (typeof probs.quote !== 'number') errors.push('Invalid quote probability');
                if (typeof probs.like !== 'number') errors.push('Invalid like probability');
                if (typeof probs.bookmark !== 'number') errors.push('Invalid bookmark probability');
                if (typeof probs.retweet !== 'number') errors.push('Invalid retweet probability');
                if (typeof probs.follow !== 'number') errors.push('Invalid follow probability');
            }
