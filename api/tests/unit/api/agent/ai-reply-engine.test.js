...
describe('AIReplyEngine', () => {
  ...
  describe('shouldReply', () => {
    it('skip due to probability (mathUtils.roll returns false)', async () => {
      ...
      expect(mathUtils.roll).toHaveBeenCalledWith(engine.config.REPLY_PROBABILITY);
    ...
    it('skip due to safety filters (excluded keyword)', async () => {
      ...
      sentimentService.analyze.mockReturnValue({
        isNegative: false,
        score: 0.1,
        composite: { riskLevel: 'low', engagementStyle: 'neutral', conversationType: 'general' },
      });

      const result = await engine.shouldReply('Let us discuss politics today', 'testuser');
    ...
    it('skip due to validation failed', async () => {
      ...
      sentimentService.analyze.mockReturnValue({
        ...
        score: 0.1,
        composite: { riskLevel: 'low', engagementStyle: 'neutral', conversationType: 'general' },
        dimensions: { valence: { valence: 0.5 }, sarcasm: { sarcasm: 0 }, toxicity: { toxicity: 0 } },
      });
      ...
      engine.generateReply = vi.fn().mockResolvedValue({ success: true, reply: 'ab' });

      const result = await engine.shouldReply('Great tweet about technology', 'testuser');
  ...
  describe('applySafetyFilters', () => {
    ...
    it('excessive caps', () => {
      const result = engine.applySafetyFilters('THIS IS ALL CAPS TEXT THAT IS VERY LOUD');
    ...
    it('safe content', () => {
      const result = engine.applySafetyFilters('This is a normal tweet about technology');
  ...
  describe('generateQuickFallback', () => {
    ...
    it('adds question-based fallback for tweets with ?', () => {
      const result = engine.generateQuickFallback('What is your opinion?', 'user');
  ...
  describe('validateReplyAdvanced', () => {
    it('valid reply passes', () => {
      const result = engine.validateReplyAdvanced('This is a valid reply', 'Different original tweet');
      expect(result.valid).toBe(true);
    });

    it('detects address in reply', () => {
      const result = engine.validateReplyAdvanced('123 Main Street is the place', 'Original tweet');
  ...
  describe('extractReplyFromResponse', () => {
    it('extracts first valid line from content', () => {
      ...
      const result = engine.extractReplyFromResponse('First short\n\nSecond line here', 'original');
  ...
  describe('captureContext', () => {
    it('returns basic context object', async () => {
      const result = await engine.captureContext({}, 'https://twitter.com/user/status/123');
