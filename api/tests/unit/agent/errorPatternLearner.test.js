...
describe('api/agent/errorPatternLearner.js', () => {
    ...
    describe('recordError()', () => {
        ...
        it('should trim patterns when exceeding maxPatterns', () => {
            ...
            for (let i = 0; i < 5; i++) {
                errorPatternLearner.recordError(`Unique error type ${i}`, { url: `https://unique${i}.com`, action: `action${i}` });
            }
    ...
    describe('_classifyError()', () => {
        it('should classify selector errors', () => {
            expect(errorPatternLearner._classifyError('Selector not found')).toBe('selector_not_found');
            expect(errorPatternLearner._classifyError('Element not found')).toBe('selector_not_found');
        });

        it('should classify visibility errors', () => {
            expect(errorPatternLearner._classifyError('Element is not visible')).toBe('element_not_visible');
            expect(errorPatternLearner._classifyError('Element is hidden')).toBe('element_not_visible');
        });

        it('should classify timeout errors', () => {
            expect(errorPatternLearner._classifyError('Operation timeout')).toBe('timeout');
            expect(errorPatternLearner._classifyError('Navigation timed out')).toBe('timeout');
        });

        it('should classify verification errors', () => {
            expect(errorPatternLearner._classifyError('Verification failed')).toBe('verification_failed');
            expect(errorPatternLearner._classifyError('Failed to verify element')).toBe('verification_failed');
        });

        it('should return unknown for unclassified errors', () => {
            expect(errorPatternLearner._classifyError('Random error')).toBe('unknown');
        });
    });

    describe('_normalizeUrl()', () => {
        it('should normalize valid URLs', () => {
            const normalized = errorPatternLearner._normalizeUrl('https://example.com/path?query=1');
    ...
    describe('_stringSimilarity()', () => {
        it('should return 1 for identical strings', () => {
            expect(errorPatternLearner._stringSimilarity('hello world', 'hello world')).toBe(1);
        });
        ...
        it('should calculate similarity based on word overlap', () => {
            const similarity = errorPatternLearner._stringSimilarity('click submit button', 'click send button');
    ...
    describe('getWarning()', () => {
        it('should return null when no relevant patterns', () => {
            const result = errorPatternLearner.getWarning({ url: 'https://example.com' });
            expect(result).toBeNull();
        });

        it('should return warning when pattern has enough occurrences', () => {
            errorPatternLearner.recordError('Test error', { url: 'https://example.com' });
            errorPatternLearner.recordError('Test error', { url: 'https://example.com' });

            const warning = errorPatternLearner.getWarning({ url: 'https://example.com' });
    ...
    describe('getPreventionStrategies()', () => {
        it('should return empty array when no relevant patterns', () => {
            const strategies = errorPatternLearner.getPreventionStrategies({ url: 'https://example.com' });
            expect(strategies).toEqual([]);
        });

        it('should return strategies for known error types', () => {
            errorPatternLearner.recordError('Selector not found', { url: 'https://example.com' });
            errorPatternLearner.recordError('Selector not found', { url: 'https://example.com' });
        ...
        it('should generate different strategies for different error types', () => {
            errorPatternLearner.recordError('Element not visible', { url: 'https://example.com' });
            errorPatternLearner.recordError('Element not visible', { url: 'https://example.com' });
        ...
        it('should limit strategies to top 3', () => {
            for (let i = 0; i < 5; i++) {
                errorPatternLearner.recordError(`Error type ${i}`, { url: 'https://example.com', action: `action${i}` });
                errorPatternLearner.recordError(`Error type ${i}`, { url: 'https://example.com', action: `action${i}` });
            }
    ...
    describe('_calculateRelevance()', () => {
        it('should give high relevance for same URL', () => {
            ...
            const relevance = errorPatternLearner._calculateRelevance(pattern, context);
        ...
        it('should cap relevance at 1', () => {
            ...
            const context = { url: 'https://example.com', goal: 'test', action: 'click' };
    ...
    describe('export() and import()', () => {
        ...
        it('should restore pattern data on import', () => {
            errorPatternLearner.recordError('Test error', { url: 'https://example.com', goal: 'Test' });
            errorPatternLearner.recordError('Test error', { url: 'https://example.com', goal: 'Test' });
            const exported = errorPatternLearner.export();
    ...
    describe('_trimPatterns()', () => {
        it('should remove oldest 20% of patterns', () => {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                errorPatternLearner.recordError(`Error ${i}`, { url: `https://example${i}.com` });
            }

            // Add one more to trigger trim
            errorPatternLearner.recordError('Error 10', { url: 'https://example10.com' });
