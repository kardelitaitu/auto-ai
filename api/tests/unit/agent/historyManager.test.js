...
describe('api/agent/historyManager.js', () => {
    ...
    describe('_scoreRelevance()', () => {
        ...
        it('should add score for successful entries', () => {
            ...
            const successScore = historyManager._scoreRelevance(successEntry, context);
        ...
        it('should add score for failed entries with recovery', () => {
            ...
            const withRecoveryScore = historyManager._scoreRelevance(entryWithRecovery, context);
            const withoutRecoveryScore = historyManager._scoreRelevance(entryWithoutRecovery, context);
    ...
    describe('_normalizeUrl()', () => {
        it('should normalize valid URLs', () => {
            const normalized = historyManager._normalizeUrl('https://example.com/path?query=1');
    ...
    describe('_stringSimilarity()', () => {
        it('should return 1 for identical strings', () => {
            expect(historyManager._stringSimilarity('hello world', 'hello world')).toBe(1);
        });
        ...
        it('should calculate similarity based on word overlap', () => {
            const similarity = historyManager._stringSimilarity('click submit button', 'click send button');
    ...
    describe('_trim()', () => {
        it('should remove least relevant entries', () => {
            ...
            historyManager.add({ content: 'Old failed', success: false, timestamp: Date.now() - 600000 });
            historyManager.add({ content: 'Recent success', success: true, timestamp: Date.now() });
            historyManager.add({ content: 'Recent 2', success: true, timestamp: Date.now() - 1000 });
            historyManager.add({ content: 'Old 2', success: false, timestamp: Date.now() - 500000 });

            // Force trim by adding one more
            historyManager.add({ content: 'Newest', success: true, timestamp: Date.now() });
    ...
    describe('_createSummary()', () => {
        it('should create summary with correct statistics', () => {
            const entries = [
                { content: 'Test 1', success: true, goal: 'Login', url: 'https://a.com' },
                { content: 'Test 2', success: false, goal: 'Login', url: 'https://b.com' },
                { content: 'Test 3', success: true, goal: 'Search', url: 'https://a.com' },
            ];
