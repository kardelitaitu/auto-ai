...
describe('api/agent/semanticMapper.js', () => {
    ...
    describe('_calculateClickConfidence()', () => {
        ...
        it('should return medium confidence for tabs and menu items', () => {
            expect(semanticMapper._calculateClickConfidence({ role: 'tab', name: 'Tab 1' })).toBe(0.85);
            expect(semanticMapper._calculateClickConfidence({ role: 'menuitem', name: 'Item' })).toBe(0.85);
        });
    ...
    describe('_calculateTypeConfidence()', () => {
        it('should return high confidence for textboxes', () => {
            expect(semanticMapper._calculateTypeConfidence({ role: 'textbox' })).toBe(0.95);
        });

        it('should return high confidence for inputs', () => {
            expect(semanticMapper._calculateTypeConfidence({ role: 'input' })).toBe(0.95);
        });

        it('should return high confidence for searchbox', () => {
            expect(semanticMapper._calculateTypeConfidence({ role: 'searchbox' })).toBe(0.95);
        });

        it('should return 0.90 for combobox', () => {
            expect(semanticMapper._calculateTypeConfidence({ role: 'combobox' })).toBe(0.90);
        });

        it('should return low confidence for non-input elements', () => {
            expect(semanticMapper._calculateTypeConfidence({ role: 'button' })).toBe(0.20);
            expect(semanticMapper._calculateTypeConfidence({ role: 'generic' })).toBe(0.20);
        });
    });

    describe('_calculateScrollConfidence()', () => {
        it('should return high confidence for lists and feeds', () => {
            expect(semanticMapper._calculateScrollConfidence({ role: 'list' })).toBe(0.85);
            expect(semanticMapper._calculateScrollConfidence({ role: 'feed' })).toBe(0.85);
        });

        it('should return medium confidence for articles and documents', () => {
            expect(semanticMapper._calculateScrollConfidence({ role: 'article' })).toBe(0.75);
            expect(semanticMapper._calculateScrollConfidence({ role: 'document' })).toBe(0.75);
        });

        it('should return 0.70 for main and region', () => {
            expect(semanticMapper._calculateScrollConfidence({ role: 'main' })).toBe(0.70);
            expect(semanticMapper._calculateScrollConfidence({ role: 'region' })).toBe(0.70);
        });

        it('should return default 0.40 for other elements', () => {
            expect(semanticMapper._calculateScrollConfidence({ role: 'generic' })).toBe(0.40);
        });
    });
    ...
    describe('_categorizeElement()', () => {
        it('should categorize buttons', () => {
            expect(semanticMapper._categorizeElement({ role: 'button' })).toBe('button');
        });
        ...
        it('should categorize textboxes and inputs', () => {
            expect(semanticMapper._categorizeElement({ role: 'textbox' })).toBe('input');
            expect(semanticMapper._categorizeElement({ role: 'input' })).toBe('input');
        });

        it('should categorize headings', () => {
            expect(semanticMapper._categorizeElement({ role: 'heading' })).toBe('heading');
        });
        ...
        it('should categorize images', () => {
            ...
            expect(semanticMapper._categorizeElement({ role: 'image' })).toBe('image');
        });

        it('should categorize articles', () => {
            expect(semanticMapper._categorizeElement({ role: 'article' })).toBe('article');
        });

        it('should categorize navigation', () => {
            expect(semanticMapper._categorizeElement({ role: 'navigation' })).toBe('navigation');
        });

        it('should return generic for unknown roles', () => {
            expect(semanticMapper._categorizeElement({ role: 'unknown' })).toBe('generic');
        });
    });
