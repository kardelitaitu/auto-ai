...
describe('api/agent/goalDecomposer.js', () => {
    ...
    describe('Constructor', () => {
        ...
        it('should have steps for each pattern', () => {
            ...
            expect(goalDecomposer.goalPatterns.purchase.steps.length).toBeGreaterThan(0);
        });
    });

    describe('decompose()', () => {
        it('should decompose login goal', async () => {
            const result = await goalDecomposer.decompose('login to my account', 'https://example.com');
        ...
        it('should decompose purchase goal', async () => {
            const result = await goalDecomposer.decompose('buy a product', 'https://shop.example.com');
            expect(result.pattern).toBe('purchase');
            expect(result.totalSteps).toBeGreaterThan(0);
        });

        it('should decompose post goal', async () => {
            const result = await goalDecomposer.decompose('post a tweet', 'https://x.com');
            expect(result.pattern).toBe('post');
        });

        it('should decompose search goal', async () => {
            const result = await goalDecomposer.decompose('find information about AI', 'https://google.com');
            expect(result.pattern).toBe('search');
        });

        it('should decompose register goal', async () => {
            const result = await goalDecomposer.decompose('sign up for an account', 'https://example.com/register');
            expect(result.pattern).toBe('register');
        });

        it('should use heuristic for unknown goals', async () => {
            const result = await goalDecomposer.decompose('do something random', 'https://example.com');
            expect(result.pattern).toBe('heuristic');
            expect(result.steps.length).toBeGreaterThan(0);
        });

        it('should detect action words in heuristic', async () => {
            const result = await goalDecomposer.decompose('click on the button', 'https://example.com');
            expect(result.pattern).toBe('heuristic');
            expect(result.steps.some(s => s.subGoal.includes('Click'))).toBe(true);
        });

        it('should detect multiple action words', async () => {
            const result = await goalDecomposer.decompose('click and type something', 'https://example.com');
    ...
    describe('_heuristicDecompose()', () => {
        it('should create generic steps for no action words', () => {
            const result = goalDecomposer._heuristicDecompose('do something', 'https://example.com');
            expect(result.pattern).toBe('heuristic');
        ...
        it('should detect click action', () => {
            const result = goalDecomposer._heuristicDecompose('click the button', 'https://example.com');
            expect(result.steps.some(s => s.subGoal.includes('Click'))).toBe(true);
        });

        it('should detect navigate action', () => {
            const result = goalDecomposer._heuristicDecompose('navigate to home page', 'https://example.com');
            expect(result.steps.some(s => s.subGoal.includes('Navigate'))).toBe(true);
        });

        it('should detect scroll action', () => {
            const result = goalDecomposer._heuristicDecompose('scroll down to find more', 'https://example.com');
