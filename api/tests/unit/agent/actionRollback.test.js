...
describe('api/agent/actionRollback.js', () => {
    ...
    describe('isCriticalAction()', () => {
        it('should return true for critical actions', () => {
            ...
            expect(actionRollback.isCriticalAction({ action: 'navigate' })).toBe(true);
    ...
    describe('rollbackLast()', () => {
        ...
        it('should rollback last action successfully', async () => {
            ...
            expect(mockPage.goto).toHaveBeenCalledWith('https://example.com/old', { waitUntil: 'domcontentloaded' });
        });
        ...
        it('should restore form value for type actions', async () => {
            ...
            actionRollback.recordAction(
                { url: 'https://example.com', scrollPosition: { x: 0, y: 0 }, formValue: 'original text' },
    ...
    describe('rollbackMultiple()', () => {
        it('should rollback multiple actions', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                actionRollback.recordAction(
                    { url: 'https://example.com', scrollPosition: { x: i * 10, y: i * 10 } },
    ...
    describe('getHistory()', () => {
        it('should return copy of history', () => {
            actionRollback.recordAction({ url: 'https://example.com' }, { action: 'click' }, {});
    ...
    describe('clearHistory()', () => {
        it('should clear all history', () => {
            actionRollback.recordAction({ url: 'https://example.com' }, { action: 'click' }, {});
            actionRollback.recordAction({ url: 'https://example.com' }, { action: 'type' }, {});
    ...
    describe('Edge Cases', () => {
        it('should handle capturePreState failure gracefully', async () => {
            const mockPage = {
                url: vi.fn().mockImplementation(() => { throw new Error('Page closed'); }),
            };

            const state = await actionRollback.capturePreState(mockPage, { action: 'click' });
