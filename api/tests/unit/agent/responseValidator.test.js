...
describe('api/agent/responseValidator.js', () => {
    ...
    describe('_validateSingle()', () => {
        ...
        it('should check x coordinate is number', () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.errors.some(e => e.includes('\'x\' must be a number'))).toBe(true);
        });
        ...
        it('should check y coordinate is number', () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.errors.some(e => e.includes('\'y\' must be a number'))).toBe(true);
        });
    });
    ...
    describe('_validateSelector()', () => {
        ...
        it('should accept unknown selector formats', () => {
            const result = responseValidator._validateSelector('complex:selector:format');
            expect(result.valid).toBe(true);
    ...
    describe('_correctActionType()', () => {
        ...
        it('should return unknown action unchanged', () => {
            expect(responseValidator._correctActionType('unknownAction')).toBe('unknownAction');
        });
    ...
    describe('getSummary()', () => {
        it('should return valid summary for valid result', () => {
            const summary = responseValidator.getSummary({ valid: true, warnings: [] });
            expect(summary).toContain('Valid');
        });

        it('should include warning count', () => {
            const summary = responseValidator.getSummary({ valid: true, warnings: ['w1', 'w2'] });
    ...
    describe('Edge Cases', () => {
        it('should handle deeply nested array validation', () => {
            const actions = Array(5).fill(null).map((_, i) => ({
                action: 'click',
                selector: `#btn${i}`,
