...
import { AIReplyEngine, SAFETY_FILTERS } from '../../../agent/ai-reply-engine/index.js';
...
vi.mock('../../../twitter/twitter-reply-prompt.js', () => ({
...
describe('AIReplyEngine', () => {
    ...
    describe('applySafetyFilters', () => {
        ...
        it('should reject text with excluded keywords', () => {
            const result = engine.applySafetyFilters('This tweet contains politics content');
            expect(result.safe).toBe(false);
    ...
    describe('validateReplyAdvanced', () => {
        it('should validate a valid reply', () => {
            const result = engine.validateReplyAdvanced('This is a valid reply', 'Original tweet');
            expect(result.valid).toBe(true);
        });

        it('should reject reply containing address', () => {
            const result = engine.validateReplyAdvanced('I live at 123 Main Street', 'Original tweet');
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('contains_address');
        });

        it('should reject duplicate content', () => {
            const result = engine.validateReplyAdvanced('Same content', 'Same content');
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('duplicate_content');
        });

        it('should handle PO Box address', () => {
            const result = engine.validateReplyAdvanced('Send to PO Box 123', 'Original tweet');
            expect(result.valid).toBe(false);
            expect(result.reason).toBe('contains_address');
        });
    ...
    describe('generateQuickFallback', () => {
        ...
        it('should include question-related fallbacks for questions', () => {
            ...
            for (let i = 0; i < 50; i++) {
                const result = engine.generateQuickFallback('What do you think?', 'user');
    ...
    describe('extractReplyFromResponse', () => {
        it('should extract valid reply from response', () => {
            const result = engine.extractReplyFromResponse('This is a valid reply text', 'Original');
        ...
        it('should extract first valid line from multi-line response', () => {
            const result = engine.extractReplyFromResponse('1. First option\nThis is the actual reply', 'Original');
    ...
    describe('buildEnhancedPrompt', () => {
        it('should build prompt with basic info', () => {
            const result = engine.buildEnhancedPrompt('Tweet text', 'username', {}, 'casual');
        ...
        it('should include image indicator', () => {
            const result = engine.buildEnhancedPrompt('Tweet', 'user', { hasImage: true }, 'casual');
        ...
        it('should include replies', () => {
            ...
            const result = engine.buildEnhancedPrompt('Tweet', 'user', context, 'casual');
        ...
        it('should truncate long tweets', () => {
            ...
            const result = engine.buildEnhancedPrompt(longTweet, 'user', {}, 'casual');
        ...
        it('should limit replies to 20', () => {
            ...
            const result = engine.buildEnhancedPrompt('Tweet', 'user', { replies }, 'casual');
    ...
    describe('adaptiveRetry', () => {
        ...
        it('should throw after max retries', async () => {
            ...
            await expect(engine.adaptiveRetry(operation, { maxRetries: 2, baseDelay: 10 }))
    ...
    describe('shouldReply', () => {
        ...
        it('should skip safety filtered content (excluded keyword)', async () => {
            ...
            const result = await engine.shouldReply('politics discussion here', 'user');
            expect(result.decision).toBe('skip');
        ...
        it('should use fallback when AI fails', async () => {
            ...
            const result = await engine.shouldReply('Valid tweet text here is long enough', 'user');
