...
vi.mock('@api/agent/visualDiff.js', () => ({
    visualDiffEngine: {
        ...
        compareScreenshots: vi.fn().mockResolvedValue({ changed: false, diffRatio: 0, method: 'pixel' })
    },
}));
...
describe('api/agent/gameRunner.js', () => {
    ...
    describe('Memoization System', () => {
        ...
        describe('_getActionCacheKey', () => {
            ...
            it('should produce different keys for different inputs', () => {
                const key1 = gameAgentRunner._getActionCacheKey('url1', 'goal1', { id: 1 });
                const key2 = gameAgentRunner._getActionCacheKey('url2', 'goal2', { id: 2 });
        ...
        describe('_getOrComputeAction', () => {
            ...
            it('should return cached result on subsequent calls', async () => {
                ...
                const result1 = await gameAgentRunner._getOrComputeAction(url, goal, element, computeFn);
                const result2 = await gameAgentRunner._getOrComputeAction(url, goal, element, computeFn);
            ...
            it('should evict oldest entry when cache exceeds limit', async () => {
                ...
                const computeFn = vi.fn().mockImplementation((val) => Promise.resolve(`result-${val}`));

                await gameAgentRunner._getOrComputeAction('url1', 'goal', { id: 1 }, () => computeFn(1));
                await gameAgentRunner._getOrComputeAction('url2', 'goal', { id: 2 }, () => computeFn(2));
                await gameAgentRunner._getOrComputeAction('url3', 'goal', { id: 3 }, () => computeFn(3));
    ...
    describe('Comparison Methods', () => {
        describe('_compareUrl', () => {
            it('should return true when URLs differ', () => {
                expect(gameAgentRunner._compareUrl('https://example.com', 'https://other.com')).toBe(true);
            });

            it('should return false when URLs are the same', () => {
                expect(gameAgentRunner._compareUrl('https://example.com', 'https://example.com')).toBe(false);
            });

            it('should detect hash changes', () => {
                expect(gameAgentRunner._compareUrl('https://example.com#old', 'https://example.com#new')).toBe(true);
            });
        });
        ...
        describe('_compareVisual', () => {
            it('should return false when either screenshot is missing', async () => {
                expect(await gameAgentRunner._compareVisual(null, 'base64data')).toBe(false);
                expect(await gameAgentRunner._compareVisual('base64data', null)).toBe(false);
                expect(await gameAgentRunner._compareVisual(undefined, undefined)).toBe(false);
            });
            ...
            it('should return result from visualDiffEngine when successful', async () => {
                ...
                visualDiffEngine.compareScreenshots.mockResolvedValue({ changed: true, diffRatio: 0.1, method: 'pixel' });

                const result = await gameAgentRunner._compareVisual('aGVsbG8', 'd29ybGQ=');
                expect(result).toBe(true);
            });

            it('should use fallback comparison on error', async () => {
                const { visualDiffEngine } = await import('@api/agent/visualDiff.js');
                visualDiffEngine.compareScreenshots.mockRejectedValue(new Error('Test error'));

                const longString1 = 'a'.repeat(1000);
                const longString2 = 'b'.repeat(100);
                const result = await gameAgentRunner._compareVisual(longString1, longString2);
                expect(result).toBe(true);
            });
        });
    });
    ...
    describe('Coordinate Scaling', () => {
        describe('_scaleActionCoordinates', () => {
            ...
            it('should return null/undefined action unchanged', () => {
                ...
                expect(gameAgentRunner._scaleActionCoordinates(undefined)).toBe(undefined);
            });
            ...
            it('should scale drag coordinates', () => {
                ...
                const action = { action: 'drag', x: 100, y: 200, targetX: 300, targetY: 400 };
                const result = gameAgentRunner._scaleActionCoordinates(action);
    ...
    describe('Prompt Building', () => {
        describe('_buildPrompt', () => {
            ...
            it('should return array with system message', () => {
                const result = gameAgentRunner._buildPrompt('test goal', 'base64data', 'axtree', 'https://example.com');
                expect(Array.isArray(result)).toBe(true);
                expect(result[0].role).toBe('system');
            });

            it('should include goal in user message', () => {
                const result = gameAgentRunner._buildPrompt('click button', 'base64data', 'axtree', 'https://example.com');
            ...
            it('should include current URL', () => {
                const result = gameAgentRunner._buildPrompt('goal', 'base64data', 'axtree', 'https://example.com');
            ...
            it('should include AXTree when enabled', () => {
                gameAgentRunner.useAXTree = true;
                const result = gameAgentRunner._buildPrompt('goal', 'base64data', 'test-axtree-content', 'https://example.com');
                const content = JSON.stringify(result[result.length - 1].content);
            ...
            it('should include screenshot when vision is enabled', () => {
                ...
                const result = gameAgentRunner._buildPrompt('goal', 'base64screenshot', 'axtree', 'https://example.com');
                const userMessage = result[result.length - 1];
                const hasImage = userMessage.content.some(p => p.type === 'image_url');
            ...
            it('should include recent history (last 4 items)', () => {
                ...
                const result = gameAgentRunner._buildPrompt('goal', 'screenshot', 'axtree', 'url');
    ...
    describe('Run Method', () => {
        it('should throw when already running', async () => {
            gameAgentRunner.isRunning = true;
            await expect(gameAgentRunner.run('test goal')).rejects.toThrow('already running');
        });
    });
    ...
    describe('Action Verification', () => {
        describe('_verifyAction', () => {
            it('should return true when AXTree is disabled', async () => {
                gameAgentRunner.useAXTree = false;
                const preState = { axTree: '{}', url: 'https://a.com', screenshot: 'abc' };
                const action = { action: 'click' };
                const result = await gameAgentRunner._verifyAction(mockPage, preState, action);
                expect(result).toBe(true);
            });
            ...
            it('should verify click action with AXTree changes', async () => {
                ...
                const preState = { axTree: '{"role":"root","name":"before"}', url: 'https://before.com', screenshot: 'before' };
                const action = { action: 'click' };
                const result = await gameAgentRunner._verifyAction(mockPage, preState, action);
            ...
            it('should verify clickAt action', async () => {
                ...
                const preState = { axTree: '{"role":"root"}', url: 'https://same.com', screenshot: 'before' };
            ...
            it('should verify type action and return boolean', async () => {
                ...
                const preState = { axTree: '{"role":"textbox","name":"Input"}', url: 'https://same.com', screenshot: 'before' };
            ...
            it('should verify type action with URL change fallback', async () => {
                ...
                const preState = { axTree: '{"role":"textbox"}', url: 'https://before.com', screenshot: 'before' };
            ...
            it('should return true for unknown action types', async () => {
                ...
                const preState = { axTree: '{}', url: 'https://example.com', screenshot: 'before' };
            ...
            it('should return false on verification error', async () => {
                gameAgentRunner.useAXTree = true;
                mockPage.waitForTimeout = vi.fn().mockRejectedValue(new Error('timeout'));
    ...
    describe('Screenshot Capture - Full Coverage', () => {
        describe('_captureScreenshot', () => {
            it('should handle missing viewport with evaluate fallback', async () => {
                ...
                mockPage.evaluate = vi.fn().mockResolvedValue({ width: 800, height: 600 });
                mockPage.screenshot = vi.fn().mockResolvedValue(Buffer.from('screenshot'));
            ...
            it('should handle V-PREP failure with fallback', async () => {
                mockPage.viewportSize = vi.fn().mockReturnValue({ width: 1920, height: 1080 });
                mockPage.screenshot = vi.fn().mockResolvedValue(Buffer.from('fallback-screenshot'));
                
                // Clear mocks for VPreprocessor
                const { VisionPreprocessor } = await import('@api/utils/VisionPreprocessor.js');
        ...
        describe('_captureAXTree', () => {
            it('should return full tree when compact is false', async () => {
                ...
                mockPage.accessibility = { snapshot: vi.fn().mockResolvedValue(mockTree) };
            ...
            it('should return compact tree by default', async () => {
                const mockTree = { 
                    ...
                    name: 'page', 
                    children: [
                        { role: 'button', name: 'Click me', selector: '#btn' }
                    ] 
                };
                mockPage.accessibility = { snapshot: vi.fn().mockResolvedValue(mockTree) };
    ...
    describe('Stuck Screenshot', () => {
        describe('_saveStuckScreenshot', () => {
            ...
            it('should return null on save failure', async () => {
                mockPage.screenshot = vi.fn().mockRejectedValue(new Error('Save failed'));
    ...
    describe('Full State Capture', () => {
        describe('_captureState', () => {
            it('should capture full state with AXTree enabled', async () => {
                ...
                mockPage.screenshot = vi.fn().mockResolvedValue(Buffer.from('screenshot'));
                mockPage.accessibility = { snapshot: vi.fn().mockResolvedValue({ role: 'root' }) };
