...
describe('api/agent/memoryInjector.js', () => {
    ...
    describe('injectMemory()', () => {
        it('should return empty string when no patterns found', async () => {
            const result = await memoryInjector.injectMemory({ url: 'https://example.com' });
            expect(result).toBe('');
        });

        it('should return formatted memory string when patterns exist', async () => {
            sessionStore.getPatternsByUrl.mockReturnValue([
                { selector: '#btn', action: 'click', confidence: 0.8, timesUsed: 5, description: 'Submit button' },
            ]);
        ...
        it('should get patterns by goal type', async () => {
            ...
            const result = await memoryInjector.injectMemory({ url: 'https://example.com', goal: 'Login to site' });
            expect(result).toContain('Learned Patterns');
        });
        ...
        it('should get patterns by page type', async () => {
            ...
            const result = await memoryInjector.injectMemory({ url: 'https://example.com', pageType: 'form' });
            expect(result).toContain('Learned Patterns');
        });
    });
    ...
    describe('_extractGoalType()', () => {
        it('should identify login goals', () => {
            expect(memoryInjector._extractGoalType('Login to the website')).toBe('login');
            expect(memoryInjector._extractGoalType('Sign in with credentials')).toBe('login');
        });

        it('should identify search goals', () => {
            expect(memoryInjector._extractGoalType('Search for products')).toBe('search');
            expect(memoryInjector._extractGoalType('Find the best deals')).toBe('search');
        });

        it('should identify purchase goals', () => {
            expect(memoryInjector._extractGoalType('Buy the item')).toBe('purchase');
            expect(memoryInjector._extractGoalType('Purchase a subscription')).toBe('purchase');
        });

        it('should identify navigation goals', () => {
            expect(memoryInjector._extractGoalType('Navigate to home')).toBe('navigation');
            expect(memoryInjector._extractGoalType('Go to settings')).toBe('navigation');
        });

        it('should identify form goals', () => {
            expect(memoryInjector._extractGoalType('Fill out the form')).toBe('form');
            expect(memoryInjector._extractGoalType('Complete registration form')).toBe('form');
        });

        it('should identify interaction goals', () => {
            expect(memoryInjector._extractGoalType('Click the button')).toBe('interaction');
            expect(memoryInjector._extractGoalType('Press submit')).toBe('interaction');
        });

        it('should return general for unrecognized goals', () => {
            expect(memoryInjector._extractGoalType('Do something random')).toBe('general');
        });
    });
    ...
    describe('getSuccessPatterns()', () => {
        it('should return success patterns from session store', () => {
            ...
            const patterns = memoryInjector.getSuccessPatterns('click', 'https://example.com');
