...
import { createSilentLogger } from '../../utils/test-helpers.js';
...
describe('ConfidenceScorer - Behavior Tests', () => {
    ...
    describe('Structure Scoring', () => {
        ...
        it('should score higher for responses with required fields', () => {
            ...
            const complete = confidenceScorer.score({ action: 'click', selector: '#btn' });
        ...
        it('should penalize unknown fields', () => {
            ...
            const valid = confidenceScorer.score({ action: 'click', selector: '#btn' });
        ...
        it('should recognize all valid action types', () => {
            const actions = ['click', 'clickAt', 'type', 'scroll', 'wait', 'navigate', 'done', 'verify'];

            actions.forEach(action => {
                const response = { action };
                if (action === 'click') response.selector = '#test';
                if (action === 'clickAt') { response.x = 100; response.y = 200; }
                if (action === 'type') { response.selector = '#input'; response.value = 'test'; }
                if (action === 'scroll' || action === 'wait' || action === 'navigate') response.value = 'test';
    ...
    describe('Selector Scoring', () => {
        ...
        it('should score role selectors well', () => {
            const response = { action: 'click', selector: 'role=button,name="Submit"' };
            const score = confidenceScorer.score(response);
        ...
        it('should penalize placeholder selectors', () => {
            ...
            const placeholders = ['...', 'placeholder', 'N/A', 'TODO'];
            const validSelector = confidenceScorer.score({ action: 'click', selector: '#submit-btn' });
        ...
        it('should reward longer, more specific selectors', () => {
            ...
            const long = confidenceScorer.score({ action: 'click', selector: '#very-specific-element-id' });
    ...
    describe('Rationale Scoring', () => {
        it('should score higher with rationale present', () => {
            ...
            const without = confidenceScorer.score({ action: 'click', selector: '#btn' });
        ...
        it('should score longer rationales higher', () => {
            ...
            const long = confidenceScorer.score({
                action: 'click',
                selector: '#btn',
                rationale: 'Click the submit button to proceed with the form submission and continue to the next page'
            });
    ...
    describe('Context Relevance', () => {
        ...
        it('should recognize form page type actions', () => {
            ...
            const clickScore = confidenceScorer.score({ action: 'click', selector: '#btn' }, context);
            const typeScore = confidenceScorer.score({ action: 'type', selector: '#input', value: 'test' }, context);
            const scrollScore = confidenceScorer.score({ action: 'scroll', value: 'down' }, context);
        ...
        it('should recognize game page type actions', () => {
            ...
            const clickAtScore = confidenceScorer.score({ action: 'clickAt', x: 100, y: 200 }, context);
            const waitScore = confidenceScorer.score({ action: 'wait', value: '1000' }, context);
