...
describe('api/agent/errorRecoveryPrompt.js', () => {
    ...
    describe('Constructor', () => {
        it('should define error patterns', () => {
            ...
            expect(errorRecoveryPrompt.errorPatterns.selector_not_found).toBeDefined();
            expect(errorRecoveryPrompt.errorPatterns.element_not_visible).toBeDefined();
            expect(errorRecoveryPrompt.errorPatterns.timeout).toBeDefined();
        });

        it('should have recovery strategies for each pattern', () => {
            for (const [_type, pattern] of Object.entries(errorRecoveryPrompt.errorPatterns)) {
    ...
    describe('_classifyError()', () => {
        it('should classify selector_not_found errors', () => {
            expect(errorRecoveryPrompt._classifyError('Selector not found')).toBe('selector_not_found');
            expect(errorRecoveryPrompt._classifyError('Cannot find element')).toBe('selector_not_found');
            expect(errorRecoveryPrompt._classifyError('no element with selector')).toBe('selector_not_found');
        });

        it('should classify element_not_visible errors', () => {
            expect(errorRecoveryPrompt._classifyError('Element is not visible')).toBe('element_not_visible');
            expect(errorRecoveryPrompt._classifyError('Element is hidden')).toBe('element_not_visible');
            expect(errorRecoveryPrompt._classifyError('Element obscured')).toBe('element_not_visible');
        });

        it('should classify timeout errors', () => {
            expect(errorRecoveryPrompt._classifyError('Operation timeout')).toBe('timeout');
            expect(errorRecoveryPrompt._classifyError('Navigation timed out')).toBe('timeout');
            expect(errorRecoveryPrompt._classifyError('wait timed out')).toBe('timeout');
        });

        it('should classify action_failed errors', () => {
            expect(errorRecoveryPrompt._classifyError('Action failed')).toBe('action_failed');
            expect(errorRecoveryPrompt._classifyError('Click failed')).toBe('action_failed');
        });

        it('should classify verification_failed errors', () => {
            expect(errorRecoveryPrompt._classifyError('Verification failed')).toBe('verification_failed');
            expect(errorRecoveryPrompt._classifyError('Check failed')).toBe('verification_failed');
        });

        it('should classify navigation_failed errors', () => {
            expect(errorRecoveryPrompt._classifyError('Navigation failed')).toBe('navigation_failed');
            expect(errorRecoveryPrompt._classifyError('Goto failed')).toBe('navigation_failed');
        });
        ...
        it('should return unknown for unclassified errors', () => {
            expect(errorRecoveryPrompt._classifyError('Random error')).toBe('unknown');
        });
    });
    ...
    describe('getRecoveryActions()', () => {
        ...
        it('should return scroll action for element_not_visible', () => {
            const actions = errorRecoveryPrompt.getRecoveryActions('Element not visible', {});
        ...
        it('should return wait action for timeout', () => {
            const actions = errorRecoveryPrompt.getRecoveryActions('Timeout error', {});
        ...
        it('should return generic wait action for unknown errors', () => {
            const actions = errorRecoveryPrompt.getRecoveryActions('Unknown error', {});
    ...
    describe('isRecoverable()', () => {
        it('should return true for recoverable errors', () => {
            expect(errorRecoveryPrompt.isRecoverable('Selector not found')).toBe(true);
            expect(errorRecoveryPrompt.isRecoverable('Timeout')).toBe(true);
            expect(errorRecoveryPrompt.isRecoverable('Element not visible')).toBe(true);
        });
