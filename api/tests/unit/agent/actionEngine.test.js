...
describe('actionEngine.js', () => {
    ...
    describe('execute()', () => {
        ...
        it('should execute click action', async () => {
            const result = await engine.execute(mockPage, { action: 'click', selector: '#btn' });
            expect(result.success).toBe(true);
        });

        it('should execute clickAt action', async () => {
            const result = await engine.execute(mockPage, { action: 'clickAt', x: 100, y: 200 });
            expect(result.success).toBe(true);
        });

        it('should execute clickAt with array coordinates', async () => {
            const result = await engine.execute(mockPage, { action: 'clickAt', x: [100, 150], y: [200, 250] });
            expect(result.success).toBe(true);
        });

        it('should execute type action', async () => {
            const result = await engine.execute(mockPage, { action: 'type', selector: '#input', value: 'hello' });
            expect(result.success).toBe(true);
        });

        it('should execute press action', async () => {
            const result = await engine.execute(mockPage, { action: 'press', key: 'Enter' });
            expect(result.success).toBe(true);
        });

        it('should execute press with value', async () => {
            const result = await engine.execute(mockPage, { action: 'press', value: 'Escape' });
            expect(result.success).toBe(true);
        });

        it('should execute scroll action', async () => {
            const result = await engine.execute(mockPage, { action: 'scroll', value: 'down' });
            expect(result.success).toBe(true);
        });

        it('should execute navigate action', async () => {
            const result = await engine.execute(mockPage, { action: 'navigate', value: 'https://example.com' });
            expect(result.success).toBe(true);
        });

        it('should execute goto action', async () => {
            const result = await engine.execute(mockPage, { action: 'goto', value: 'https://example.com' });
            expect(result.success).toBe(true);
        });

        it('should execute wait action', async () => {
            const result = await engine.execute(mockPage, { action: 'wait', value: '1000' });
            expect(result.success).toBe(true);
        });

        it('should execute delay action', async () => {
            const result = await engine.execute(mockPage, { action: 'delay', value: '500' });
            expect(result.success).toBe(true);
        });
        ...
        it('should handle verify action', async () => {
            const result = await engine.execute(mockPage, { action: 'verify', description: 'Check element' });
            expect(result.success).toBe(true);
        });
        ...
        it('should handle action execution error', async () => {
            ...
            const result = await engine.execute(mockPage, { action: 'click', selector: '#missing' });
            expect(result.success).toBe(false);
    ...
    describe('getLocator()', () => {
        ...
        it('should throw for non-string selector', () => {
            expect(() => engine.getLocator(mockPage, null)).toThrow('Invalid selector');
        });

        it('should throw for placeholder selectors', () => {
            expect(() => engine.getLocator(mockPage, '...')).toThrow('placeholder');
            expect(() => engine.getLocator(mockPage, 'placeholder')).toThrow('placeholder');
            expect(() => engine.getLocator(mockPage, 'N/A')).toThrow('placeholder');
        });

        it('should handle role= selector', () => {
            const locator = engine.getLocator(mockPage, 'role=button,name="Click me"');
    ...
    describe('performPress()', () => {
        ...
        it('should throw for missing key', async () => {
            await expect(engine.performPress(mockPage, null)).rejects.toThrow('Key is required');
        });
    });
    ...
    describe('performNavigate()', () => {
        ...
        it('should throw for missing URL', async () => {
            await expect(engine.performNavigate(mockPage, null)).rejects.toThrow('URL is required');
        });
    });
    ...
    describe('performWait()', () => {
        ...
        it('should throw for invalid wait time', async () => {
            await expect(engine.performWait(mockPage, 'invalid')).rejects.toThrow('Invalid wait time');
        });
    });
    ...
    describe('performClickAt()', () => {
        ...
        it('should throw for missing coordinates', async () => {
            await expect(engine.performClickAt(mockPage, undefined, 200)).rejects.toThrow('requires x and y');
        });

        it('should throw for non-numeric coordinates', async () => {
            await expect(engine.performClickAt(mockPage, 'x', 'y')).rejects.toThrow('requires numeric');
        });
    });
    ...
    describe('performMultiSelect()', () => {
        it('should perform multi-select', async () => {
            const items = [{ x: 100, y: 100 }, { x: 200, y: 200 }];
        ...
        it('should throw for non-array items', async () => {
            await expect(engine.performMultiSelect(mockPage, null)).rejects.toThrow('requires an array');
        });
    });
});
