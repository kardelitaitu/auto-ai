...
import { validatePayload, validateTaskExecution } from '@api/utils/validator.js';
...
describe('Orchestrator Coverage Extensions', () => {
  ...
  describe('executeTask (Coverage)', () => {
    it('should execute successfully when task module is valid', async () => {
      ...
      const page = { isClosed: vi.fn().mockReturnValue(false), context: vi.fn(() => ({ browser: () => ({ isConnected: () => true }) })) };
    ...
    it('should fail if task does not have default export', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining("[Orchestrator] Task 'coverage_no_default_task' error:"),
    ...
    it('should fail if validation fails', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining("[Orchestrator] Task 'coverage_valid_task' error:"),
  ...
  describe('processTasks (Coverage)', () => {
    ...
    it('should warn and return if no active sessions', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith("[Orchestrator] No active sessions available.");
    });
  });
  ...
  describe('waitForTasksToComplete', () => {
    it('should resolve immediately if queue is empty and not processing', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("Queue empty, resolving immediately"));
    });
  ...
  describe('shutdown', () => {
    it('should handle force shutdown', async () => {
      const waitForTasksSpy = vi.spyOn(orchestrator, 'waitForTasksToComplete').mockResolvedValue({ timedOut: false });
