...
vi.mock('../../utils/retry.js', () => ({ calculateBackoffDelay: vi.fn() }));
...
vi.mock('../../utils/human-interaction.js', () => ({
  HumanInteraction: class {
    ...
    postTweet() {
      return postTweetImpl ? postTweetImpl() : Promise.resolve({ success: true, reason: 'posted' });
    }
    typeText() {
    ...
    findElement(page, selectors) {
      return findElementImpl ? findElementImpl(page, selectors) : Promise.resolve({
        selector: selectors[0],
        element: {
          boundingBox: () => Promise.resolve({ y: 100, x: 10, width: 10, height: 10 }),
...
describe('ai-reply-engine', () => {
  ...
  beforeEach(async () => {
    ...
    ({ calculateBackoffDelay } = await import('../../utils/retry.js'));
    engine = new AIReplyEngine({ processRequest: vi.fn(), sessionId: 'test' }, { replyProbability: 0.5, maxRetries: 1 });
  });
  ...
  const createPageMock = (options = {}) => {
    ...
    const page = {
      ...
      evaluate: vi.fn((fn) => {
        ...
        global.window = page._window || { location: { href: 'https://x.com' }, innerHeight: 800, scrollTo: vi.fn() };
        let result;
        try {
          result = fn();
        } finally {
  ...
  it('returns reply when generation and validation succeed', async () => {
    mathUtils.roll.mockReturnValue(true);
    sentimentService.analyze.mockReturnValue(baseSentiment);
    vi.spyOn(engine, 'generateReply').mockResolvedValue({ success: true, reply: 'Nice reply  ' });
  ...
  it('builds enhanced prompt with guidance and replies', () => {
    const promptData = engine.buildEnhancedPrompt(
      'tweet text',
      'author',
      { replies: [{ author: 'a', text: 'A longer reply that should be included' }, { author: 'b', text: 'This is a longer short reply' }] },
      'humorous'
    );
    ...
    const article = {
      $: vi.fn((selector) => {
        if (selector.includes('tweetText') || selector.includes('[dir="auto"]')) {
          return Promise.resolve({ innerText: vi.fn().mockResolvedValue('@user123 Hello there') });
        }
