...
describe('BaseHandler', () => {
    ...
    describe('normalizeProbabilities', () => {
        ...
        it('should clamp probabilities to 0-1 range', () => {
            const handler = new BaseHandler(mockAgent);
            const result = handler.normalizeProbabilities({ tweetDive: 2, profileDive: -1 });
        ...
        it('should apply fatigue bias when active', () => {
            ...
            expect(result.tweetDive).toBeLessThan(mockAgent.config.probabilities.tweetDive);
        });
    ...
    describe('performHealthCheck', () => {
        ...
        it('should return unhealthy when network is inactive', async () => {
            ...
            const handler = new BaseHandler(mockAgent);
            handler.lastNetworkActivity = Date.now() - 60000; // 1 minute ago
        ...
        it('should detect critical error pages', async () => {
            const { api } = await import('@api/index.js');
            api.eval.mockResolvedValue('Something went wrong ERR_TOO_MANY_REDIRECTS');
