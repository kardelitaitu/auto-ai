...
describe('ai-quote-engine', () => {
    ...
    it('generates a quote successfully', async () => {
        ...
        const agent = {
            processRequest: vi
                .fn()
                .mockResolvedValue({ success: true, data: { content: 'Great take here.' } }),
            sessionId: 'test',
        };
    ...
    it('handles empty LLM content', async () => {
        ...
        const agent = {
            processRequest: vi.fn().mockResolvedValue({ success: true, data: { content: '' } }),
            sessionId: 'test',
        };
        engine = new AIQuoteEngine(agent, { quoteProbability: 1, maxRetries: 1 });
        const result = await engine.generateQuote('tweet text', 'user', {});
    ...
    it('returns failure when LLM request fails', async () => {
        ...
        const agent = {
            processRequest: vi.fn().mockResolvedValue({ success: false, error: 'bad_request' }),
            sessionId: 'test',
        };
        engine = new AIQuoteEngine(agent, { quoteProbability: 1, maxRetries: 1 });
        const result = await engine.generateQuote('tweet text', 'user', {});
    ...
    it('returns failure when cleaned quote is too short', async () => {
        ...
        const agent = {
            processRequest: vi.fn().mockResolvedValue({ success: true, content: 'short' }),
            sessionId: 'test',
        };
        engine = new AIQuoteEngine(agent, { quoteProbability: 1, maxRetries: 1 });
    ...
    it('returns failure when validation fails', async () => {
        ...
        const agent = {
            processRequest: vi
                .fn()
                .mockResolvedValue({ success: true, content: 'politics are bad today' }),
            sessionId: 'test',
        };
        engine = new AIQuoteEngine(agent, { quoteProbability: 1, maxRetries: 1 });
    ...
    it('continues to verifyComposerOpen when composer wait times out - triggers line 1456', async () => {
        ...
        const verifySpy = vi
            .fn()
            .mockReturnValue({ open: true, selector: '[data-testid="tweetTextarea_0"]' });
        const human = {
            logStep: vi.fn(),
    ...
    describe('quoteMethodB_Retweet error handling for coverage', () => {
        ...
        describe('quoteMethodA_Keyboard fallback and timeout paths', () => {
            it('falls back to quoteMethodB_Retweet when hasQuotePreview is false (lines 1105-1108)', async () => {
                ...
                const resultPromise = engine.quoteMethodA_Keyboard(page, 'Test quote', human);
        ...
        it('returns failure when retweet menu does not open (line 1231)', async () => {
            ...
            const resultPromise = engine.quoteMethodB_Retweet(page, 'Test quote', human);
