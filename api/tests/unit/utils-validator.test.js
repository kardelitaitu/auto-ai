...
describe('api/utils/validator.js', () => {
    ...
    describe('validatePayload', () => {
        it('should validate valid payload', () => {
            const result = validator.validatePayload({ name: 'test' }, { name: { type: 'string', required: true } });
            expect(result.isValid).toBe(true);
        });

        it('should reject missing required field', () => {
            const result = validator.validatePayload({}, { name: { type: 'string', required: true } });
            expect(result.isValid).toBe(false);
        });

        it('should reject invalid type', () => {
            const result = validator.validatePayload({ count: 'abc' }, { count: { type: 'number' } });
            expect(result.isValid).toBe(false);
        });
    });
    ...
    describe('validateBrowserConnection', () => {
        it('should validate valid ws endpoint', () => {
            const result = validator.validateBrowserConnection('ws://localhost:9222/devtools/browser/abc123');
            expect(result.isValid).toBe(true);
        });
    ...
    describe('validateApiResponse', () => {
        it('should validate successful response for known apiType', () => {
            const result = validator.validateApiResponse({ code: 0, data: [] }, 'roxybrowser');
            expect(result.isValid).toBe(true);
        });

        it('should return valid for unknown apiType', () => {
            const result = validator.validateApiResponse({ success: false, error: 'fail' }, 'unknown');
            expect(result.isValid).toBe(true);
        });
    });
});
