...
vi.mock('@api/core/sessionManager.js', () => ({
  default: class {
    ...
    get activeSessionsCount() { return 0; }
  }
}));
...
describe('Orchestrator', () => {
  ...
  describe('formatSessionDisplayName', () => {
    it('should format session display names correctly', () => {
      expect(orchestrator._formatSessionDisplayName({ type: 'localChrome', port: 9222 })).toBe('chrome:9222');
      expect(orchestrator._formatSessionDisplayName({ type: 'ixbrowser', windowName: 'Profile1' })).toBe('ix:Profile1');
      expect(orchestrator._formatSessionDisplayName({ type: 'unknown', ws: 'ws://localhost:1234' })).toBe('unknown');
    });
  });
  ...
  describe('Task Processing', () => {
    it('should distribute tasks to sessions', async () => {
      ...
      expect(checklistSpy).toHaveBeenCalledWith(mockSession, expect.arrayContaining([expect.objectContaining({ taskName: 'mockTask' })]));
  ...
  describe('startDiscovery', () => {
    it('should load connectors and discover browsers', async () => {
      mockDiscovery.discoverBrowsers.mockResolvedValue([
        { ws: 'ws://localhost:1234', windowName: 'Chrome 1', type: 'localChrome' }
      ]);
  ...
  describe('addTask', () => {
    it('should add valid task to queue and trigger processing', async () => {
      ...
      expect(orchestrator.taskQueue[0]).toMatchObject({ taskName: 'testTask', payload: { foo: 'bar' } });
  ...
  describe('Shutdown', () => {
    it('should shutdown gracefully', async () => {
      vi.spyOn(orchestrator, 'waitForTasksToComplete').mockResolvedValue({ timedOut: false });
