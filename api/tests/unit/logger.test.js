...
describe('logger', () => {
  ...
  it('should leave pre-colored tokens unchanged', async () => {
    ...
    logger.info('\u001b[31muser@host\u001b[0m "\u001b[32mquote\u001b[0m" (\u001b[33mparen\u001b[0m) \u001b[34mhttp://x.com\u001b[0m');
    await vi.runAllTimersAsync();
    expect(consoleSpy).toHaveBeenCalled();
    consoleSpy.mockRestore();
  });
  ...
  describe('runWithContext', () => {
    it('should run function with context', async () => {
      const { runWithContext, loggerContext } = await import('../../core/logger.js');
  ...
  describe('Coloring Edge Cases', () => {
    ...
    it('should include taskName and sessionId from context in script name', async () => {
      const { createLogger, runWithContext } = await import('../../core/logger.js');
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    it('should colorize all URL protocols and handle ANSI safety checks', async () => {
      ...
      logger.info('\u001b[31m@masked\u001b[0m "\u001b[32mquoted\u001b[0m" (\u001b[33mparen\u001b[0m) \u001b[34mhttps://masked.com\u001b[0m');
  ...
  describe('File Logging branches', () => {
    ...
    it('should handle writeToLogFile without context store', async () => {
      ...
      try {
        const { createLogger, loggerContext } = await import('../../core/logger.js');
    ...
    it('should include traceId in log entry if present in context', async () => {
      ...
      try {
        const { createLogger, runWithContext, logEmitter } = await import('../../core/logger.js');
        const logger = createLogger('test');
    ...
    it('should handle flushLogBuffer error', async () => {
      ...
      const fs = await import('fs');
      const appendFileSpy = vi.spyOn(fs, 'appendFile').mockImplementation((path, data, opt, cb) => {
