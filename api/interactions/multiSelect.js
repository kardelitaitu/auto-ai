...
async function resolveToCoords(input) {
    ...
    throw new Error('Invalid multi-select item. Use selector string, {x, y}, or element ID');
}
 ...
 */
async function clickWithModifiers(x, y, shift = false, ctrl = false, alt = false) {
    const page = getPage();
...
export async function multiSelect(items, options = {}) {
    ...
    try {
        if (mode === 'range') {
            if (items.length < 2) {
                throw new Error('Range selection requires at least 2 items (start and end)');
            }

            const startCoords = await resolveToCoords(items[0]);
            const endCoords = await resolveToCoords(items[items.length - 1]);

            logger.info(`Range: ${stringify(items[0])} → ${stringify(items[items.length - 1])}`);
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                logger.debug(`Clicking item ${i + 1}/${items.length}: (${coords.x}, ${coords.y})`);
