...
async function extractResources(screenshot, regions) {
    try {
        const regionDescriptions = Object.entries(regions)
            .map(([name, region]) => `${name}: region at (${region.x},${region.y}) size ${region.width}x${region.height}`)
            .join(', ');
...
async function extractAllTextResources(page) {
    ...
    const resourcePatterns = [
        { key: 'gold', selectors: ['[class*="gold"]', '[id*="gold"]', ':has-text("Gold")]' ] },
        { key: 'wood', selectors: ['[class*="wood"]', '[id*="wood"]', ':has-text("Wood")]' ] },
        { key: 'food', selectors: ['[class*="food"]', '[id*="food"]', ':has-text("Food")]' ] },
    ];
