...
import { ElementObscuredError as _ElementObscuredError, SessionDisconnectedError } from '../core/errors.js';
...
async function resolveToCoords(input, page) {
    ...
    if (typeof input === 'number') {
        ...
        if (!element) {
            throw new Error(`Element with ID ${input} not found. Call api.agent.see() first.`);
        }
    ...
    throw new Error('Invalid drag source/target. Use selector string, {x, y}, or element ID');
}
...
async function moveBezier(cursor, startX, startY, endX, endY, steps) {
    ...
    for (let i = 1; i <= steps; i++) {
        ...
        const x = mt3 * startX + 3 * mt2 * t * controlX + 3 * mt * t2 * controlX + t3 * endX;
        const y = mt3 * startY + 3 * mt2 * t * controlY + 3 * mt * t2 * controlY + t3 * endY;
...
export async function drag(source, target, options = {}) {
    ...
    return await pipeline(
        async () => {
            ...
            logger.info(`Dragging from ${stringify(source)} to ${stringify(target)} (${durationMs}ms)`);
            ...
            if (!isInViewport(startCoords.x, startCoords.y, viewport)) {
                throw new Error(`Start coordinates (${startCoords.x}, ${startCoords.y}) outside viewport`);
            }
            if (!isInViewport(endCoords.x, endCoords.y, viewport)) {
                throw new Error(`End coordinates (${endCoords.x}, ${endCoords.y}) outside viewport`);
            }
            ...
            switch (movement) {
                case 'direct':
                    await moveDirect(cursor, startCoords.x, startCoords.y, targetX, targetY, steps);
                    break;
                case 'arc':
                    await moveArc(cursor, startCoords.x, startCoords.y, targetX, targetY, steps);
                    break;
                case 'bezier':
                default:
                    await moveBezier(cursor, startCoords.x, startCoords.y, targetX, targetY, steps);
                    break;
            }
            ...
            logger.info(`Drag completed: (${Math.round(startCoords.x)},${Math.round(startCoords.y)}) → (${Math.round(targetX)},${Math.round(targetY)})`);
