...
async function moveWithPath(cursor, startX, startY, targetX, targetY, steps) {
    ...
    for (let i = 1; i <= steps; i++) {
        ...
        const x = mt3 * startX + 3 * mt2 * t * controlX + 3 * mt * t2 * controlX + t3 * targetX;
        const y = mt3 * startY + 3 * mt2 * t * controlY + 3 * mt * t2 * controlY + t3 * targetY;
...
export async function clickAt(x, y, options = {}) {
    ...
    const speedConfig = {
        ...
        normal: { moveMultiplier: 1.0, clickDelay: [50, 150], disableHumanPath: false },
        slow: { moveMultiplier: 2.0, clickDelay: [100, 300], disableHumanPath: false },
    };
    ...
    return await pipeline(
        async () => {
            ...
            if (x < 0 || x > viewport.width || y < 0 || y > viewport.height) {
                throw new Error(`Coordinates (${x}, ${y}) outside viewport (${viewport.width}x${viewport.height})`);
            }
            ...
            if (moveToFirst) {
                const startPos = cursor.previousPos || { x: viewport.width / 2, y: viewport.height / 2 };
                const useHumanPath = humanPath && !speedSettings.disableHumanPath;
                
                if (useHumanPath) {
                    const distance = Math.sqrt(Math.pow(targetX - startPos.x, 2) + Math.pow(targetY - startPos.y, 2));
                    const baseDuration = (100 + 0.3 * distance) * speedSettings.moveMultiplier;
                    const steps = Math.max(2, Math.floor(baseDuration / 20));
                    
                    await moveWithPath(cursor, startPos.x, startPos.y, targetX, targetY, steps);
                } else {
            ...
            await page.waitForTimeout(mathUtils.randomInRange(speedSettings.clickDelay[0], speedSettings.clickDelay[1]));
            await page.mouse.up({ button });

            cursor.previousPos = { x: targetX, y: targetY };

            logger.info(`ClickAt completed at (${Math.round(targetX)}, ${Math.round(targetY)})`);
            
            // Visual debug marker (only if enabled)
            if (visualDebug.isEnabled()) {
                await visualDebug.mark(Math.round(targetX), Math.round(targetY), 'CLICK');
            }
