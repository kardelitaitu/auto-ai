...
class Automator {
    ...
    getHealthyConnectionCount() {
        return Array.from(this.connections.values()).filter((conn) => conn.healthy).length;
    }
    ...
    async closeAll() {
        ...
        const closePromises = Array.from(this.connections.values()).map(async (connectionInfo) => {
            try {
    ...
    async checkPageResponsive(page) {
        try {
            ...
            const isResponsive =
                result.documentReady === 'complete' || result.documentReady === 'interactive';
