...
export default class HistoryCompactor {
    ...
    _performCompaction(actions) {
        ...
        if (result.length > this.targetLength) {
            const prioritizedActions = ['click', 'scroll', 'navigate', 'type', 'goto', 'wait', 'select'];
            ...
            const kept = new Set(sorted.slice(0, this.targetLength).map(r => r.action + '|' + r.target));
            return result.filter(r => kept.has(r.action + '|' + r.target)).slice(0, this.targetLength);
        }
    ...
    _createGroupedEntry(group) {
        ...
        return {
            ...
            target: uniqueTargets.length > 1 ? `${uniqueTargets.length} different targets` : group.targets[0],
            success: isSuccess,
            timestamp: group.timestamps[0]
        };
    }
