/**
 * @fileoverview Session Manager V2 - Robust worker management with health monitoring.
 * @module core/sessionManager-v2
 */
...
const logger = createLogger('sessionManager-v2.js');
...
class SessionManagerV2 {
  ...
  _initDatabase() {
    try {
      ...
      `);
      logger.info(`[V2] Database initialized at ${SESSION_DB_FILE}`);
    } catch (error) {
      logger.error(`[V2] Database init failed: ${error.message}`);
    }
  }
  ...
  async _checkStuckWorkers() {
    ...
    if (stuckWorkers.length > 0) {
      logger.warn(`[V2] Found ${stuckWorkers.length} stuck workers, force releasing...`);
  ...
  async loadConfiguration() {
    try {
      ...
      logger.info(`[V2] Configured: session=${this.sessionTimeoutMs}ms, concurrency=${this.concurrencyPerBrowser}`);
    } catch (error) {
      logger.warn('[V2] Config load failed:', error.message);
    }
  }
  ...
  addSession(browser, browserInfo, wsEndpoint) {
    ...
    if (existingIndex !== -1) {
      logger.info(`[V2] Session ${id} exists, updating browser`);
    ...
    logger.info(`[V2] Session added: ${id} (${this.concurrencyPerBrowser} workers)`);
  ...
  async replaceBrowserByEndpoint(wsEndpoint, newBrowser) {
    ...
    if (session) {
      logger.info(`[V2] Replacing browser for ${session.id}`);
  ...
  markSessionFailed(sessionId) {
    logger.warn(`[V2] Session ${sessionId} marked failed, removing`);
  ...
  removeSession(sessionId) {
    ...
    if (index !== -1) {
      ...
      logger.info(`[V2] Session removed: ${sessionId}`);
      return true;
    }
    return false;
  }
  ...
  async acquireWorker(sessionId, options = {}) {
    ...
    try {
      if (!(await sem.acquire(timeoutMs))) {
        logger.warn(`[V2] Acquire timeout for ${sessionId}`);
        return null;
      }
    } catch (err) {
      logger.error(`[V2] Semaphore error for ${sessionId}:`, err.message);
      return null;
    }
  ...
  async releaseWorker(sessionId, workerId) {
    ...
    if (worker && worker.status === 'busy') {
      ...
      logger.debug(`[V2] Released worker ${workerId} in ${sessionId} (${duration}ms)`);
    }
  }
  ...
  async forceReleaseWorker(sessionId, workerId) {
    ...
    if (worker) {
      ...
      this.workerOccupancy.delete(`${sessionId}:${workerId}`);
      this._getSemaphore(sessionId).release();
      logger.warn(`[V2] Force released worker ${workerId} in ${sessionId}`);
      return true;
    }
    return false;
  }
  ...
  async saveSessionState() {
    ...
    try {
      ...
      logger.error(`[V2] Save state failed: ${e.message}`);
    }
  }
  ...
  async loadSessionState() {
    ...
    try {
      ...
      logger.info(`[V2] Loaded ${rows.length} sessions from DB`);
      return { rows, meta };
    } catch (e) {
      logger.error(`[V2] Load state failed: ${e.message}`);
      return null;
    }
  }
  ...
  async shutdown() {
    logger.info('[V2] SessionManager shutting down...');
    ...
    logger.info('[V2] SessionManager shutdown complete');
  }
