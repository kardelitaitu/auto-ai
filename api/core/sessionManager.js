...
class SessionManager {
    ...
    async loadConfiguration() {
        try {
            ...
            logger.info(
                `[SessionManager] Configured: session=${this.sessionTimeoutMs}ms, concurrency=${this.concurrencyPerBrowser}`
            );
        } catch (error) {
    ...
    get idleSessionsCount() {
        return this.sessions.filter(
            (s) =>
                s.browser && s.browser.isConnected() && s.workers.some((w) => w.status === 'idle')
        ).length;
    }
    ...
    async cleanupTimedOutSessions() {
        ...
        this.sessions = this.sessions.filter((s) => now - s.lastActivity < this.sessionTimeoutMs);
