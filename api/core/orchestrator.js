...
class Orchestrator extends EventEmitter {
    ...
    _formatSessionDisplayName(endpointData) {
        ...
        const actualPort =
            endpointData.port || (endpointData.ws ? new URL(endpointData.ws).port : null);
    ...
    async processTasks() {
        ...
        const orderedSessions = [...sessions].sort(
            (a, b) => this._getSessionFailureScore(a.id) - this._getSessionFailureScore(b.id)
        );
        ...
        try {
            ...
            const workerPromises = session.workers.map(async (_worker) => {
                while (true) {
                    ...
                    const allocatedWorker = await this.sessionManager.acquireWorker(session.id, {
    ...
    async _importTaskModule(taskName) {
        ...
        try {
            ...
            const match = files.find(
                (f) => f.toLowerCase().replace('.js', '') === lowerBase && f.endsWith('.js')
            );
            if (match) {
