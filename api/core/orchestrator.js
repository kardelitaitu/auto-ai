...
class Orchestrator extends EventEmitter {
    ...
    try {
      ...
      for (const result of results) {
        if (result.status === 'fulfilled' && result.value) {
          ...
          logger.info(`[Orchestrator] Connected: ${displayName}`);
        }
      }

      const connectedCount = this.sessionManager.activeSessionsCount;
      logger.info(`[Orchestrator] Discovery complete. Connected to ${connectedCount} browsers.`);
  ...
  addTask(taskName, payload = {}) {
    ...
    logger.info(`[Orchestrator] Task '${taskName}' added (timeout: ${effectiveTimeout}ms). Queue: ${this.taskQueue.length}`);
  ...
  async processSharedChecklistForSession(session, tasks, _options = {}) {
    logger.info(`[Orchestrator][${session.id}] Starting checklist: ${tasks.length} tasks, ${session.workers.length} workers`);
      ...
      const workerPromises = session.workers.map(async (_worker) => {
        while (true) {
          ...
          if (this._isGroupTimeoutExceeded()) {
            logger.warn(`[Orchestrator][${session.id}] Group timeout exceeded. Stopping.`);
            break;
          }
          ...
          if (!allocatedWorker) {
            logger.warn(`[Orchestrator][${session.id}] No idle workers. Requeuing task '${task.taskName}'`);
            requeueTask(task);
          ...
          try {
            ...
            if (!pageHealth.healthy) {
              logger.warn(`[Orchestrator][${session.id}] Page unresponsive. Re-creating.`);
              await page.close().catch(() => { });
              page = await this.sessionManager.acquirePage(session.id, sharedContext);
            }

            logger.info(`[Orchestrator][${session.id}][Worker ${allocatedWorker.id}] Starting '${task.taskName}'`);
            await this.executeTask(task, page, session);
            session.completedTaskCount = (session.completedTaskCount || 0) + 1;
          } catch (e) {
            logger.error(`[Orchestrator][${session.id}][Worker ${allocatedWorker.id}] Task error:`, e.message);
            this._recordSessionOutcome(session.id, false);

            if (task.retriesLeft === undefined) task.retriesLeft = this.maxTaskRetries;
            if (task.retriesLeft > 0) {
              task.retriesLeft--;
              logger.warn(`[Orchestrator][${session.id}] Retrying '${task.taskName}'. Left: ${task.retriesLeft}`);
              requeueTask(task);
            } else {
              logger.error(`[Orchestrator][${session.id}] Task '${task.taskName}' failed permanently.`);
  ...
  async executeTask(task, page, session) {
    ...
    try {
      ...
      error = err;
      success = false;
      logger.error(`[Orchestrator] Task '${task.taskName}' error:`, err.message);

      if (err instanceof TaskTimeoutError) {
        logger.warn(`[Orchestrator] Task '${task.taskName}' timed out after ${effectiveTimeout}ms`);
  ...
  async _importTaskModule(taskName) {
    ...
    try {
      ...
      if (match) {
        ...
        logger.info(`[Orchestrator] Task '${taskName}' resolved via case-insensitive lookup → ${match}`);
        return await import(fileUrl);
      }
    } catch (_e) {
      logger.warn(`[Orchestrator] Case-insensitive task lookup failed for '${taskName}': ${_e.message}`);
    }
  ...
  async waitForTasksToComplete(options = {}) {
    ...
    return new Promise((resolve, _reject) => {
      ...
      const _timeoutId = setTimeout(() => {
        ...
        logger.warn(`[Orchestrator] waitForTasksToComplete timed out after ${timeoutMs}ms`);
  ...
  async shutdown(force = false) {
    ...
    if (!force) {
      ...
      if (result.timedOut) {
        logger.warn('[Orchestrator] Shutdown: tasks still running, forcing cancel');
        this._forceCancelAllTasks();
      }
    } else {
      this._forceCancelAllTasks();
  ...
  async startDashboard(port = 3001) {
    try {
      ...
      if (fs.existsSync(uiPath)) {
        ...
        logger.info(`[Orchestrator] Dashboard started on port ${port}`);
      }
    } catch (_error) {
      logger.warn('[Orchestrator] Failed to start dashboard:', _error.message);
    }
  }
