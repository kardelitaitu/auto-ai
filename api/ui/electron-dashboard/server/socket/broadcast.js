...
export class BroadcastManager {
    ...
    collectMetrics() {
        try {
            ...
            if (this.SESSION_TTL_MS > 0 && this.server.dashboardData.sessions.length > 0) {
                const cutoff = now - this.SESSION_TTL_MS;
                const beforeCount = this.server.dashboardData.sessions.length;
                this.server.dashboardData.sessions = this.server.dashboardData.sessions.filter(s => {
                    const lastSeen = s.lastSeen || s.firstSeen || 0;
                    return lastSeen > cutoff;
                });
                if (this.server.dashboardData.sessions.length < beforeCount) {
                    logger.debug(`Cleaned up ${beforeCount - this.server.dashboardData.sessions.length} stale sessions`);
                }
            }
            ...
            const hasOnlineSession = this.server.latestMetrics?.sessions?.some(s => s?.status === 'online') || false;
            const hasActivity = hasOnlineSession ||
                (this.server.latestMetrics?.queue?.queueLength > 0);
