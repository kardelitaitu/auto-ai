...
import { validateTask, validateSession, validateMetrics, validatePayload } from './socket/validation.js';
...
export class DashboardServer {
    ...
    updateMetrics(payload) {
        ...
        // Merge queue
        if (payload.queue) {
            this.latestMetrics.queue = { ...this.latestMetrics.queue, ...payload.queue };
            this.dashboardData.queue = payload.queue;
        }

        // Merge metrics
        if (payload.metrics) {
            this.latestMetrics.metrics = { ...this.latestMetrics.metrics, ...payload.metrics };
        ...
        if (payload.cumulative) {
            this.cumulativeMetrics = { ...this.cumulativeMetrics, ...payload.cumulative };
        }

        // Merge errors
        if (payload.errors) {
            this.dashboardData.errors = [...this.dashboardData.errors, ...payload.errors].slice(-100);
        }
    }
    ...
    mergeTaskData(task) {
        ...
        const existingIndex = this.dashboardData.tasks.findIndex(t => t.id === task.id);
        if (existingIndex >= 0) {
            this.dashboardData.tasks[existingIndex] = { ...this.dashboardData.tasks[existingIndex], ...task };
        } else {
    ...
    async start() {
        ...
        try {
            ...
            expressApp.use(createDashboardRouter({
                server: this,
                authEnabled: AUTH_ENABLED,
                authToken: AUTH_TOKEN
            }));
            expressApp.use(createExportRouter({
                server: this,
                authEnabled: AUTH_ENABLED,
                authToken: AUTH_TOKEN
            }));
    ...
    collectMetrics() {
        ...
        return { error: 'Broadcast manager not initialized', timestamp: Date.now() };
    }
