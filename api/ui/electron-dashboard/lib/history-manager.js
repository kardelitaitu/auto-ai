...
export class HistoryManager {
    ...
    load() {
        try {
            ...
            if (fs.existsSync(this.historyFilePath)) {
                ...
                this.apiMetrics = data.apiMetrics || { calls: 0, failures: 0, successRate: 100, avgResponseTime: 0 };
                logger.info(`Loaded ${this.tasks.length} tasks and cumulative metrics from ${path.basename(this.historyFilePath)}`);
            }
        } catch (err) {
    ...
    getTwitterActions() {
        return this.twitterActions || { likes: 0, retweets: 0, replies: 0, quotes: 0, follows: 0, bookmarks: 0, total: 0 };
    }
    ...
    getApiMetrics() {
        return this.apiMetrics || { calls: 0, failures: 0, successRate: 100, avgResponseTime: 0 };
    }
