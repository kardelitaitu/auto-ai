...
async function createWindow() {
    ...
    const hasRemoteHost = config?.connect?.remoteHost && config.connect.remoteHost.trim().length > 0;
    ...
    if (!connectToRemote) {
        ...
        try {
            ...
            if (err.code === 'EADDRINUSE') {
                console.log(`[Dashboard] Port ${serverPort} already in use - connecting to existing server`);
    ...
    const windowOptions = {
        width: windowConfig?.bounds?.width || (config?.ui?.defaultCompact ? 400 : 1200),
        height: windowConfig?.bounds?.height || (config?.ui?.defaultCompact ? 600 : 800),
    ...
    if (!app.isPackaged) {
        ...
        const reactPath = path.join(__dirname, 'renderer', 'dist', 'index.html');
        mainWindow.loadFile(reactPath, { query: { server: serverUrl } }).catch((err) => {
    ...
    ipcMain.on('set-window-size', (event, { width, height, compact }) => {
        ...
        console.log(`[Dashboard] Window resized to: ${width}x${height} (Compact: ${compact})`);
    });
}
