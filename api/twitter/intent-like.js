...
const logger = createLogger('api/twitter/intent-like.js');

/**
 * Navigates to the Twitter like intent URL and confirms the action.
 * @param {string} url - The URL of the tweet to like
 * @returns {Promise<{success: boolean, reason?: string}>}
 * @example
 * await api.twitter.intent.like('https://x.com/user/status/123456789');
 */
export async function like(url) {
    ...
    const actionPromise = (async () => {
        ...
        try {
            ...
            const intentUrl = `https://x.com/intent/like?tweet_id=${tweetId}`;
            ...
            if (await visible(confirmBtn)) {
                ...
                logger.warn('Confirm button not found. May already be liked.');
                return { success: false, reason: 'confirm_button_not_found' };
            }
        } catch (error) {
            logger.error(`Unhandled error in like intent: ${error.message}`);
            return { success: false, reason: 'unhandled_error', error: error.message };
        } finally {
            if (navigated) {
