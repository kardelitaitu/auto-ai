...
const createPageOps = (page) => {
    ...
    const waitForSelector = (selector, options = {}) => {
        ...
        return page.waitForSelector ? page.waitForSelector(selector, options) : Promise.resolve();
    };
...
export class AITwitterAgent extends TwitterAgent {
    constructor(page, initialProfile, logger, options = {}) {
        ...
        microConfig.highlightChance = Math.min(0.12, microConfig.highlightChance * actionScale);
        microConfig.rightClickChance = Math.min(0.08, microConfig.rightClickChance * actionScale);
        microConfig.logoClickChance = Math.min(0.12, microConfig.logoClickChance * actionScale);
        microConfig.whitespaceClickChance = Math.min(
            0.1,
            microConfig.whitespaceClickChance * actionScale
        );
        microConfig.fidgetChance = Math.max(0.02, Math.min(0.2, microMoveChance * 1.2));
        ...
        this.microHandler = microInteractions.createMicroInteractionHandler(microConfig);
    ...
    async performIdleCursorMovement() {
        try {
            const viewport = this.pageOps.viewportSize() || { width: 1280, height: 720 };
    ...
    async triggerMicroInteraction(context = 'reading') {
        try {
            ...
            const result = await this.microHandler.executeMicroInteraction(this.pageOps.page, {
                logger: {
    ...
    async smartClick(context, options = {}) {
        ...
        try {
            const result = await this.motorHandler.smartClick(this.pageOps.page, null, {
                logger: {
                    info: (msg) => this.log(msg),
    ...
    async _diveTweetWithAI(queueWrapper = null) {
        try {
            ...
            if (Math.random() < 0.7) {
                this.log('[Dive] Processing feed before selecting target (thinking)...');
            ...
            if (!targetTweet) {
                for (let attempt = 0; attempt < 3; attempt++) {
                    ...
                    const freshTweets = this.pageOps.locator('article[data-testid="tweet"]');
            ...
            if ((await permalinkLink.count()) > 0 && (await api.visible(permalinkLink))) {
                clickTarget = permalinkLink;
                this.log('[Debug] Targeting tweet permalink link (Primary).');
            } else if ((await tweetTextEl.count()) > 0 && (await api.visible(tweetTextEl))) {
                clickTarget = tweetTextEl;
                this.log('[Debug] Targeting tweet text body (Fallback).');
            } else if ((await timeStamp.count()) > 0 && (await api.visible(timeStamp))) {
            ...
            let tweetText = '';
            const freshTextEl = this.pageOps.locator('[data-testid="tweetText"]').first();
    ...
    async _quickFallbackEngagement() {
        try {
            ...
            const engagementType =
                engagementRoll < 0.4 ? 'like' : engagementRoll < 0.7 ? 'bookmark' : 'none';
    ...
    async _ensureExplorationScroll() {
        ...
        const docHeight = await this.pageOps.evaluate(() => document.body.scrollHeight);
        ...
        if (currentY < 100 || isNewSession) {
            const reason = isNewSession ? 'New session/navigated home' : 'At top of feed';
    ...
    async _readExpandedTweet() {
        ...
        const persona = api.getPersona();
        const hoverMin = persona && typeof persona.hoverMin === 'number' ? persona.hoverMin : 1000;
        const hoverMax = persona && typeof persona.hoverMax === 'number' ? persona.hoverMax : 3000;
        ...
        while (Date.now() - readStart < readTime) {
            ...
            const segment = Math.max(600, Math.min(2500, mathUtils.randomInRange(800, 2000)));
        ...
        const scrollSpeed =
            persona && typeof persona.scrollSpeed === 'number' ? persona.scrollSpeed : 1;
     ...
     */
    async runSession(cycles = 10, minDurationSec = 0, maxDurationSec = 0, options = {}) {
    ...
    async executeAIReply(replyText) {
        try {
            ...
            const result = await this.replyEngine.executeReply(this.pageOps.page, replyText);
    ...
    async executeAIQuote(quoteText, _tweetUrl = '') {
        try {
            ...
            const result = await this.quoteEngine.executeQuote(this.pageOps.page, quoteText);
    ...
    async verifyReplyPosted(replyText) {
        try {
            ...
            const composer = this.pageOps.locator('[data-testid="tweetText"]').first();
    ...
    async humanTypingWithTypos(inputEl, text) {
        ...
        const persona = api.getPersona();
        const speed = typeof persona.speed === 'number' && persona.speed > 0 ? persona.speed : 1;
        const typoRate = typeof persona.typoRate === 'number' ? persona.typoRate : 0.08;
        const correctionRate =
            typeof persona.correctionRate === 'number' ? persona.correctionRate : 0.85;
        ...
        try {
            switch (action) {
                case 'bookmark': {
                    ...
                    const bm = this.pageOps.locator('button[data-testid="bookmark"]').first();
    ...
    async handleLike(tweetText = null) {
        try {
            ...
            // Check engagement limits from both systems
            const canLike =
                this.engagementTracker.canPerform('likes') && this.diveQueue.canEngage('likes');
            if (!canLike) {
            ...
            const likeResult = await this.humanInteraction.findWithFallback(likeSelectors, {
                visible: true,
                timeout: 5000,
            });
            ...
            const unlikeResult = await this.humanInteraction.findWithFallback(unlikeSelectors, {
                visible: true,
                timeout: 1000,
            });
            if (unlikeResult) {
            ...
            if (!isActionable) {
                this.log('[Like] ⚠️ Element may be covered, trying scroll adjustment...');
                try {
    ...
    async handleBookmark() {
        try {
            ...
            const bookmarkResult = await this.humanInteraction.findWithFallback(bookmarkSelectors, {
                visible: true,
                timeout: 5000,
            });
            ...
            if (!isActionable) {
                this.log('[Bookmark] ⚠️ Element may be covered, trying scroll adjustment...');
                try {
    ...
    async handleAIQuote(tweetText, username, options = {}) {
        ...
        const result = await this.quoteEngine.executeQuote(this.pageOps.page, quoteResult.quote);
        ...
        if (result.success) {
            ...
            if (!composerClosed) {
                this.log(`[AI-Quote] ⚠️ Composer may still be open, attempting to close...`);
                try {
                    await this.pageOps.keyboardPress('Escape');
                    await this.wait(500);
                } catch (e) {
    ...
    logEngagementStatus() {
        ...
        for (const [action, data] of Object.entries(status)) {
            const emoji =
                data.remaining === 0 ? '🚫' : parseFloat(data.percentage) >= 80 ? '⚠️' : '✅';
    ...
    async flushLogs() {
        ...
        await Promise.all([this.queueLogger.shutdown(), this.engagementLogger.shutdown()]);
