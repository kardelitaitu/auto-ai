...
class MetricsCollector {
    ...
    recordSocialAction(type, count = 1) {
        ...
        if (typeof type !== 'string') {
            logger.warn(`[recordSocialAction] Invalid type: expected string, got ${typeof type}`);
            return;
        }

        // Validate count
        const safeCount =
            typeof count === 'number' && !isNaN(count) && count > 0 ? Math.floor(count) : 0;
    ...
    getStats() {
        ...
        const apiSuccessRate =
            this.metrics.apiCalls > 0
                ? (
                      ((this.metrics.apiCalls - this.metrics.apiFailures) / this.metrics.apiCalls) *
                      100
                  ).toFixed(2)
                : 0;
            ...
            browsers: {
                ...
                connectionRate:
                    this.metrics.browsersDiscovered > 0
                        ? (
                              (this.metrics.browsersConnected / this.metrics.browsersDiscovered) *
                              100
                          ).toFixed(2)
                        : 0,
            },
