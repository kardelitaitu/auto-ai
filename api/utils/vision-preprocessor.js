...
export class VisionPreprocessor {
    ...
    async process(input, config = {}) {
        ...
        try {
            ...
            logger.info(`[V-PREP] Processing: ${metadata.width}x${metadata.height}, ${formatBytes(originalSize)}`);
            ...
            if (!appliedROI && cfg.autoROI) {
                ...
                if (appliedROI) {
                    logger.info(`[V-PREP] Auto-detected ROI: ${JSON.stringify(appliedROI)}`);
                }
            }
            ...
            this.stats.avgCompressionRatio =
                (this.stats.avgCompressionRatio * (this.stats.totalProcessed - 1) + compressionRatio) /
                this.stats.totalProcessed;
            ...
            for (let y = 0; y < height - blockSize; y += blockSize / 2) {
                for (let x = 0; x < width - blockSize; x += blockSize / 2) {
                    const saliency = this._calculateBlockSaliency(
                        data, x, y, blockSize, width, channels
                    );
