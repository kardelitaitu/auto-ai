...
export class AIReplyEngine {
  ...
  async shouldReply(tweetText, authorUsername, context = {}) {
    ...
    this.logger.debug(`[AIReply] Tweet clipped from ${tweetText.length} to ${clippedTweet.length} chars`);
    ...
    if (tweetSentiment.composite?.riskLevel === "high") {
      this.stats.skips++;
      return { decision: "skip", reason: "high_risk_conversation", action: this.randomFallback() };
    }
    ...
    return { decision: "reply", reason: "success", action: "post_reply", reply: aiResult.reply.trim() };
  }
  ...
  async generateReply(tweetText, authorUsername, context = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const prompt = this.buildEnhancedPrompt(tweetText, authorUsername, context, strategy);
        ...
        lastError = error;
        this.logger.warn(`[AIReply] Generation attempt ${attempt}/${maxAttempts} failed: ${error.message}`);
  ...
  resetStats() {
    this.stats = { attempts: 0, successes: 0, skips: 0, failures: 0, safetyBlocks: 0, errors: 0 };
  }
  ...
  async replyMethodA_Keyboard(page, replyText, human) {
    ...
    const focusSelectors = ["article time", '[data-testid="tweetText"]', 'article[role="article"] [dir="auto"]'];
