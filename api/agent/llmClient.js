...
class LLMClient {
    ...
    async generateCompletion(messages) {
        ...
        try {
            ...
            logger.info(`Raw LLM Output: ${content.substring(0, 500)}${content.length > 500 ? '...' : ''}`);
            ...
            try {
                ...
                logger.warn(`Initial JSON parse failed, attempting robust extraction...`);
                ...
                if (cleanIdx !== -1) {
                    ...
                    if (lastBrace !== -1) {
                        ...
                        try {
                            ...
                            logger.warn(`Extracted JSON parse failed, attempting repair: ${extractError.message}`);
                            ...
                            // Balance quotes
                            const quoteCount = (repaired.match(/"/g) || []).filter(c => c !== '\\"').length;
                        ...
                        logger.warn('Detected truncated JSON with no closing brace, attempting emergency repair...');
                        ...
                        // Balance quotes
                        if (!repaired.endsWith('"') && (repaired.match(/"/g) || []).length % 2 !== 0) repaired += '"';
                        ...
                        try {
                            ...
                            const targetPattern = /["']?x["']?\s*:\s*(\d+)\s*,\s*["']?y["']?\s*:\s*(\d+)(?:\s*,\s*["']?price["']?\s*:\s*["']?(\w+)["']?)?/g;
                            const targets = [];
                            let match;
                            ...
                            if (targets.length > 0) {
                                logger.info(`Regex extraction successful: found ${targets.length} target(s)`);
                                targets.forEach((t, i) => {
                                    logger.info(`  Target ${i + 1}: x=${t.x}, y=${t.y}, price=${t.price}`);
                                });
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    logger.warn(`LLM attempt ${attempt}/${maxRetries} failed, retrying in ${delay}ms...`);
            ...
            function: {
                ...
                parameters: {
                    ...
                    properties: {
                        action: {
                            type: "string",
                            enum: ["click", "clickAt", "type", "press", "scroll", "wait", "verify", "done", "drag", "navigate"],
