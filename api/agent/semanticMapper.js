...
class SemanticMapper {
    ...
    _traverseAndEnrich(node, depth = 0) {
        ...
        if (node.children && depth < 5) { // Limit depth
            enriched.children = node.children.map(child =>
                this._traverseAndEnrich(child, depth + 1)
            ).filter(Boolean);
        }
    ...
    _identifySemantics(node) {
        ...
        if (combinedText.includes('submit') || combinedText.includes('post') || combinedText.includes('tweet')) {
            tags.add('action');
            tags.add('submit');
        }
    ...
    _calculateClickConfidence(node) {
        ...
        for (const word of actionWords) {
            if (name.includes(word)) return 0.80;
        }

        // Lower confidence for generic elements
        if (role.includes('generic') || role.includes('text')) return 0.40;
    ...
    _calculateTypeConfidence(node) {
        ...
        if (role.includes('combobox')) return 0.90;
        if (role.includes('searchbox')) return 0.95;

        return 0.20; // Low confidence for non-input elements
    }
    ...
    _calculateScrollConfidence(node) {
        ...
        if (role.includes('main') || role.includes('region')) return 0.70;
    ...
    _suggestActions(node) {
        ...
        if (tags.includes('interactive') || tags.includes('clickable')) {
            suggestions.push({ action: 'click', confidence: this._calculateClickConfidence(node) });
        }

        if (tags.includes('input')) {
            suggestions.push({ action: 'type', confidence: this._calculateTypeConfidence(node) });
        }

        if (tags.includes('structural') || tags.includes('content')) {
            suggestions.push({ action: 'scroll', confidence: this._calculateScrollConfidence(node) });
        }
    ...
    _countElements(node, summary) {
        ...
        if (node.category) {
            summary.categories[node.category] = (summary.categories[node.category] || 0) + 1;
        }
        ...
        if (node.suggestedActions && node.suggestedActions.length > 0) {
            ...
            summary.topActions[bestAction] = (summary.topActions[bestAction] || 0) + 1;
        }
