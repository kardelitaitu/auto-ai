...
class ActionRollback {
    ...
    recordAction(preState, action, result) {
        ...
        logger.debug(`[Rollback] Recorded action: ${action.action} (history: ${this.actionHistory.length})`);
    }
    ...
    async rollbackLast(page) {
        ...
        try {
            ...
            if (lastRecord.preState.url && lastRecord.preState.url !== page.url()) {
                ...
                await page.goto(lastRecord.preState.url, { waitUntil: 'domcontentloaded' });
            }
            ...
            if (lastRecord.action.action === 'type' && lastRecord.preState.formValue !== null) {
                try {
                    await page.evaluate((selector, value) => {
                        const el = document.querySelector(selector);
                        if (el) el.value = value;
                    }, lastRecord.action.selector, lastRecord.preState.formValue);
