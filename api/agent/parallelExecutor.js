...
class ParallelExecutor {
    constructor() {
        ...
        this.dependentActions = new Set(['click', 'clickAt', 'type', 'navigate', 'scroll', 'drag', 'multiSelect']);
    }
    ...
    async executeSequence(actions, executeFn) {
        ...
        logger.info(`[ParallelExecutor] Executing ${actions.length} actions: ${independent.length} independent, ${dependent.length} dependent`);

        const results = [];

        // Execute independent actions in parallel
        if (independent.length > 0) {
            const parallelResults = await this._executeParallel(independent, executeFn);
            results.push(...parallelResults);
        }

        // Execute dependent actions sequentially
        if (dependent.length > 0) {
            const sequentialResults = await this._executeSequential(dependent, executeFn);
    ...
    async _executeParallel(actions, executeFn) {
        const promises = actions.map(async (action) => {
            try {
                ...
                logger.error(`[ParallelExecutor] Parallel action failed: ${action.action}`, error.message);
                return { action, result: { success: false, error: error.message }, success: false };
            }
        });
        ...
        return results.map((r, i) => ({
            action: actions[i],
            result: r.status === 'fulfilled' ? r.value.result : { success: false, error: 'Promise rejected' },
    ...
    async _executeSequential(actions, executeFn) {
        ...
        for (const action of actions) {
            try {
                ...
                logger.error(`[ParallelExecutor] Sequential action failed: ${action.action}`, error.message);
    ...
    _orderByOriginalOrder(results, originalActions) {
        ...
        return originalActions.map((_, index) => resultMap.get(index)).filter(Boolean);
    }
    ...
    canParallelize(actions) {
        ...
        const independentCount = actions.filter(a => this._isIndependent(a)).length;
    ...
    estimateSpeedup(actions) {
        ...
        const parallelTime = Math.max(...independent.map(() => 100), 0) + (dependent.length * 200);
