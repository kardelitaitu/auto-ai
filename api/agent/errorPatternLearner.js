...
class ErrorPatternLearner {
    ...
    recordError(error, context = {}) {
        ...
        logger.debug(`[ErrorPatternLearner] Recorded error pattern: ${patternKey} (count: ${pattern.count})`);
    }
    ...
    _calculateRelevance(pattern, context) {
        ...
        if (pattern.context.url && context.url) {
            ...
            if (patternUrl === contextUrl) {
                relevance += 0.5;
            } else if (patternUrl.includes(contextUrl) || contextUrl.includes(patternUrl)) {
                relevance += 0.25;
            }
        }
        ...
        if (pattern.context.goal && context.goal) {
            const goalSimilarity = this._stringSimilarity(pattern.context.goal, context.goal);
    ...
    _generatePreventionStrategy(pattern) {
        ...
        switch (errorType) {
            case 'selector_not_found':
                return {
                    pattern: pattern.key,
                    strategy: 'Use more specific selectors or try text/role-based selectors',
