...
class HistoryManager {
    ...
    _scoreRelevance(entry, context) {
        ...
        if (entry.url && context.url) {
            ...
            if (entryUrl === contextUrl) {
                score += 0.3;
            } else if (entryUrl.includes(contextUrl) || contextUrl.includes(entryUrl)) {
                score += 0.15;
            }
        }
    ...
    _compress() {
        ...
        logger.debug(`[HistoryManager] Compressed ${oldEntries.length} entries into summary`);
    }
    ...
    _createSummary(entries) {
        ...
        const urls = [...new Set(entries.map(e => this._normalizeUrl(e.url)).filter(Boolean))];
