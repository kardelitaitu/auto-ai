...
const _logger = createLogger('api/agent/confidenceScorer.js');

class ConfidenceScorer {
    constructor() {
        this.commonPatterns = {
            click: [
                'button', 'link', 'submit', 'login', 'sign', 'start', 'open',
                'view', 'read', 'see', 'go', 'navigate', 'select', 'choose'
            ],
            type: [
                'input', 'field', 'box', 'text', 'enter', 'fill', 'write',
                'username', 'password', 'email', 'search', 'query'
            ],
            scroll: [
                'more', 'down', 'up', 'top', 'bottom', 'page', 'content',
                'article', 'post', 'feed', 'list', 'results'
            ],
            wait: [
                'loading', 'animation', 'transition', 'appear', 'show',
                'complete', 'finish', 'ready', 'stable'
            ],
        };
    }
    ...
    _scoreStructure(response) {
        ...
        const hasAllRequired = requiredFields.every(field => response[field] !== undefined);
        if (hasAllRequired) score += 0.4;

        // No extra/unknown fields
        const validFields = ['action', 'selector', 'x', 'y', 'value', 'key', 'description', 'rationale', 'clickType', 'duration', 'items', 'mode'];
        const extraFields = Object.keys(response).filter(f => !validFields.includes(f));
    ...
    _scoreSelector(response) {
        ...
        if (selector.startsWith('#')) score += 0.4; // ID
        else if (selector.startsWith('.')) score += 0.3; // Class
        else if (selector.includes('[')) score += 0.2; // Attribute
        else if (selector.includes('text=')) score += 0.2; // Text
    ...
    _scoreRationale(response) {
        ...
        const actionVerbs = ['click', 'type', 'scroll', 'wait', 'verify', 'navigate', 'select'];
