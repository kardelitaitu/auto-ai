...
class AgentRunner {
    ...
    async run(goal, config = {}) {
        ...
        logger.info(`Starting Agent with goal: "${goal}" (Max Steps: ${this.maxSteps})`);
        ...
        while (stepCount < this.maxSteps && !this._abortController.signal.aborted) {
            ...
            const messages = this._buildPrompt(goal, state.screenshot, state.axTree, state.url);
            ...
            if (useAdaptive) {
                try {
                    await page.waitForLoadState('networkidle', { timeout: this.stepDelay });
                } catch {
    ...
    _generateSemanticStateHash(axTreeJson) {
        ...
        try {
            let text = typeof axTreeJson === 'string' ? axTreeJson : JSON.stringify(axTreeJson);
