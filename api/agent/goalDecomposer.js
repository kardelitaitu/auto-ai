...
class GoalDecomposer {
    constructor() {
        this.goalPatterns = {
            'login': {
                ...
                steps: [
                    { subGoal: 'Navigate to login page', expectedOutcome: 'Login form visible' },
                    { subGoal: 'Enter username/email', expectedOutcome: 'Username field filled' },
                    { subGoal: 'Enter password', expectedOutcome: 'Password field filled' },
            ...
            'purchase': {
                ...
                steps: [
                    { subGoal: 'Find the product', expectedOutcome: 'Product page visible' },
                    { subGoal: 'Add product to cart', expectedOutcome: 'Product in cart' },
                    { subGoal: 'Go to checkout', expectedOutcome: 'Checkout page visible' },
                    { subGoal: 'Enter shipping details', expectedOutcome: 'Shipping form filled' },
            ...
            'post': {
                ...
                steps: [
                    { subGoal: 'Navigate to compose area', expectedOutcome: 'Compose form visible' },
                    { subGoal: 'Enter message content', expectedOutcome: 'Content typed' },
                    { subGoal: 'Add media if needed', expectedOutcome: 'Media attached' },
                    { subGoal: 'Click publish/post button', expectedOutcome: 'Content published' },
                ],
            },
            'search': {
                keywords: ['search', 'find', 'look for', 'locate'],
                steps: [
                    { subGoal: 'Find search input', expectedOutcome: 'Search field visible' },
                    { subGoal: 'Enter search query', expectedOutcome: 'Query typed' },
                    { subGoal: 'Execute search', expectedOutcome: 'Results displayed' },
                    { subGoal: 'Find target in results', expectedOutcome: 'Target located' },
                ],
            },
            'register': {
                keywords: ['register', 'sign up', 'create account'],
                steps: [
                    { subGoal: 'Navigate to registration page', expectedOutcome: 'Registration form visible' },
                    { subGoal: 'Fill required fields', expectedOutcome: 'Form filled' },
                    { subGoal: 'Accept terms if required', expectedOutcome: 'Terms accepted' },
                    { subGoal: 'Submit registration', expectedOutcome: 'Account created' },
                ],
            },
        };
    }
    ...
    async decompose(goal, currentUrl) {
        ...
        logger.info(`[GoalDecomposer] No pattern match, using heuristic decomposition`);
    ...
    _heuristicDecompose(goal, _currentUrl) {
        ...
        for (const [word, description] of Object.entries(actionWords)) {
            if (goalLower.includes(word)) {
                steps.push({ subGoal: description, expectedOutcome: 'Action completed' });
                hasAction = true;
            }
        }
        ...
        if (!hasAction) {
            steps.push(
                { subGoal: 'Analyze the current page state', expectedOutcome: 'Page understood' },
                { subGoal: 'Locate target element or area', expectedOutcome: 'Target found' },
                { subGoal: 'Perform the required action', expectedOutcome: 'Action completed' },
                { subGoal: 'Verify the action succeeded', expectedOutcome: 'Goal achieved' }
            );
        }
    ...
    getProgress(decomposition) {
        ...
        return Math.round((decomposition.currentStep / decomposition.totalSteps) * 100);
    }
}
