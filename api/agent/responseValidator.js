...
const _logger = createLogger('api/agent/responseValidator.js');

class ResponseValidator {
    constructor() {
        this.validActions = [
            'click', 'clickAt', 'type', 'wait', 'verify', 'done',
            'scroll', 'navigate', 'drag', 'multiSelect', 'press'
        ];
    ...
    _validateSingle(action, index = null) {
        ...
        if (!action.action) {
            return { valid: false, errors: [`${prefix}Missing 'action' field`], warnings: [] };
        }
        ...
        if (action.selector) {
            ...
            if (!selectorValid.valid) {
                warnings.push(`${prefix}Selector may be invalid: ${selectorValid.reason}`);
            }
        }
        ...
        if (action.value !== undefined && typeof action.value !== 'string' && typeof action.value !== 'number') {
            warnings.push(`${prefix}'value' should be a string or number`);
        }

        // Check for rationale (recommended)
        if (!action.rationale) {
            warnings.push(`${prefix}Missing 'rationale' (recommended for better decision tracking)`);
        }
    ...
    _validateSelector(selector) {
        ...
        const placeholders = ['...', 'placeholder', 'N/A', 'TODO', 'FIXME'];
