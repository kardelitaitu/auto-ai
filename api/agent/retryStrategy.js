...
class RetryStrategy {
    ...
    getNextStrategy(error, failedAction, attempt, context = {}) {
        ...
        const action = this._generateRetryAction(strategyName, failedAction, context);

        logger.info(`[RetryStrategy] Strategy for attempt ${attempt}: ${strategyName}`);
    ...
    _classifyError(error) {
        ...
        if (errorLower.includes('selector') || errorLower.includes('not found') || errorLower.includes('cannot find')) {
            return 'selector_not_found';
        }

        if (errorLower.includes('not visible') || errorLower.includes('hidden') || errorLower.includes('obscured')) {
        ...
        switch (strategyName) {
            ...
            case 'use_text_selector':
                if (action.selector) {
                    ...
                    const text = this._extractTextFromSelector(action.selector) || action.rationale;
