...
class VisualDiffEngine {
    ...
    async _initSharp() {
        try {
            ...
            logger.warn('Sharp module not available, using fallback comparison:', error.message);
            this.sharp = null;
        }
    }
    ...
    _fallbackComparison(preBuffer, postBuffer) {
        if (!preBuffer || !postBuffer) {
            return { changed: false, diffRatio: 0, diffPixels: 0, significantChange: false, method: 'fallback' };
        }
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                if (diffBuffer[idx] > 0 && !visited.has(idx)) {
                    const region = this._floodFill(diffBuffer, x, y, width, height, visited);
                    if (region.pixelCount > 50) { // Minimum region size
                        regions.push(region);
                    }
                }
            }
