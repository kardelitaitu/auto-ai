...
class SelfHealingPrompt {
    ...
    generateHealingInstructions(context) {
        ...
        instructions += 'The following actions have failed recently. Consider alternative approaches:\n\n';
    ...
    _getRelevantFailures(context) {
        return this.recentFailures.filter(failure => {
            ...
            if (failure.context?.goal && context.goal) {
                const similarity = this._stringSimilarity(failure.context.goal, context.goal);
                if (similarity > 0.5) {
                    return true;
                }
            }
    ...
    _generateGeneralAdvice(failures) {
        ...
        if (errorCounts.selector_not_found > 1) {
            advice.push('Multiple selector failures detected. Try using text-based or role-based selectors.');
        }

        if (errorCounts.element_not_visible > 1) {
            advice.push('Elements are frequently not visible. Always scroll to elements before clicking.');
        }

        if (errorCounts.timeout > 1) {
            advice.push('Multiple timeouts detected. Consider increasing wait times or checking page load status.');
        }

        if (failures.length >= 3) {
            advice.push('Multiple failures detected. Consider taking a completely different approach.');
        }
    ...
    generateRecoveryPrompt(failure) {
        ...
        prompt += '4. Simplify the action (e.g., use coordinates instead of selector)\n';
