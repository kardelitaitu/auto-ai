...
class ActionEngine {
        ...
        try {
            switch (action.action) {
                ...
                case 'clickAt': {
                    ...
                    await this.performClickAt(page, cx, cy, action.clickType, action.duration);
                    break;
                }
                case 'type':
                ...
                case 'delay':
                    ...
                    break;
                case 'drag':
                    await this.performDrag(page, action.source, action.target, action.duration);
                    break;
                case 'multiSelect':
                ...
                case 'verify':
                    ...
                    logger.info(`Verification step reached: ${action.description || 'No description'}`);
                    break;
                case 'done':
    ...
    getLocator(page, selector) {
        if (!selector || typeof selector !== 'string') {
            throw new Error(`Invalid selector: "${selector}". Selector must be a non-empty string.`);
        }

        // Defensive check for placeholders the model might mimic
        if (selector === '...' || selector === 'placeholder' || selector === 'N/A' || selector.includes('-id')) {
            throw new Error(`Model error: "${selector}" is a placeholder. Please use a real selector from the AXTree or screenshot.`);
        }
    ...
    async performType(page, selector, value) {
        ...
        if (this.humanization.enabled) {
            for (const char of value) {
                ...
                await page.waitForTimeout(mathUtils.randomInRange(
    ...
    async performClickAt(page, x, y, clickType = 'single', duration = 500) {
        ...
        if (typeof finalX !== 'number' || typeof finalY !== 'number') {
            throw new Error(`clickAt requires numeric coordinates. Got x: ${typeof finalX}, y: ${typeof finalY}`);
        }
    ...
    async performDrag(page, source, target, duration = 500) {
        logger.info(`Dragging from ${JSON.stringify(source)} to ${JSON.stringify(target)}`);
    ...
    async performMultiSelect(page, items, mode = 'add') {
        ...
        if (mode === 'range' && items.length >= 2) {
            ...
            const endCoords = await this._resolveCoords(page, items[items.length - 1]);
