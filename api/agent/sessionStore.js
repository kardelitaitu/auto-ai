...
class SessionStore {
    ...
    async _initDatabase() {
        try {
            ...
            logger.warn('[SessionStore] Database not available, using in-memory fallback:', error.message);
            this.db = null;
            this.enabled = false;
    ...
    recordAction(url, selector, actionType, success) {
        ...
        if (this.db) {
            try {
                ...
                `);
                stmt.run(urlPattern, selector, actionType, success ? 1 : 0, success ? 0 : 1);
            } catch (error) {
                logger.error('[SessionStore] Failed to record action:', error.message);
            }
        } else {
            this.memoryStore.actionPatterns.push({ urlPattern, selector, actionType, success });
        }
    }
    ...
    getActionSuccessRate(url, selector, actionType) {
        ...
        if (this.db) {
            try {
                ...
                logger.error('[SessionStore] Failed to get success rate:', error.message);
            }
        }
    ...
    learnSelector(url, description, selector, success) {
        ...
        if (this.db) {
            ...
            this.memoryStore.learnedSelectors.push({ urlPattern, description, selector, success });
        }
    }
        ...
        if (this.db) {
            try {
                ...
                if (row && row.confidence > 0.6) {
                    logger.debug(`[SessionStore] Found learned selector: ${row.selector} (confidence: ${row.confidence})`);
                    return row.selector;
                }
            } catch (error) {
                logger.error('[SessionStore] Failed to get best selector:', error.message);
            }
        }
    ...
    getStats() {
        ...
        if (this.db) {
            try {
                ...
                const patternCount = this.db.prepare('SELECT COUNT(*) as count FROM action_patterns').get().count;
                const selectorCount = this.db.prepare('SELECT COUNT(*) as count FROM learned_selectors').get().count;
    ...
    clear() {
        if (this.db) {
            try {
                this.db.exec('DELETE FROM sessions; DELETE FROM action_patterns; DELETE FROM learned_selectors;');
