...
class ResponseCache {
    ...
    get(context) {
        ...
        if (cached && !this._isExpired(cached)) {
            logger.debug(`[ResponseCache] Cache hit for key: ${key.substring(0, 50)}...`);
        ...
        if (similar) {
            logger.debug(`[ResponseCache] Semantic cache hit (similarity: ${similar.similarity.toFixed(2)})`);
            return similar.response;
        }

        logger.debug(`[ResponseCache] Cache miss for key: ${key.substring(0, 50)}...`);
        return null;
    }
    ...
    set(context, response, ttl = this.defaultTTL) {
        ...
        logger.debug(`[ResponseCache] Cached response for key: ${key.substring(0, 50)}...`);
    }
    ...
    setSimilarityThreshold(threshold) {
        ...
        logger.info(`[ResponseCache] Similarity threshold set to ${this.similarityThreshold}`);
    }
}
