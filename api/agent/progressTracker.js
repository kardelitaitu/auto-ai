...
class ProgressTracker {
    ...
    recordStep(stepNumber, action, result) {
        ...
        logger.debug(`[ProgressTracker] Step ${stepNumber}: ${action.action} - ${result.success ? 'SUCCESS' : 'FAILED'}`);
    }
    ...
    recordLlmCall(success, duration) {
        ...
        this._emit('llm:call', {
            ...
            failureRate: this.metrics.llmCalls > 0 ? this.metrics.llmFailures / this.metrics.llmCalls : 0,
            timestamp: Date.now(),
        });
    }
    ...
    recordStuck(stepNumber, reason) {
        ...
        logger.warn(`[ProgressTracker] Agent stuck at step ${stepNumber}: ${reason}`);
    }
    ...
    completeSession(success, reason) {
        ...
        logger.info(`[ProgressTracker] Session ${success ? 'COMPLETED' : 'FAILED'}: ${reason} (${duration}ms)`);
    }
    ...
    getMetrics() {
        return {
            ...
            successRate: this.metrics.actionsAttempted > 0
                ? this.metrics.actionsSucceeded / this.metrics.actionsAttempted
                : 0,
            llmFailureRate: this.metrics.llmCalls > 0
