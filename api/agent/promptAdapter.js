...
class PromptAdapter {
    constructor() {
        this.pageTypes = {
            form: {
                keywords: ['input', 'textarea', 'select', 'button', 'submit', 'form', 'login', 'register', 'signup'],
                rules: [
                ...
                examples: [
                    { action: 'type', selector: '#email', value: 'user@example.com', rationale: 'Fill email field' },
                    { action: 'click', selector: '#submit-btn', rationale: 'Submit the form' },
                ],
            },
            navigation: {
                keywords: ['nav', 'menu', 'sidebar', 'header', 'footer', 'link', 'breadcrumb'],
                rules: [
                ...
                examples: [
                    { action: 'click', selector: 'nav a[href="/products"]', rationale: 'Navigate to products page' },
                    { action: 'verify', description: 'Check if products page loaded', rationale: 'Confirm navigation' },
                ],
            },
            content: {
                keywords: ['article', 'post', 'content', 'text', 'paragraph', 'heading', 'image'],
                rules: [
                ...
                examples: [
                    ...
                    { action: 'click', selector: '.read-more', rationale: 'Open full article' },
                ],
            },
            game: {
                keywords: ['game', 'canvas', 'score', 'level', 'player', 'unit', 'build', 'train'],
                rules: [
                ...
                examples: [
                    { action: 'clickAt', x: 450, y: 320, rationale: 'Click on game unit' },
                    { action: 'wait', value: '1000', rationale: 'Wait for animation' },
                ],
            },
            social: {
                keywords: ['tweet', 'post', 'share', 'like', 'follow', 'comment', 'reply', 'retweet'],
                rules: [
                ...
                examples: [
                    { action: 'type', selector: '.tweet-box', value: 'Hello world!', rationale: 'Compose tweet' },
                    { action: 'click', selector: '.tweet-btn', rationale: 'Publish tweet' },
                ],
            },
            ecommerce: {
                keywords: ['product', 'cart', 'checkout', 'price', 'buy', 'add to cart', 'order'],
                rules: [
                ...
                examples: [
                    { action: 'click', selector: '.add-to-cart', rationale: 'Add product to cart' },
                    { action: 'click', selector: '.checkout-btn', rationale: 'Proceed to checkout' },
                ],
            },
        };
    }
    ...
    detectPageType(axTree, url) {
        try {
            ...
            logger.info(`[PromptAdapter] Detected page type: ${detectedType} (score: ${maxScore})`);
            return detectedType;
        } catch (error) {
    ...
    _generateGoalTips(goal, _pageType) {
        ...
        if (goalLower.includes('login') || goalLower.includes('sign in')) {
            ...
            tips += '- Verify login succeeded by checking for user profile or dashboard\n';
        }
        ...
        if (goalLower.includes('buy') || goalLower.includes('purchase') || goalLower.includes('order')) {
        ...
        // Navigation goals
        if (goalLower.includes('navigate') || goalLower.includes('go to') || goalLower.includes('open')) {
    ...
    getContextualExamples(pageType) {
        ...
        if (!config) {
            ...
            return [
                ...
                { action: 'type', selector: 'input', value: 'text', rationale: 'Type text' },
    ...
    enhancePrompt(basePrompt, pageType, goal, axTree) {
        const contextualPrompt = this.generateContextualPrompt(pageType, goal, axTree);
