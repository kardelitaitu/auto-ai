...
class ContextCompressor {
    ...
    compressAXTree(tree, _maxTokens = this.maxTokens) {
        try {
            ...
            logger.debug(`[ContextCompressor] Compressed AXTree: ${compressed.length} chars`);
            ...
            logger.error('[ContextCompressor] Failed to compress AXTree:', error.message);
    ...
    _extractForms(tree, depth = 0) {
        ...
        if (tree.role === 'form' || tree.role === 'textbox' || tree.role === 'combobox') {
            forms.push({
                role: tree.role,
                name: tree.name || '',
                selector: tree.selector || '',
    ...
    _extractNavigation(tree, depth = 0) {
        ...
        if (tree.role === 'navigation' || tree.role === 'menubar' || tree.role === 'menu') {
            navElements.push({
                role: tree.role,
                name: tree.name || '',
    ...
    describeScreenshot(screenshot) {
        ...
        const size = typeof screenshot === 'string'
            ? screenshot.length
            : screenshot.length;
    ...
    getStats(compressionResult) {
        return {
            ...
            savingsPercent: compressionResult.originalSize > 0
