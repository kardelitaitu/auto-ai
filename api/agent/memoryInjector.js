...
class MemoryInjector {
    ...
    async injectMemory(context) {
        try {
            ...
            let memory = '\n## 🧠 Learned Patterns\n';
            memory += 'Based on previous sessions, here are patterns that worked:\n\n';
    ...
    async _getRelevantPatterns(context) {
        ...
        const filteredPatterns = uniquePatterns.filter(p => p.confidence >= this.minConfidence);
    ...
    _extractGoalType(goal) {
        ...
        if (goalLower.includes('login') || goalLower.includes('sign in')) return 'login';
        if (goalLower.includes('search') || goalLower.includes('find')) return 'search';
        if (goalLower.includes('buy') || goalLower.includes('purchase')) return 'purchase';
