...
describe('proxy-agent.js', () => {
  ...
  describe('createProxyAgent', () => {
    ...
    it('should decode URL-encoded credentials', () => {
      const agent = createProxyAgent('http://user%40domain:pass%21@proxy.example.com:8080');
  ...
  describe('getAgent', () => {
    it('should create HttpsProxyAgent with auth', async () => {
      ...
      expect(mocks.HttpsProxyAgent).toHaveBeenCalledWith('http://user:pass@proxy.example.com:8080');
    ...
    it('should return null and log warning if HttpsProxyAgent creation fails', async () => {
      ...
      expect(mocks.logger.warn).toHaveBeenCalledWith(expect.stringContaining('Failed to create proxy agent'));
    });
  });

  describe('fetchWithProxy', () => {
    it('should use fetch without agent if no proxyUrl provided', async () => {
      await ProxyAgent.fetchWithProxy('https://example.com', { method: 'GET' }, null);
      
      expect(mocks.fetch).toHaveBeenCalledWith('https://example.com', { method: 'GET' });
    ...
    it('should use fetch with agent if proxyUrl provided', async () => {
      ...
      expect(mocks.fetch).toHaveBeenCalledWith('https://example.com', expect.objectContaining({
        method: 'GET',
    ...
    it('should fall back to direct connection if agent creation fails', async () => {
      ...
      expect(mocks.logger.warn).toHaveBeenCalledWith(expect.stringContaining('Falling back to direct connection'));
    });
  });
});
