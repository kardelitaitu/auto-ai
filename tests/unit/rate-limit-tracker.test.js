...
describe('rate-limit-tracker.js', () => {
  ...
  describe('checkKey', () => {
    it('should return cached data if within cache duration', async () => {
      ...
      tracker.cache.set('test-key', { data: cachedData, timestamp: Date.now() });
    ...
    it('should fetch new data if cache is expired', async () => {
      const apiData = { data: { limit_remaining: 50, is_free_tier: false, usage_daily: 10 } };
      global.fetch.mockResolvedValue({
        ok: true,
        json: vi.fn().mockResolvedValue(apiData)
      });

      tracker.cache.set('test-key', { data: {}, timestamp: Date.now() - 70000 });
  ...
  describe('_fetchKeyInfo', () => {
    it('should throw on non-ok response status', async () => {
      ...
      await expect(tracker._fetchKeyInfo('test-key')).rejects.toThrow('HTTP 429');
    });
    ...
    it('should throw on JSON parse error', async () => {
      ...
      await expect(tracker._fetchKeyInfo('test-key')).rejects.toThrow('Invalid JSON');
    });
  ...
  describe('getRequestRate', () => {
    ...
    it('should exclude requests exactly at window boundary (strict less than)', () => {
      ...
      tracker.requestHistory.set('test-key', {
        requests: [
          { timestamp: now - 60000, model: 'gpt-4' }
        ],
        total: 1
      });
    ...
    it('should exclude requests just outside window', () => {
      ...
      tracker.requestHistory.set('test-key', {
        requests: [
          { timestamp: now - 60001, model: 'gpt-4' }
        ],
        total: 1
      });
  ...
  describe('refreshKey', () => {
    it('should invalidate cache and fetch new data', async () => {
      tracker.cache.set('test-key', { data: { old: true }, timestamp: Date.now() });
  ...
  describe('getAllKeyStatus', () => {
    it('should return status for multiple keys', async () => {
      tracker.cache.set('key1', {
        data: { data: { limit_remaining: 100, is_free_tier: false, usage_daily: 10 } },
        timestamp: Date.now()
      });
      tracker.cache.set('key2', {
        data: { data: { limit_remaining: 50, is_free_tier: true, usage_daily: 5 } },
        timestamp: Date.now()
      });
