...
class MockConfigService {
  ...
  async getTwitterActivity() {
    ...
    return this._settings?.twitter?.activity || {
      defaultCycles: 10,
      defaultMinDuration: 300,
      defaultMaxDuration: 540
    };
  ...
  async getTiming() {
    if (!this._initialized) await this.init();
    return this._settings?.twitter?.timing || {
      warmupMin: 2000,
      warmupMax: 15000,
      scrollMin: 300,
      scrollMax: 700,
  ...
  async getAIReplyConfig() {
    if (!this._initialized) await this.init();
    return this._settings?.ai?.reply || {
      probability: 0.5,
      maxRetries: 2,
      timeout: 30000
    };
...
describe('ConfigService Integration Scenarios', () => {
  ...
  describe('Full configuration loading', () => {
    ...
    it('should have all engagement types in limits', async () => {
      ...
      const limits = await config.getEngagementLimits();
      const requiredTypes = ['replies', 'retweets', 'quotes', 'likes', 'follows', 'bookmarks'];
    ...
    it('should have all timing types in config', async () => {
      ...
      const timing = await config.getTiming();
      const requiredTypes = ['warmupMin', 'warmupMax', 'scrollMin', 'scrollMax', 'globalScrollMultiplier'];
