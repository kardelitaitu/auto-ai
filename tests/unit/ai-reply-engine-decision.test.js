...
describe('ai-reply-engine/decision', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await import('../../utils/ai-reply-engine/decision.js');
    shouldReply = module.shouldReply;
    
    const engineModule = await import('../../utils/ai-reply-engine/index.js');
    AIReplyEngine = engineModule.default;
    
    const mathModule = await import('../../utils/mathUtils.js');
    mathUtils = mathModule.mathUtils;
    
    const sentimentModule = await import('../../utils/sentiment-service.js');
    sentimentService = sentimentModule.sentimentService;

    mockEngine = new AIReplyEngine({ processRequest: vi.fn(), sessionId: 'test' }, { replyProbability: 1, maxRetries: 1 });
  });
  ...
  describe('shouldReply', () => {
    ...
    it('should skip negative content', async () => {
      ...
      sentimentService.analyze.mockReturnValue({ ...baseSentiment, isNegative: true, score: 0.5 });
    ...
    it('should proceed when all checks pass', async () => {
      ...
      mockEngine.generateReply = vi.fn().mockResolvedValue({ success: true, reply: 'Great reply!' });
      mockEngine.validateReply = vi.fn().mockReturnValue({ valid: true, reason: 'passed' });
      mockEngine.applySafetyFilters = vi.fn().mockReturnValue({ safe: true, reason: 'passed' });
      
      const result = await shouldReply(mockEngine, 'This is a good tweet', 'user');
  ...
  describe('applySafetyFilters (via engine)', () => {
    ...
    it('should reject text with excluded keywords', () => {
      const result = mockEngine.applySafetyFilters('I love politics and elections');
      expect(result.safe).toBe(false);
      expect(result.reason).toContain('excluded_keyword');
    });

    it('should reject text with excessive caps', () => {
      const result = mockEngine.applySafetyFilters('THIS IS ALL CAPS AND VERY LONG TEXT THAT SHOULD BE BLOCKED');
      expect(result.safe).toBe(false);
    ...
    it('should accept valid text', () => {
      const result = mockEngine.applySafetyFilters('This is a normal tweet about technology');
