...
describe('OrchestratorV2 Unit Tests', () => {
  ...
  describe('Group Timeout Logic', () => {
    ...
    it('should not detect timeout when group not started', () => {
      ...
      const isExceeded = currentGroupStartTime ? (Date.now() - currentGroupStartTime) >= 600000 : false;
  ...
  describe('Worker Health Logic', () => {
    it('should calculate worker health status', () => {
      ...
      const health = {
        ...
        stuck: workers.filter(w => w.status === 'busy' && w.occupiedAt && (now - w.occupiedAt) > stuckWorkerThresholdMs).length,
      };
    ...
    it('should detect stuck workers', () => {
      ...
      const stuck = workers.filter(w => w.status === 'busy' && w.occupiedAt && (now - w.occupiedAt) > stuckWorkerThresholdMs).length;
  ...
  describe('Queue Status', () => {
    it('should return correct queue status', () => {
      ...
      activeTasks.set('task-1', { startTime: Date.now(), task: { taskName: 'test' } });
  ...
  describe('Task Group Parsing', () => {
    ...
    it('should handle complex chain', () => {
      const args = 'pageview cookiebot then cookiebot cookiebot then api-twitteractivity'.split(' ');
      const taskGroups = [];
      let currentGroup = [];
  ...
  describe('Wait For Tasks Complete Logic', () => {
    it('should resolve immediately when queue empty and not processing', () => {
      ...
      const shouldResolveImmediately = taskQueue.length === 0 && !isProcessingTasks;
