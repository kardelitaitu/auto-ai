...
vi.mock('../../utils/twitterAgent.js', () => {
  const MockTwitterAgent = vi.fn(function (page, initialProfile, logger) {
    ...
    this.log = (msg) => { if (logger) logger.info(msg); };
    this.start = vi.fn();
    this.stop = vi.fn();
...
describe('AITwitterAgent', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should initialize DiveQueue with correct options', () => {
      expect(DiveQueue).toHaveBeenCalledWith(expect.objectContaining({
        maxConcurrent: 1,
        maxQueueSize: 30,
        replies: 10,
        likes: 20
    ...
    it('should initialize engagement tracker', () => {
      expect(engagementLimits.createEngagementTracker).toHaveBeenCalledWith(mockOptions.engagementLimits);
    });
  });
  ...
  describe('handleAIReply', () => {
    ...
    it('skips when sentiment is negative', async () => {
      ...
      await agent.handleAIReply('bad content', 'user1', { url: 'https://x.com/1' });
    ...
    it('runs pre-validated reply path and executes reply', async () => {
      ...
      await agent.handleAIReply('hello', 'user1', { url: 'https://x.com/1', action: 'reply' });
  ...
  describe('handleLike', () => {
    ...
    it('likes tweet and records engagement', async () => {
      ...
      const likeButton = {
        boundingBox: vi.fn().mockResolvedValue({ x: 10, y: 10, width: 20, height: 20 }),
      ...
      expect(agent.humanClick).toHaveBeenCalledWith(likeButton, 'Like Button', { precision: 'high' });
  ...
  describe('handleAIQuote', () => {
    const baseSentiment = {
      ...
      composite: { riskLevel: 'low', conversationType: 'neutral', engagementStyle: 'neutral' }
    };
    ...
    it('executes quote flow when quote is generated', async () => {
      ...
      await agent.handleAIQuote('tweet', 'user1', { url: 'https://x.com/1', action: 'quote' });

      expect(agent.quoteEngine.executeQuote).toHaveBeenCalledWith(mockPage, 'quote text');
