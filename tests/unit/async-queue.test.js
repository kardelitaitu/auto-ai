...
describe('Coverage Gap Tests', () => {
  describe('AsyncQueue - add() method', () => {
    ...
    it('should use custom timeout from options', async () => {
      ...
      const result = await queue.add(slowTask, { timeout: 100, name: 'timeout-test' });
  ...
  describe('AsyncQueue - _processQueue() and _processItems()', () => {
    ...
    it('should process items in priority order', async () => {
      ...
      queue.queue.push({
        id: '1', taskFn: task1, priority: 1, taskName: 'p1', resolve: () => {}, reject: () => {}, enqueueTime: Date.now()
      });
      queue.queue.push({
        id: '2', taskFn: task2, priority: 5, taskName: 'p5', resolve: () => {}, reject: () => {}, enqueueTime: Date.now()
      });
      queue.queue.push({
        id: '3', taskFn: task3, priority: 10, taskName: 'p10', resolve: () => {}, reject: () => {}, enqueueTime: Date.now()
      });
  ...
  describe('AsyncQueue - clear() and health methods', () => {
    ...
    it('should clear queue and return dropped count', async () => {
      const task = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 1000)));
  ...
  describe('DiveQueue - addDive() with fallback', () => {
    ...
    it('should handle both primary and fallback failing', async () => {
      const primaryFn = vi.fn().mockReturnValue(Promise.reject(new Error('Primary failed')));
      const fallbackFn = vi.fn().mockReturnValue(Promise.reject(new Error('Fallback also failed')));
