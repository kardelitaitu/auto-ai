...
describe('api/core/plugins - Enhanced', () => {
  ...
  describe('PluginManager - Constructor', () => {
    it('should throw if events not provided', () => {
      expect(() => new PluginManager()).toThrow('PluginManager requires an APIEvents instance');
      expect(() => new PluginManager(null)).toThrow('PluginManager requires an APIEvents instance');
      expect(() => new PluginManager(undefined)).toThrow('PluginManager requires an APIEvents instance');
    });
  ...
  describe('PluginManager - Register', () => {
    ...
    it('should throw on invalid plugin object', () => {
      ...
      expect(() => pluginManager.register(undefined)).toThrow('must be an object');
    });

    it('should throw on missing name', () => {
      expect(() => pluginManager.register({ name: '' })).toThrow('must have a name');
      expect(() => pluginManager.register({ name: null })).toThrow('must have a name');
      expect(() => pluginManager.register({ hooks: {} })).toThrow('must have a name');
    });
    ...
    it('should throw on invalid hooks type', () => {
      ...
      expect(() => pluginManager.register(plugin)).toThrow('hooks must be an object');
    });

    it('should call init for legacy plugin', () => {
      const initFn = vi.fn();
      pluginManager.register({ name: 'init-test', init: initFn, config: { test: true } });
  ...
  describe('PluginManager - Hook Binding', () => {
    it('should bind valid hooks to events', () => {
      ...
      expect(events.on).toHaveBeenCalledWith('before:click', expect.any(Function));
    });
  ...
  describe('PluginManager - Unregister', () => {
    ...
    it('should call onDisable and onUnload for BasePlugin on unregister', async () => {
      ...
      class FullLifecyclePlugin extends BasePlugin {
        constructor() { super('full-lifecycle'); }
