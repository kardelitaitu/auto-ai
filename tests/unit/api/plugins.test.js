...
describe('api/core/plugins', () => {
  ...
  describe('Constructor', () => {
    it('should throw if events is missing', () => {
      expect(() => new PluginManager(null)).toThrow('PluginManager requires an APIEvents instance');
    });
  });
  ...
  describe('PluginManager', () => {
    ...
    it('should throw when registering invalid plugin', () => {
      expect(() => pluginManager.register(null)).toThrow('Plugin must be an object');
      expect(() => pluginManager.register('not an object')).toThrow('Plugin must be an object');
      expect(() => pluginManager.register({})).toThrow('Plugin must have a name string');
    });

    it('should throw if plugin already registered', () => {
      pluginManager.register({ name: 'dup' });
      expect(() => pluginManager.register({ name: 'dup' })).toThrow('Plugin "dup" is already registered');
    });
  ...
  describe('loadBuiltinPlugins detailed', () => {
    it('should handle registration errors', () => {
      ...
      const registerSpy = vi.spyOn(manager, 'register').mockImplementation(() => {
  ...
  describe('register - edge cases for coverage', () => {
    it('should handle plugin with getHooks returning truthy', () => {
      ...
      pluginManager.register(plugin);
      expect(mockEvents.on).toHaveBeenCalledWith('before:click', expect.any(Function));
    });
  ...
  describe('register - Invalid hooks (not object)', () => {
    it('should throw error for non-object hooks', () => {
      ...
      expect(() => pluginManager.register(invalidPlugin)).toThrow('Plugin hooks must be an object');
    });
  });
  ...
  describe('register - Unknown hooks', () => {
    it('should log warning for unknown hooks', () => {
      ...
      expect(pluginManager.get('unknown-hook-plugin').instance).toBe(unknownHookPlugin);
    });
  });
  ...
  describe('disable - when already disabled', () => {
    ...
    describe('register - BasePlugin with falsy getHooks', () => {
      it('should handle falsy hooks from getHooks', () => {
        class NoHooksPlugin extends BasePlugin {
          constructor() { super('no-hooks'); }
          getHooks() { return null; }
        }
    ...
    describe('listEnabled', () => {
      it('should list only enabled plugins', () => {
        class PluginA extends BasePlugin { constructor() { super('plugin-a'); } }
        class PluginB extends BasePlugin { constructor() { super('plugin-b'); } }
    ...
    describe('clear', () => {
      it('should clear all plugins', () => {
        pluginManager.register(new TestPlugin());
        class AnotherPlugin extends BasePlugin { constructor() { super('another'); } }
    ...
    describe('isEnabled', () => {
      it('should check enabled status', () => {
        class TestPlugin2 extends BasePlugin {
          constructor() { super('test-plugin2'); }
        }
    ...
    describe('Hook Binding and Event Emission', () => {
      it('should call registered hook handler when event is emitted', async () => {
        ...
        expect(mockEvents.on).toHaveBeenCalledWith('before:click', expect.any(Function));
        const wrappedHandler = mockEvents.on.mock.calls.find(call => call[0] === 'before:click')[1];
      ...
      it('should NOT call handler if plugin is disabled', async () => {
        const handler = vi.fn();
        pluginManager.register({ name: 'disabled-plugin', hooks: { 'after:click': handler } });
        pluginManager.disable('disabled-plugin');

        const wrappedHandler = mockEvents.on.mock.calls.find(call => call[0] === 'after:click')[1];
      ...
      it('should handle errors in hook handlers', async () => {
        ...
        pluginManager.register({ name: 'error-plugin', hooks: { 'on:action:error': handler } });

        const wrappedHandler = mockEvents.on.mock.calls.find(call => call[0] === 'on:action:error')[1];
    ...
    describe('Unregistering with Hooks', () => {
      ...
      it('should only remove its own hook bindings when unregistering', () => {
        const pluginA = { name: 'plugin-a', hooks: { 'before:click': () => { } } };
        const pluginB = { name: 'plugin-b', hooks: { 'before:click': () => { } } };
