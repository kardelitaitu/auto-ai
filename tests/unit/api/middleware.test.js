...
describe('api/core/middleware.js', () => {
  ...
  describe('createPipeline', () => {
    it('should execute middleware in order', async () => {
      const order = [];
      const m1 = async (ctx, next) => { order.push(1); return await next(); };
      const m2 = async (ctx, next) => { order.push(2); return await next(); };
      const action = async (ctx) => { order.push(3); return 'done'; };
  ...
  describe('loggingMiddleware', () => {
    ...
    it('should respect disabled logging options', async () => {
      const m = loggingMiddleware({ logArgs: false, logResult: false, logTime: false });
      const next = vi.fn().mockResolvedValue('ok');
  ...
  describe('validationMiddleware', () => {
    it('should validate DOM actions', async () => {
      ...
      await expect(m({ action: 'click', selector: null }, next)).rejects.toThrow('Selector is required');
      await expect(m({ action: 'click', selector: ' ' }, next)).rejects.toThrow('Empty selector');
      await expect(m({ action: 'click', selector: 123 }, next)).rejects.toThrow('Expected string or Locator');
    ...
    it('should validate timeoutMs', async () => {
      const m = validationMiddleware();
      await expect(m({ action: 'wait', options: { timeoutMs: -1 } }, vi.fn())).rejects.toThrow('timeoutMs must be non-negative');
    });

    it('should validate maxRetries', async () => {
      const m = validationMiddleware();
      await expect(m({ action: 'click', selector: '#a', options: { maxRetries: -1 } }, vi.fn())).rejects.toThrow('maxRetries must be non-negative');
    });
  });
  ...
  describe('retryMiddleware', () => {
    ...
    it('should respect shouldRetry condition', async () => {
      const m = retryMiddleware({ maxRetries: 5, shouldRetry: (e) => e.message === 'retry me' });
  ...
  describe('recoveryMiddleware', () => {
    ...
    it('should bypass recovery if disabled', async () => {
      const m = recoveryMiddleware({ scrollOnDetached: false, retryOnObscured: false });
  ...
  describe('metricsMiddleware', () => {
    it('should emit metrics on success', async () => {
      ...
      expect(mockEvents.emitSafe).toHaveBeenCalledWith('on:metrics', expect.objectContaining({
        action: 'test',
        success: true
      }));
    });
  ...
  describe('rateLimitMiddleware', () => {
    ...
    it('should throttle actions exceeding limit', async () => {
      ...
      const p3 = m({}, next);
      // We can't easily test the exact wait without fake timers or longer windows
      // but we can ensure it eventually proceeds
      await p3;
