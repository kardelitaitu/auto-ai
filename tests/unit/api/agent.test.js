...
import { screenshot, injectAnnotations, removeAnnotations } from '../../../api/agent/vision.js';
...
describe('api/agent/observer.js - processDomElements', () => {
  it('should process button elements', () => {
    const mockElement = {
      ...
      setAttribute: vi.fn(),
      getBoundingClientRect: vi.fn().mockReturnValue({ width: 100, height: 50, top: 10, left: 10 })
    };
  ...
  it('should skip hidden elements', () => {
    ...
    const mockDoc = {
      querySelectorAll: vi.fn().mockReturnValue([mockElement]),
      defaultView: {
        getComputedStyle: vi.fn().mockReturnValue({ display: 'none', visibility: 'visible', opacity: '1' })
      }
    };
  ...
  it('should assign sequential IDs to elements', () => {
    const mockElements = [
      { tagName: 'BUTTON', innerText: 'One', getAttribute: vi.fn().mockReturnValue(null), setAttribute: vi.fn(), getBoundingClientRect: vi.fn().mockReturnValue({ width: 100, height: 50 }) },
      { tagName: 'BUTTON', innerText: 'Two', getAttribute: vi.fn().mockReturnValue(null), setAttribute: vi.fn(), getBoundingClientRect: vi.fn().mockReturnValue({ width: 100, height: 50 }) },
      { tagName: 'BUTTON', innerText: 'Three', getAttribute: vi.fn().mockReturnValue(null), setAttribute: vi.fn(), getBoundingClientRect: vi.fn().mockReturnValue({ width: 100, height: 50 }) }
    ];
...
describe('api/agent/finder.js', () => {
  ...
  describe('find', () => {
    ...
    it('should return inclusion match when exact fails', async () => {
      const mockElements = [
        { id: 1, role: 'button', label: 'Submit Form' }
      ];
...
describe('api/agent/executor.js', () => {
  ...
  describe('doAction', () => {
    ...
    it('should click element by label inclusion', async () => {
      const mockElements = [
        { id: 1, role: 'button', label: 'Submit Form', selector: '#submit-btn' }
      ];
    ...
    it('should throw when element not found by ID', async () => {
      ...
      await expect(doAction('click', 999)).rejects.toThrow('not found in current view');
    });
    ...
    it('should throw when element not found by label', async () => {
      ...
      await expect(doAction('click', 'Nonexistent')).rejects.toThrow('not found in current view');
    });
