...
describe('APIEvents', () => {
  ...
  describe('emitAsync', () => {
    ...
    it('should handle rejected promises and return null', async () => {
      ...
      events.on('test', async () => { throw new Error('fail'); });
    ...
    it('should handle rejected promises without message', async () => {
      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => { });
      events.on('test', async () => { throw 'no message'; });

      const results = await events.emitAsync('test');
      expect(results).toEqual([null]);
      expect(consoleSpy).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('no message'));
      consoleSpy.mockRestore();
    });
  });
  ...
  describe('emitSync', () => {
    ...
    it('should catch errors and return null', () => {
      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => { });
      events.on('test', () => { throw new Error('sync fail'); });
  ...
  describe('createHookWrapper', () => {
    ...
    it('should obey emitBefore/emitAfter options', async () => {
      const spy = vi.spyOn(apiEvents, 'emitSafe');
      const wrapped = createHookWrapper('test', async () => { }, { emitBefore: false, emitAfter: false });
    ...
    it('should emit on:action:error on failure', async () => {
      ...
      const wrapped = createHookWrapper('test', async () => { throw err; });

      await expect(wrapped('arg')).rejects.toThrow(err);
      expect(spy).toHaveBeenCalledWith('on:action:error', { action: 'test', error: err, args: ['arg'] });
      spy.mockRestore();
    });
  });
  ...
  describe('withErrorHook', () => {
    ...
    it('should emit on:error and on:detection on failure', async () => {
      ...
      await expect(withErrorHook('ctx', async () => { throw err; })).rejects.toThrow(err);

      expect(spy).toHaveBeenCalledWith('on:error', { context: 'ctx', error: err });
      expect(spy).toHaveBeenCalledWith('on:detection', { type: 'error', details: 'boom' });
