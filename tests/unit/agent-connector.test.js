...
vi.mock('../../core/local-client.js', () => {
    return {
        default: vi.fn(function() {
            return {
                processRequest: vi.fn().mockResolvedValue({ success: true, content: 'local response', metadata: { routedTo: 'local' } }),
                sendRequest: vi.fn().mockResolvedValue({ success: true, content: 'local response', metadata: { routedTo: 'local' } }),
...
vi.mock('../../core/cloud-client.js', () => {
    return {
        default: vi.fn(function() {
            return {
                processRequest: vi.fn().mockResolvedValue({ success: true, content: 'cloud response', metadata: { routedTo: 'cloud' } }),
                sendRequest: vi.fn().mockResolvedValue({ success: true, content: 'cloud response', metadata: { routedTo: 'cloud' } }),
                getStats: vi.fn().mockReturnValue({}),
...
vi.mock('../../core/vision-interpreter.js', () => {
    return {
        default: vi.fn(function() {
            return {
                analyze: vi.fn().mockResolvedValue({ success: true, content: 'vision response', metadata: { routedTo: 'vision' } }),
...
describe('AgentConnector', () => {
    ...
    describe('Initialization', () => {
        it('should initialize all dependencies', () => {
            ...
            expect(connector.stats).toEqual(expect.objectContaining({
                totalRequests: 0,
    ...
    describe('Routing Logic', () => {
        ...
        it('should route vision action to handleVisionRequest', async () => {
            ...
            visionInterpreterMock.parseResponse.mockReturnValue({ success: true, data: { action: 'click' } });
    ...
    describe('handleGenerateReply', () => {
        ...
        it('should fallback to cloud if local fails', async () => {
            ...
            cloudClientMock.sendRequest.mockResolvedValue({ success: true, content: 'cloud reply' });
    ...
    describe('Coverage Edge Cases', () => {
        ...
        it('should handle vision parsing failure', async () => {
            ...
            visionInterpreterMock.parseResponse.mockReturnValue({ success: false, error: 'Parse error' });
