...
import IxbrowserDiscover from '../../../connectors/discovery/ixbrowser.js';
  ...
  describe('discover()', () => {
    ...
    it('should handle multiple profiles', async () => {
      mockApiHandler.post.mockResolvedValue({
        error: { code: 0, message: 'Success' },
        data: [
          { profile_id: '1', ws: 'ws://1:1/devtools', debugging_address: '1:1', debugging_port: 1 },
          { profile_id: '2', ws: 'ws://2:2/devtools', debugging_address: '2:2', debugging_port: 2 },
          { profile_id: '3', ws: 'ws://3:3/devtools', debugging_address: '3:3', debugging_port: 3 },
        ]
      });
      ...
      expect(result.map(p => p.id)).toEqual(['ixbrowser-1', 'ixbrowser-2', 'ixbrowser-3']);
    });
  });
});
