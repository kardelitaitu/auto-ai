...
import UndetectableDiscover from '../../../connectors/discovery/undetectable.js';
...
describe('UndetectableDiscover', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const apiHandler = await import('../../../utils/apiHandler.js');
  ...
  describe('Constructor', () => {
    ...
    it('should add trailing slash to API URL if missing', async () => {
      const { getEnv } = await import('../../../utils/envLoader.js');
  ...
  describe('discover()', () => {
    ...
    it('should handle multiple profiles', async () => {
      mockApiHandler.get.mockResolvedValue({
        ...
        msg: 'Success',
        data: {
          'uuid-1': { name: 'Profile 1', websocket_link: 'ws://1.1.1.1:9001/devtools/browser/1' },
          'uuid-2': { name: 'Profile 2', websocket_link: 'ws://2.2.2.2:9002/devtools/browser/2' },
          'uuid-3': { name: 'Profile 3', websocket_link: 'ws://3.3.3.3:9003/devtools/browser/3' },
        }
      });
    ...
    it('should use index for sortNum', async () => {
      mockApiHandler.get.mockResolvedValue({
        ...
        msg: 'Success',
        data: {
          'uuid-a': { name: 'Profile A', websocket_link: 'ws://1.1.1.1:9001/devtools/browser/a' },
          'uuid-b': { name: 'Profile B', websocket_link: 'ws://2.2.2.2:9002/devtools/browser/b' },
        }
      });
