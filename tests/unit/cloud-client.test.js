...
vi.mock('../../utils/free-api-router.js', () => {
    ...
    MockFreeRouter.prototype.getModelsInfo = vi.fn().mockReturnValue({ testedWorking: [], allConfigured: [] });
...
vi.mock('../../utils/free-openrouter-helper.js', () => {
    ...
    MockHelper.prototype.getOptimizedModelList = vi.fn().mockReturnValue({ primary: 'p', fallbacks: [] });
...
describe('CloudClient', () => {
    ...
    describe('Free Router Error Handling', () => {
        ...
        it('should handle invalid JSON from free router', async () => {
            ...
            expect(mocks.logger.debug).toHaveBeenCalledWith(expect.stringContaining('not valid JSON'));
        });
    ...
    describe('Coverage Edge Cases', () => {
        ...
        it('should execute testFreeModels successfully', async () => {
             ...
             // Mock helper instance
               const mockHelper = {
                      testAllModelsInBackground: vi.fn().mockResolvedValue({ testedWorking: ['p'], failed: [] }),
                      isTesting: () => false,
                      isReady: () => false,
        ...
        it('should handle key fallbacks correctly in multi-client', async () => {
            ...
            const spy = vi.spyOn(CloudClient.prototype, '_loadConfig').mockImplementation(async () => {});
            ...
            expect(mocks.logger.info).toHaveBeenCalledWith(
                expect.stringContaining('fallback(s)')
            );
        ...
        it('should handle no providers configured in _loadConfig', async () => {
         ...
         it('should restart tests if cache is stale in testFreeModels', async () => {
              ...
              expect(mocks.logger.info).toHaveBeenCalledWith(
                  expect.stringContaining('Cache stale')
              );
         ...
         it('should reuse shared helper if already exists in _loadConfig', async () => {
             ...
             expect(mocks.logger.info).toHaveBeenCalledWith(
                 expect.stringContaining('Reuse cache')
             );
        ...
        it('should handle missing content in free router response', async () => {
             ...
             const { FreeApiRouter: _FreeApiRouter } = await import('../../utils/free-api-router.js');
             const mockRouter = {
                 processRequest: vi.fn().mockResolvedValue({ success: true, content: null, model: 'test' }),
        ...
        describe('Additional Coverage Gaps', () => {
             ...
             it('should pass custom temperature to free router', async () => {
                  client.freeApiRouter = {
                      ...
                      syncWithHelper: vi.fn(),
                      getModelsInfo: vi.fn().mockReturnValue({ testedWorking: [], allConfigured: [] }),
                      processRequest: vi.fn().mockResolvedValue({ success: true, content: 'response' })
                  };
                  client._loadConfig = vi.fn().mockResolvedValue();
             ...
             it('should return correct status from testFreeModels', async () => {
                  ...
                  const settings = { open_router_free_api: { enabled: true, models: { primary: 'p' } } };
                  const { getSettings } = await import('../../utils/configLoader.js');
                  vi.mocked(getSettings).mockResolvedValue(settings);
             ...
             it('should use optimized fallbacks if available', async () => {
                 ...
                 const mockHelper = {
                     testAllModelsInBackground: vi.fn(),
                     getOptimizedModelList: vi.fn().mockReturnValue({ primary: 'opt-p', fallbacks: ['opt-f'] }),
                     getResults: vi.fn(),
                     isTesting: () => false,
                     updateConfig: vi.fn()
                 };
             ...
             it('should reset stats', () => {
                 ...
                 client.resetStats();
                 expect(client.stats.totalRequests).toBe(0);
                 expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('Statistics reset'));
             });

             it('should throw error in _makeRequest if multi-client is active', async () => {
                 client.multiClient = {};
                 await expect(client._makeRequest({})).rejects.toThrow('_makeRequest called but multi-client is active');
             });
             ...
             it('should log tests running in _loadConfig', async () => {
                 ...
                 expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('Tests already running'));
             });
