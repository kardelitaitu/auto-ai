...
describe('SessionHandler', () => {
    ...
    describe('runSession', () => {
        ...
        it('should set session duration if min/max provided', async () => {
            ...
            handler.isSessionExpired.mockReturnValueOnce(false).mockReturnValueOnce(true);
        ...
        it('should handle theme application failure', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Failed to apply theme'));
        });
        ...
        it('should retry navigation home up to 3 times', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Navigation attempt 1/3 failed'));
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Navigation attempt 2/3 failed'));
        });
        ...
        it('should abort if 3+ consecutive login failures', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Critical: 3+ consecutive login failures'));
        ...
        it('should run loop until cycles complete', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Completed 2 engagement cycles'));
        });
        ...
        it('should handle burst mode transition', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Burst mode ended'));
        });
    });
    ...
    describe('executeEngagementCycle', () => {
        ...
        it('should return if health check fails', async () => {
            handler.performHealthCheck.mockResolvedValue({ healthy: false, reason: 'Test fail' });
            
            await handler.executeEngagementCycle();
            
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Health check failed'));
    ...
    describe('simulateReading', () => {
        ...
        it('should execute reading loop with random behaviors', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Simulating human reading'));
    ...
    describe('simulateFidget', () => {
        it('should handle TEXT_SELECT fidget', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Simulating text select'));
        ...
        it('should handle RANDOM_CLICK fidget', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Simulating random click'));
        ...
        it('should handle SCROLL_JITTER fidget', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Simulating scroll jitter'));
    ...
    describe('simulateFidget', () => {
        it('should handle errors gracefully', async () => {
             ...
             expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('[Fidget] Error: Fidget error'));
        });
    });
    ...
    describe('humanType', () => {
        it('should type text with delays and typos', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Simulating human typing'));
        ...
        it('should handle errors during typing', async () => {
            ...
            await expect(handler.humanType(mockElement, 'hi')).rejects.toThrow('Click failed');
            
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('[Type] Error: Click failed'));
        });
    });
    ...
    describe('postTweet', () => {
        it('should post tweet successfully via button', async () => {
            ...
            mockPage.locator.mockImplementation((selector) => {
                if (selector.includes('SideNav_NewTweet_Button')) return mockComposerBtn;
                if (selector.includes('tweetButton')) return mockPostBtn;
                return { waitFor: vi.fn().mockResolvedValue(), count: vi.fn().mockResolvedValue(0) };
            });
            ...
            expect(handler.safeHumanClick).toHaveBeenCalledWith(mockComposerBtn, 'Composer Button');
            expect(handler.humanType).toHaveBeenCalled();
            expect(handler.safeHumanClick).toHaveBeenCalledWith(mockPostBtn, 'Post Button');
        ...
        it('should return false if posting fails', async () => {
            vi.spyOn(handler, 'humanType').mockRejectedValue(new Error('Typing failed'));
            
            const result = await handler.postTweet('Hello');
            
            expect(result).toBe(false);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Failed to post tweet'));
        });
    });
    ...
    describe('checkLoginState', () => {
        ...
        it('should return false if primary timeline not visible on home', async () => {
             ...
             expect(result).toBe(false);
             expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Suspected not logged in'));
        });

        it('should handle errors during check', async () => {
            mockPage.getByText.mockImplementation(() => { throw new Error('Check error'); });
            
            const result = await handler.checkLoginState();
            
            expect(result).toBe(false);
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('checkLoginState failed'));
        });
    });
});
