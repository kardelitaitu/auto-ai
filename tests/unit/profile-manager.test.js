...
describe('profileManager', () => {
    const mockProfiles = [
        { id: 'fast', type: 'engagement', timings: { scrollPause: { mean: 2000 } }, probabilities: { dive: 50, like: 30 } },
    ...
    describe('Selection Logic (Sync)', () => {
        ...
        it('should throw error for non-existent ID', () => {
            expect(() => profileManager.getById('missing')).toThrow('Profile "missing" not found');
        });
    ...
    describe('Selection Logic (Async)', () => {
        ...
        it('should throw error for non-existent ID async', async () => {
            await expect(profileManager.getByIdAsync('none')).rejects.toThrow('Profile "none" not found');
        });
    ...
    describe('Generation Fallback', () => {
        ...
        it('should return false if generation fails', async () => {
            clearProfiles();
            vi.mocked(fs.existsSync).mockReturnValue(false);
            vi.mocked(execSync).mockImplementationOnce(() => { throw new Error('Exec failed'); });
    ...
    describe('Validation', () => {
        it('should warn on invalid profiles during reload', () => {
            ...
            profileManager.reload();
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Profile validation issues'));
            consoleSpy.mockRestore();
        });

        it('should handle validation failures with specific errors', () => {
            const badProfiles = [
                { id: 'bad', type: 'e', timings: { scrollPause: {} }, probabilities: { dive: 150 } }
            ];
            vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify(badProfiles));
            const consoleSpy = vi.spyOn(console, 'warn').mockImplementation(() => { });
