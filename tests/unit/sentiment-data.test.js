...
} from '../../utils/sentiment-data.js';

import sentimentDataDefault from '../../utils/sentiment-data.js';
...
describe('sentiment-data.js', () => {
    ...
    describe('CONTEXTUAL_PATTERNS', () => {
        ...
        it('should have fakePositivity pattern', () => {
            ...
            expect(CONTEXTUAL_PATTERNS.fakePositivity).toHaveProperty('classification');
        ...
        it('should have actionGate in fakePositivity pattern', () => {
            expect(typeof CONTEXTUAL_PATTERNS.fakePositivity.actionGate).toBe('object');
        });

        it('should have adjustment in fakePositivity', () => {
            expect(CONTEXTUAL_PATTERNS.fakePositivity.adjustment).toBeDefined();
            expect(typeof CONTEXTUAL_PATTERNS.fakePositivity.adjustment).toBe('object');
        });
    });
    ...
    describe('SENTIMENT_THRESHOLDS', () => {
        ...
        it('should have skip thresholds in ascending order', () => {
            expect(SENTIMENT_THRESHOLDS.skipRetweet).toBeLessThan(SENTIMENT_THRESHOLDS.skipLike);
            expect(SENTIMENT_THRESHOLDS.skipLike).toBeLessThan(SENTIMENT_THRESHOLDS.skipReply);
            expect(SENTIMENT_THRESHOLDS.skipQuote).toBeLessThan(SENTIMENT_THRESHOLDS.skipReply);
        });
    });
    ...
    describe('Default Export', () => {
        it('should match all named exports', () => {
            ...
            expect(sentimentDataDefault.CONTEXTUAL_PATTERNS).toBe(CONTEXTUAL_PATTERNS);
            expect(sentimentDataDefault.ACTION_GATES).toBe(ACTION_GATES);
            expect(sentimentDataDefault.PERSONALITY_PROFILES).toBe(PERSONALITY_PROFILES);
            expect(sentimentDataDefault.TOPIC_KEYWORDS).toBe(TOPIC_KEYWORDS);
            expect(sentimentDataDefault.SENTIMENT_THRESHOLDS).toBe(SENTIMENT_THRESHOLDS);
        });
    });
    ...
    describe('Data Integrity', () => {
        it('should not have empty arrays in POSITIVE_LEXICON', () => {
            for (const [category, words] of Object.entries(POSITIVE_LEXICON)) {
                expect(words.length).toBeGreaterThan(0, `Category ${category} should not be empty`);
            }
        });
