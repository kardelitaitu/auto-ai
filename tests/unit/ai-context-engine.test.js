...
const createPageMock = (options = {}) => {
  const page = {
    ...
    evaluate: vi.fn((fn, arg) => {
      ...
      global.document = page._document || { querySelectorAll: () => [], querySelector: () => null, body: { scrollHeight: 1000 } };
      global.window = page._window || { innerHeight: 800, scrollBy: vi.fn(), scrollTo: vi.fn() };
      let result;
      try {
        result = fn(arg);
      } finally {
...
const createElementFactory = (HTMLElementClass) => (text = '', attributes = {}) => {
...
describe('ai-context-engine', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    ({ AIContextEngine } = await import('../../utils/ai-context-engine.js'));
  ...
  it('detects promotional tone', () => {
    const tone = engine.detectTone('limited time offer, check out the link in bio');
  ...
  it('parses numbers and engagement levels', () => {
    ...
    expect(engine.calculateEngagementLevel({ likes: 0, retweets: 0, replies: 0, views: 200000 })).toBe('viral');
    expect(engine.calculateEngagementLevel({ likes: 800, retweets: 200, replies: 5, views: 5000 })).toBe('high');
    expect(engine.calculateEngagementLevel({ likes: 50, retweets: 60, replies: 5, views: 500 })).toBe('medium');
    expect(engine.calculateEngagementLevel({ likes: 7, retweets: 2, replies: 3, views: 200 })).toBe('low');
    expect(engine.calculateEngagementLevel({ likes: 1, retweets: 1, replies: 1, views: 50 })).toBe('minimal');
  });
  ...
  it('detects tweet images and captures screenshots', async () => {
    ...
    const document = {
      querySelector: () => ({
        querySelector: (selector) => (selector.includes('tweetPhoto') ? {} : null)
      })
    };
    ...
    page.$ = vi.fn().mockResolvedValue({ screenshot: vi.fn().mockResolvedValue('jpeg') });
  ...
  it('extracts replies and authors from multiple strategies', async () => {
    ...
    const article = {
      evaluate: vi.fn(async (fn) => fn({ getBoundingClientRect: () => ({ height: 150 }) })),
    ...
    expect(replies.some(r => r.text.includes('Reply') || r.text.includes('Deep reply'))).toBe(true);
  });
  ...
  it('extracts author from visible text context', async () => {
    ...
    const page = createPageMock({ document, HTMLElement: FakeHTMLElement });
    const author = await engine.extractAuthorFromVisibleText(page, 'Some reply');
  ...
  it('skips metrics extraction when disabled', async () => {
    ...
    vi.spyOn(engine, 'analyzeSentiment').mockReturnValue({ overall: 'neutral', score: 0 });
    vi.spyOn(engine, 'detectTone').mockReturnValue({ primary: 'serious', confidence: 0 });
    vi.spyOn(engine, 'classifyConversation').mockReturnValue('general');
    vi.spyOn(engine, 'analyzeReplySentiment').mockReturnValue({ overall: 'neutral', positive: 0, negative: 0 });
    vi.spyOn(engine, 'checkForTweetImage').mockResolvedValue(false);
    const result = await engine.extractEnhancedContext(page, 'url', 'tweet text', 'author');
  ...
  it('returns zero metrics when evaluation fails', async () => {
    ...
    const metrics = await engine.extractMetrics(page);
    expect(metrics).toEqual({ likes: 0, retweets: 0, replies: 0, views: 0, bookmarks: 0 });
  });
  ...
  it('extracts deep replies when other strategies are empty', async () => {
    ...
    page.evaluate = vi.fn((fn, arg) => {
      ...
      if (source.includes('window.scrollBy') || source.includes('window.scrollTo')) return undefined;
  ...
  it('summarizes reply sentiment distribution', () => {
    ...
    expect(summary.distribution.positive + summary.distribution.negative + summary.distribution.neutral).toBe(3);
  });
  ...
  it('extracts enhanced context with metrics and screenshot', async () => {
    const page = createPageMock();
    const metrics = { likes: 10, retweets: 2, replies: 1, views: 100, bookmarks: 0 };
    ...
    vi.spyOn(engine, 'analyzeSentiment').mockReturnValue({ overall: 'positive', score: 0.4 });
    vi.spyOn(engine, 'detectTone').mockReturnValue({ primary: 'serious' });
    vi.spyOn(engine, 'classifyConversation').mockReturnValue('general');
    vi.spyOn(engine, 'analyzeReplySentiment').mockReturnValue({ overall: 'neutral' });
