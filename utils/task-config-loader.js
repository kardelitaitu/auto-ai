...
export class TaskConfigLoader {
    ...
    async loadAiTwitterActivityConfig(payload = {}) {
        ...
        this.loadCount++;
        logger.info(`[ConfigLoader] Loading configuration (cache miss: ${cacheKey})`);
        ...
        try {
            ...
            logger.info(`[ConfigLoader] Configuration loaded successfully in ${loadTime}ms`);
            logger.debug(`[ConfigLoader] Cache stats: ${this.hitCount} hits, ${this.loadCount} loads`);
    ...
    async loadSettings() {
        try {
            ...
            logger.warn(`[ConfigLoader] Failed to load settings, using defaults: ${error.message}`);
     ...
     * @private
     */
    buildConfig({ settings, activityConfig, timingConfig, engagementLimits, humanizationConfig, payload }) {
        return {
            ...
            session: {
                ...
                timeout: payload.taskTimeoutMs ?? (activityConfig.defaultMinDuration + activityConfig.defaultMaxDuration) * 1000
            },
            ...
            monitoring: {
                ...
                engagementProgress: {
                    ...
                    showProgressBar: settings.logging?.engagementProgress?.showProgressBar ?? true
                }
            },
...
export const loadAiTwitterActivityConfig = (payload) => taskConfigLoader.loadAiTwitterActivityConfig(payload);
