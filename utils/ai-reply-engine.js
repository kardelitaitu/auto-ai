...
export class AIReplyEngine {
  ...
  async shouldReply(tweetText, authorUsername, context = {}) {
    ...
    logger.info(
      `[AIReply]   - Risk Level: ${tweetSentiment.composite?.riskLevel || "N/A"}`,
    );
    logger.info(
    ...
    logger.info(
      `[AIReply] Generated reply: "${aiResult.reply.substring(0, 50)}..."`,
    );
  ...
  async generateReply(tweetText, authorUsername, context = {}) {
    ...
    if (replies.length > 0) {
      ...
      logger.info(
        `[AIReply] Reply selection strategy: ${sentimentAnalysis.strategy} ` +
      ...
      logger.info(
        `[AIReply] Selected ${selectedReplies.length} replies for LLM context ` +
    ...
    logger.info(`[DEBUG] ----------------------------------------------`);
    logger.info(
      `[DEBUG] REPLIES CONTEXT (${replies.length} loaded, showing longest 30):`,
    );
    ...
    sortedDebugReplies.forEach((reply, idx) => {
      ...
      logger.info(
        `[DEBUG] [${idx + 1}] Reply${idx + 1}: "@${authorName}: ${shortText}${ellipsis}"`,
      );
    });
    logger.info(`[DEBUG] ----------------------------------------------`);
    ...
    logger.info(
      `[DEBUG] System Prompt Length: ${REPLY_SYSTEM_PROMPT.length} chars`,
    );
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        logger.info(
          `[AIReply] Generating ${promptData.language} reply (attempt ${attempt}/${maxAttempts})...`,
        );
        ...
        const estimatedInputTokens = Math.ceil(
          (REPLY_SYSTEM_PROMPT.length + promptData.text.length) / 4,
        );
        ...
        logger.info(
          `[DEBUG]   Input Tokens: ~${estimatedInputTokens} (system + prompt + ${replies.length} replies)`,
        );
        logger.info(
          `[DEBUG]   Output Tokens: ~${estimatedOutputTokens} (response)`,
        );
        ...
        let reply = this.extractReplyFromResponse(response.content);

        if (reply) {
          // Intercept and replace addresses if needed
          reply = this.interceptAddress(reply);

          logger.info(
            `[AIReply] Generated ${promptData.language} reply: "${reply.substring(0, 50)}..."`,
          );
          return {
            success: true,
            reply,
        ...
        logger.warn(
          `[AIReply] Generate attempt ${attempt} failed: ${error.message}`,
        );
      }
    }
  ...
  detectReplyLanguage(replies) {
    ...
    const allText = replies.map((r) => r.text || r.content || "").join(" ");
  ...
  buildEnhancedPrompt(
    ...
    const toneGuidance = this.getSentimentGuidance(
      sentiment,
      conversationType,
      sarcasmScore,
    );
    const lengthGuidance = this.getReplyLengthGuidance(
      conversationType,
      valence,
    );
...
Write ONE short reply (1 sentence max):`;
  ...
  async captureContext(page, tweetUrl = "") {
    ...
    try {
      ...
      const extractedReplies =
        await this.extractRepliesMultipleStrategies(page);
  ...
  async extractRepliesMultipleStrategies(page) {
      ...
      for (let i = 0; i < scrollUpSteps; i++) {
        ...
        for (const text of visibleReplies) {
          ...
          if (addReply(author, text)) {
            logger.debug(
              `[AIReply] Extracted reply from @${author}: "${text.substring(0, 50)}..."`,
            );
          }
        }
    ...
    if (replies.length < 50) {
      try {
        ...
        for (const selector of selectors) {
          try {
            ...
            if (elements.length > 0) {
              logger.debug(
                `[AIReply] Selector "${selector}": ${elements.length} elements`,
              );
            ...
            logger.debug(
              `[AIReply] Selector "${selector}" error: ${e.message}`,
            );
          }
        }
        ...
        if (replies.length >= 50) {
          logger.debug(
            `[AIReply] Strategy 1 success: ${replies.length} replies`,
          );
    ...
    if (replies.length < 50) {
      try {
        ...
        if (replies.length >= 50) {
          logger.debug(
            `[AIReply] Strategy 2 success: ${replies.length} replies`,
          );
          await this._returnToMainTweet(page);
          return replies.slice(0, 50);
        }
      ...
      try {
        ...
        const bodyText = await page.evaluate(() => {
          ...
          const sections = document.querySelectorAll(
            '[role="region"], main, [aria-label*="Timeline"]',
          );
    ...
    if (replies.length < 10) {
      logger.debug(
        `[AIReply] Strategy 5: ULTRA AGGRESSIVE - extracting any text from articles...`,
      );
      ...
      try {
        ...
        logger.debug(
          `[AIReply] Strategy 5 found ${ultraAggressive.length} potential replies`,
        );
        ...
        for (const text of ultraAggressive) {
          ...
          if (cleaned.length > 3 && cleaned.length < 200) {
            ...
            replies.push({ author, text: cleaned.substring(0, 280) });
            logger.debug(
              `[AIReply] Added from Strategy 5: "${cleaned.substring(0, 40)}..."`,
            );
          }
        }
      } catch (e) {
  ...
  async extractAuthorFromArticle(article) {
    try {
      ...
      const headerLink = await article.$(
        'a[href^="/"][role="link"], a[href*="/status/"]',
      );
  ...
  /**
   * Intercept and replace EVM addresses in the reply with a random one from local list
   * Only runs if the reply STARTS with an EVM address (0x...)
   * @param {string} reply - The generated reply
   * @returns {string} Reply with potentially replaced address
   */
  interceptAddress(reply) {
    if (!reply) return reply;

    // Only intercept if reply STARTS with EVM address (0x followed by 40 hex chars)
    const startsWithEvmRegex = /^0x[a-fA-F0-9]{40}/;

    if (!startsWithEvmRegex.test(reply.trim())) {
      return reply;
    }

    // Also check for any EVM address in the reply for replacement
    const evmRegex = /0x[a-fA-F0-9]{40}/g;

    try {
      const addressFilePath = path.join(process.cwd(), "evm_address.txt");
      if (!fs.existsSync(addressFilePath)) {
        this.logger.debug(
          `[AIReply] evm_address.txt not found at ${addressFilePath}, skipping interception`,
        );
        return reply;
      }
      ...
      if (addresses.length === 0) {
        this.logger.debug(
          `[AIReply] No valid addresses found in evm_address.txt`,
        );
        return reply;
      }

      const randomAddress = mathUtils.sample(addresses);
      this.logger.info(
        `[AIReply] Reply starts with EVM address, replacing with: ${randomAddress}`,
      );

      // Replace all occurrences of any EVM address with our random one
      return reply.replace(evmRegex, randomAddress);
    } catch (error) {
      this.logger.error(
        `[AIReply] Error during address interception: ${error.message}`,
      );
      return reply;
    }
  }
  ...
  async adaptiveRetry(operation, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const finalDelay = calculateBackoffDelay(attempt - 1, {
          baseDelay,
          maxDelay,
          factor: backoffMultiplier,
          jitterMin: jitter ? 0.5 : 1,
          jitterMax: jitter ? 1 : 1,
        });

        this.logger.debug(
          `[AIReply] Attempt ${attempt} failed: ${lastError}, retrying in ${Math.round(finalDelay)}ms...`,
        );
        await new Promise((resolve) => setTimeout(resolve, finalDelay));
      } catch (error) {
        lastError = error.message;
        this.logger.warn(
          `[AIReply] Attempt ${attempt} exception: ${error.message}`,
        );
    ...
    // All attempts failed
    this.logger.warn(
      `[AIReply] All ${maxAttempts} attempts failed: ${lastError}`,
    );
  ...
  generateQuickFallback(tweetText, authorUsername) {
    ...
    if (
      ...
      lowerTweet.includes("worst") ||
      lowerTweet.includes("why")
    ) {
      const responses = [
        "It's giving rough",
        "Too real",
  ...
  validateReplyAdvanced(reply, originalTweet = "") {
    ...
    if (reply.length > 250) {
      this.logger.debug(
        `[AIReply] Long reply detected (${reply.length} chars), may need trimming`,
      );
    }
    ...
    if (reply.length < 5) {
      this.logger.debug(
        `[AIReply] Very short reply detected (${reply.length} chars)`,
      );
    }
  ...
  normalizeReply(reply) {
    ...
    cleaned = cleaned
      .replace(/^(reply:|response:|my take:|answer:)/i, "")
      .trim();
  ...
  getStats() {
    ...
    return {
      ...
      successRate:
        total > 0
          ? ((this.stats.successes / total) * 100).toFixed(1) + "%"
          : "0%",
      skipRate:
        total > 0 ? ((this.stats.skips / total) * 100).toFixed(1) + "%" : "0%",
    };
  }
  ...
  async executeReply(page, replyText, options = {}) {
    this.logger.info(
      `[AIReply] Executing reply (${replyText.length} chars)...`,
    );
    ...
    try {
      ...
      this.logger.error(
        `[AIReply] Method ${selected.name} failed: ${error.message}`,
      );
  ...
  async replyMethodA_Keyboard(page, replyText, human) {
    ...
    const focusMainTweet = async () => {
      ...
      for (const selector of focusSelectors) {
        try {
          ...
          if ((await el.count()) > 0) {
            const success = await human.safeHumanClick(
              el,
              "Main Tweet - Focus",
              3,
            );
    ...
    const replyContext = await page.evaluate(() => {
      const el =
        ...
        document.querySelector('textarea[placeholder*="Post your reply"]');
    ...
    if (!replyContext) {
      human.logStep(
        "VERIFY_CONTEXT_FAILED",
        "Composer not in reply context, switching to button flow",
      );
      await page.keyboard.press("Escape");
      await new Promise((resolve) => setTimeout(resolve, 300));
      return await this.replyMethodB_Button(page, replyText, human);
    ...
    // Post with Ctrl+Enter or Reply Button
  ...
  async replyMethodB_Button(page, replyText, human) {
    ...
    if (!mainTweetClicked) {
      human.logStep(
        "FOCUS_MAIN",
        "Could not click main tweet, continuing anyway",
      );
    }
    ...
    const btnResult = await human.findElement(page, replyBtnSelectors, {
      visibleOnly: true,
    });
    ...
    try {
      ...
      if (btnBox && btnBox.y > viewportHeight * 0.5) {
        human.logStep(
          "WARNING",
          `Button at y=${Math.round(btnBox.y)} > 50% of viewport - might be on reply thread`,
        );
        // Scroll to top and retry
        await page.evaluate(() => window.scrollTo(0, 0));
        await new Promise((resolve) => setTimeout(resolve, 1000));

        const btnResult2 = await human.findElement(page, replyBtnSelectors, {
          visibleOnly: true,
        });
        ...
        human.logStep(
          "POSITION_OK",
          `Button at y=${Math.round(btnBox.y)} (top half of viewport)`,
        );
      }
    } catch (e) {
    ...
    const clickSuccess = await human.safeHumanClick(
      btnResult.element,
      "Reply Button",
      3,
    );
    if (!clickSuccess) {
      human.logStep(
        "CLICK_FAILED",
        "humanClick failed after retries, trying force click",
      );
  ...
  async replyMethodC_Tab(page, replyText, human) {
    ...
    for (let i = 0; i < maxTabs; i++) {
      ...
      const isReplyButton =
        (ariaLower.includes("reply") &&
          ...
          !ariaLower.includes("replying")) ||
        (focused.tag === "BUTTON" && ariaLower.includes("reply"));

      if (isReplyButton) {
        human.logStep(
          "TAB_SUCCESS",
          `Found reply button at tab ${tabCount}: aria="${focused.aria}"`,
        );
        break;
      } else if (tabCount % 5 === 0) {
        human.logStep(
          "TAB_DEBUG",
          `Tab ${tabCount}: ${focused.tag} aria="${focused.aria}"`,
        );
      }
    }
    ...
    if (!verify.open) {
      human.logStep(
        "ENTER_FAILED",
        "Composer did not open with Enter, trying R key",
      );
  ...
  async replyMethodD_RightClick(page, replyText, human) {
    ...
    const replyBtnSelectors = ['[data-testid="reply"]', '[aria-label="Reply"]'];
    ...
    const contextMenuOpen =
      (await page.locator('[role="menuitem"], [role="menu"]').count()) > 0;
    human.logStep("CONTEXT_MENU", contextMenuOpen ? "Opened" : "Not opened");

    // Try clicking Reply from menu if visible
    if (contextMenuOpen) {
      const replyOption = page
        .locator("text=Reply", '[aria-label*="Reply"]')
        .first();
