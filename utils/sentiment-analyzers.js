...
export class ValenceAnalyzer {
    ...
    buildWordWeights(lexicon) {
        ...
        for (const [intensity, words] of Object.entries(lexicon)) {
            const weight = intensity === 'strong' ? 1.0 :
                          intensity === 'moderate' ? 0.6 :
                          intensity === 'weak' ? 0.3 : 0.5;
...
export class SarcasmAnalyzer {
    ...
    hasContradictoryPhrasing(text) {
        const positiveSuperlatives = ['wonderful', 'fantastic', 'amazing', 'brilliant',
                                     'perfect', 'excellent', 'outstanding', 'incredible'];
        const negativeContext = ['disaster', 'fail', 'bad', 'worse', 'worst',
                               'terrible', 'awful', 'hate', 'sucks', 'stupid'];
