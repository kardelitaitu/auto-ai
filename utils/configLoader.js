  ...
  async loadConfig(filename, defaults = {}) {
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        logger.warn(`Configuration file ${filename}.json not found, using defaults`);
      } else {
    ...
    for (const [key, rules] of Object.entries(schema)) {
      ...
      if (key in config) {
        ...
        if (rules.type && typeof value !== rules.type) {
          errors.push(`Configuration key '${key}' must be of type ${rules.type}`);
        }
        
        if (rules.type === 'number') {
          if (rules.min !== undefined && value < rules.min) {
            errors.push(`Configuration key '${key}' must be at least ${rules.min}`);
          }
          if (rules.max !== undefined && value > rules.max) {
            errors.push(`Configuration key '${key}' must be at most ${rules.max}`);
          }
        }
      }
    }
...
export async function getTimeouts() {
  return configLoader.loadConfig('timeouts', {
    ...
    automation: {
      ...
      scrolling: { minReadDelayMs: 1000, maxReadDelayMs: 3500, minPauseMs: 500, maxPauseMs: 1500 }
    },
