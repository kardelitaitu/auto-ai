...
export class AITwitterAgent extends TwitterAgent {
  ...
  async startDive() {
    ...
    while (this.operationLock) {
      // AUTO-EXPIRING LOCK SAFETY (Max 3 minutes)
      if (this._operationLockTimestamp && Date.now() - this._operationLockTimestamp > 180000) {
        this.log(`[DiveLock] 🚨 CRITICAL: Lock held for > 3 minutes. Force releasing stale lock.`);
        this.operationLock = false;
        break; // Break so we can acquire it
      }
      ...
      if (firstWait || now - this.lastWaitLogTime >= this.waitLogInterval) {
        const timeSince = this.lastWaitLogTime ? ((now - this.lastWaitLogTime) / 1000).toFixed(0) : "0";
        this.log(
          `[DiveLock] ⏳ Waiting for existing operation to complete... (${timeSince}s since last check)`,
        );
  ...
  async endDive(success = true, returnHome = false) {
    ...
    this.operationLock = false;
    this._operationLockTimestamp = null;
  ...
  async _safeNavigateHome() {
    try {
      ...
      // Try to navigate to home with hard timeout
      const hardTimeoutMs = TWITTER_TIMEOUTS.NAVIGATION || 30000;
      const hardTimeout = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('HARD_NAVIGATION_TIMEOUT')), hardTimeoutMs)
      );
      ...
      this.log(`[DiveLock] Navigation error: ${error.message}. Attempting hard fallback.`);
      // Fallback: direct navigation
      try {
        const hardTimeoutMs = TWITTER_TIMEOUTS.NAVIGATION || 30000;
        const hardTimeout2 = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('HARD_FALLBACK_TIMEOUT')), hardTimeoutMs)
        );

        await Promise.race([
          api.goto("https://x.com/home", {
        ...
        this.log(`[DiveLock] 🚨 Fallback navigation failed completely: ${e.message}`);
        return false;
      }
    }
  }
  ...
  async _postDiveHomeScroll() {
    try {
      ...
      this.log(`[DiveLock] Home scroll failed: ${error.message}`);
    }
  }
  ...
  async diveTweet() {
    ...
    try {
      ...
      const diveSequence = this._diveTweetWithAI(async (aiTask) => {
      ...
      // Unified hard timeout for the entire dive sequence (3 minutes)
      const hardTimeoutMs = 180000;
      let hardTimeoutId;
      const hardTimeoutPromise = new Promise((_, reject) => {
        hardTimeoutId = setTimeout(() => {
          reject(new Error(`HARD_DIVE_TIMEOUT: Entire dive sequence exceeded ${hardTimeoutMs}ms`));
        }, hardTimeoutMs);
      });

      try {
        await Promise.race([diveSequence, hardTimeoutPromise]);
      } finally {
      ...
      // but we ensure cleanup if something crashed or hard timed out before that.
      if (this.operationLock) {
        this.log(`[DiveLock] 🧹 Force releasing lock in finally block for ${diveId}`);
        await this.endDive(false, true).catch(() => {
          // Failsafe if endDive itself crashes
          this.operationLock = false;
          this._operationLockTimestamp = null;
          this.diveLockAcquired = false;
          this.scrollingEnabled = true;
  ...
  async _diveTweetWithAI(queueWrapper = null) {
    try {
      ...
      if (clickTarget) {
        ...
        const permalinkHref = await permalinkLink
          .getAttribute("href")
          .catch(() => null);
        const selectorForApi = permalinkHref
          ? `a[href="${String(permalinkHref).replace(/\\"/g, "\\\\\"")}"]`
          : null;

        this.log("[Attempt] API Click on Permalink...");

        await api.withPage(this.page, async () => {
          if (selectorForApi) {
            await api.click(selectorForApi, {
              precision: "high",
        ...
        try {
          await this.page.waitForURL("**/status/**", { timeout: 1500 });
        } catch (_e) {
          if (selectorForApi) {
            this.log("[Retry] API smartClick on Permalink...");
            await api.withPage(this.page, async () => {
              await api.smartClick(selectorForApi).catch(() => null);
            });
          }
        }
      }
      ...
      const safeWaitForURL = async (urlPattern, timeoutMs) => {
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('WAIT_FOR_URL_TIMEOUT')), timeoutMs)
        );
        return Promise.race([
      ...
      try {
        ...
        await safeWaitForURL("**/status/**", TWITTER_TIMEOUTS.NAVIGATION);
        tweetUrl = api.getCurrentUrl();
      ...
      // Smart wait for tweet content to fully load with text representation
      try {
        await this.page.waitForFunction(() => {
          const els = document.querySelectorAll(
            '[data-testid="tweetText"], [lang] > div, article [role="group"], .tweet-text'
          );
      ...
      await api.wait(1000); // Brief final settle time
      ...
      for (const selector of alternatives) {
        ...
        if ((await altEl.count()) > 0) {
          ...
          if (altText.trim().length > tweetText.length) {
            tweetText = altText.trim();
            this.log(
              `[AI] Found tweet text (${altText.length} chars) using ${selector}`
            );
            if (tweetText.length > 10) break; // Good enough
          }
        }
      }

      if (!tweetText) {
        this.log("[AI] WARNING: Could not find any valid tweet text elements!");
      }
      ...
      const executeAILogic = async () => {
        ...
        if (!action) {
          ...
          const context = {
            ...
            username,
            tweetUrl,
            tweetElement: this.page.locator('article[data-testid="tweet"]').first(),
  ...
  async _ensureExplorationScroll() {
    ...
    const docHeight = await this.page.evaluate(
    ...
    if (currentY < 100) {
      this.log(
        `[Scroll] Starting from top, performing exploration scroll (${this._scrollExplorationThreshold}px)...`,
      );
  ...
  async runSession(
    ...
    try {
      ...
      this.log(`Starting Session on ${await api.getCurrentUrl()}`);
        ...
        switch (errorType) {
          ...
          case "skip": {
            if (i < chars.length - 1 && Math.random() < 0.5) {
              ...
              await this.page.keyboard.type(nextCharTyped, {
                delay: baseDelay,
              });
              await api.wait(mathUtils.randomInRange(500, 1500));
              await this.page.keyboard.press("Backspace", {
  ...
  async handleAIQuote(tweetText, username, options = {}) {
    ...
    // Log basic sentiment (backward compatible)
    this.log(
      `[Sentiment] ${sentimentResult.isNegative ? "🚫 NEGATIVE" : "✅ Neutral/Positive"} content (score: ${sentimentResult.score.toFixed(2)})`,
    );
    ...
    if (result.success) {
      ...
      for (let i = 0; i < 5; i++) {
        try {
          const composerVisible = await api.visible(this.page
  ...
  async performHealthCheck() {
    try {
      ...
      const pageHealth = await this.page
        .evaluate(() => {
          return {
  ...
  async shutdown() {
    ...
    try {
      if (typeof this.shutdownLegacy === 'function') {
        this.shutdownLegacy();
      }
    } catch (e) {
      this.logWarn(`[Shutdown] Error in legacy shutdown: ${e.message}`);
    }
